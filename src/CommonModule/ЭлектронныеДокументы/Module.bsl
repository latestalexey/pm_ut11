...
//    расположеной в первой строке таблицы-владельца. (подробнее см. схему ХДТО "АктТитулИсполнителя", список "ОписРабот"
//    , вложенный список "Работа").
//
Процедура ДобавитьДанныеВДеревоДопДанных(СтруктураПараметров,
										СтруктураДанных,
...
//                      будут помещены в структуру, для возможности последующкго разбора на клиенте.
//
// Возвращаемое значение:
...
Функция ПолучитьТекстЗапросаЭлектронныхДокументовНаПодписи(ДляРабочегоСтола = Истина, СтруктураДопОтборов = Неопределено) Экспорт
	...
	Иначе
		...
		|	ЕСТЬNULL(СертификатыНаПодпись.СуммаДокументаВладельца, 0) КАК СуммаДокумента,
	...
	|	ИЗ
	|		(ВЫБРАТЬ 
	|			%ПолеСуммыДокумента% КАК СуммаДокументаВладельца,
	...
	ИмяРеквизитаСуммы = ЭлектронныеДокументыПереопределяемый.ОпределитьИмяРеквизитаСуммы();
	ПолеСуммыДокумента = ?(ИмяРеквизитаСуммы=Неопределено,"0", "СостоянияЭД.СсылкаНаОбъект." + ИмяРеквизитаСуммы);
	ТекстЗапросаОснова = СтрЗаменить(ТекстЗапросаОснова, "%ПолеСуммыДокумента%", ПолеСуммыДокумента);
