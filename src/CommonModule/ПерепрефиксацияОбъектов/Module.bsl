...
Процедура УстановитьНовыйПрефиксИВыполнитьОбработкуДанныхСНестандартнымФорматомКода(
	...
	Если ПредыдущийПрефиксИнформационнойБазы = Неопределено Тогда
		...
		ПредыдущийПрефиксИнформационнойБазы = СтандартныеПодсистемыПереопределяемый.ПрефиксИнформационнойБазы();
	...
	Попытка
		...
		СтандартныеПодсистемыПереопределяемый.УстановитьПрефиксИнформационнойБазы(НовыйПрефикс);
	...
	Исключение
		...
		СтандартныеПодсистемыПереопределяемый.УстановитьПрефиксИнформационнойБазы(ПредыдущийПрефиксИнформационнойБазы);
		
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Префиксация объектов.Изменение префикса информационной базы'"),
...
Процедура ДополнитьТаблицуМетаданных(Результат, Знач ИмяОбработчика)
	...
	Для Каждого Подписка Из ПодпискиНаСобытияПоИмениОбработчика(ИмяОбработчика) Цикл
		...
		Для Каждого ТипИсточника Из Подписка.Источник.Типы() Цикл
			...
			ИначеЕсли РазделениеВключено И Не ОбщегоНазначенияПовтИсп.ЭтоРазделенныйОбъектМетаданных(ИмяОбъекта) Тогда
...
Процедура ПроверитьВозможностьУстановкиПрефикса()
	...
	Если Не СтандартныеПодсистемыПереопределяемый.ЕстьФункциональнаяОпцияПрефиксИнформационнойБазы() Тогда
