...
Процедура ДобавитьПараметрыРаботыКлиента(Параметры) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		ВыводитьОповещениеОНеакутальности = Ложь; // В модели сервиса обновляется автоматически.
	ИначеЕсли НЕ ПравоДоступа("Изменение", Метаданные.Справочники.КлассификаторБанковРФ) Тогда
		ВыводитьОповещениеОНеакутальности = Ложь; // Пользователь без необходимых прав.
	ИначеЕсли КлассификаторОбновляетсяПоРасписанию() Тогда
		ВыводитьОповещениеОНеакутальности = Ложь; // Классификатор обновляется автоматически.
	Иначе
		ВыводитьОповещениеОНеакутальности = НЕ КлассификаторАктуален();
	КонецЕсли;
	
	Параметры.Вставить("Банки", Новый ФиксированнаяСтруктура("ВыводитьОповещениеОНеакутальности", ВыводитьОповещениеОНеакутальности));
...
Функция КлассификаторАктуален() Экспорт
	...
	ДопустимаяПросрочка = 3*60*60*24;
...
// Определяет запускается ли регламентное задание "ЗагрузкаКлассификатораБанковРФССайтаРБК" по расписанию.
//   В файловом режиме возвращает Ложь.
//
Функция КлассификаторОбновляетсяПоРасписанию()
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Задание = РегламентныеЗадания.НайтиПредопределенное(
		Метаданные.РегламентныеЗадания.ЗагрузкаКлассификатораБанковРФССайтаРБК
	);
