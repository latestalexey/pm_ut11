...
Функция ВидЦеныПрайсЛист() Экспорт
	...
	Иначе
		...
		ВидЦеныПрайсЛист.ВалютаЦены = Константы.ВалютаУправленческогоУчета.Получить();
...
Процедура ПроверитьКорректностьЗаполненияДокументаУстановкиЦенНоменклатурыПоставщика(ДокументУстановкиЦен,
	...
	Для Каждого ТекЭлемент Из МассивПроверок Цикл
		...
		Если ТекЭлемент = "ВременнаяТаблицаТовары" Тогда
			...
			СформироватьЗапросВременнаяТаблицаЦенДляРегистрацииЦенПоставщика(ТекстЗапроса, ПараметрыЗапроса, ДокументУстановкиЦен);
		...
		ИначеЕсли ТекЭлемент = "НаличиеУслуг" Тогда
			...
			СформироватьЗапросНаличиеУслугДляРегистрацииЦенПоставщика(ТекстЗапроса, ПараметрыЗапроса, ДокументУстановкиЦен);
	...
	Для ТекИндекс = 0 По МассивРезультатов.Количество()-1 Цикл
		...
		ИначеЕсли МассивПроверок[ТекИндекс] = "НаличиеУслуг" Тогда
			...
			СообщитьОбОшибкахНаличиеУслугДляРегистрацииЦенПоставщика(Выборка, ДокументУстановкиЦен, Отказ);
...
////////////////////////////////////////////////////////////////////////////////
// Процедуры и функции пересчета сумм в табличных частях.
...
Процедура ПересчитатьСуммыВСтрокеПоСуммеСНДС(
	...
	Если АвтоматическаяСкидка И СтрокаТаблицы.ПроцентАвтоматическойСкидки > 0 Тогда
		СтрокаТаблицы.СуммаАвтоматическойСкидки = Окр(СуммаБезСкидки*СтрокаТаблицы.ПроцентАвтоматическойСкидки/100, 2);
	КонецЕсли;
	
	Если РучнаяСкидка  И СтрокаТаблицы.ПроцентРучнойСкидки > 0 Тогда
...
// Параметры:
// Документ       - ДокументОбъект, для которого необходимо осуществить пересчет
// СуммаДокумента - Число - Сумма документа, которую необходимо распределить на этапы оплаты
//
Процедура РаспределитьСуммуПоЭтапамОплаты(Документ, Знач СуммаДокумента) Экспорт
	
	КоличествоЭтаповОплаты = Документ.ЭтапыГрафикаОплаты.Количество();
	ПроцентЭтаповОплаты    = Документ.ЭтапыГрафикаОплаты.Итог("ПроцентПлатежа");
	НомерЭтапа             = 1;
	РаспределеннаяСумма    = 0;
	
	Если КоличествоЭтаповОплаты = 0 Или ПроцентЭтаповОплаты = 0 Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого ТекЭтап Из Документ.ЭтапыГрафикаОплаты Цикл
		
		ЭтоПоследнийЭтап = ПроцентЭтаповОплаты = 100 И КоличествоЭтаповОплаты = НомерЭтапа;
		
		Если ЭтоПоследнийЭтап Тогда
			ТекЭтап.СуммаПлатежа = СуммаДокумента - РаспределеннаяСумма;
			
		Иначе
			
			ТекЭтап.СуммаПлатежа = Окр(СуммаДокумента * ТекЭтап.ПроцентПлатежа / 100, 2, РежимОкругления.Окр15как20);
			РаспределеннаяСумма  = РаспределеннаяСумма + ТекЭтап.СуммаПлатежа;
...
Процедура РассчитатьСуммыВзаиморасчетовВТабличнойЧасти(Документ, ИмяТабличнойЧасти) Экспорт
	...
	Если Документ.СуммаВзаиморасчетов = СуммаВзаиморасчетовПоТЧ
		ИЛИ Документ.СуммаВзаиморасчетов < СуммаВзаиморасчетовПоТЧ Тогда
		Возврат;
	КонецЕсли;
	...
	Если ВалютаДокумента = ВалютаЗаказа Тогда
		...
		Для н=0 По ТабличнаяЧасть.Количество()-1 Цикл
			Если Не ЗначениеЗаполнено(ТабличнаяЧасть[н].СуммаВзаиморасчетов) Тогда
				ТабличнаяЧасть[н].СуммаВзаиморасчетов = ТабличнаяЧасть[н].СуммаСНДС;
			КонецЕсли;
		КонецЦикла;
	...
	Иначе
		...
		Для н=0 По ТабличнаяЧасть.Количество()-1 Цикл
			Если Не ЗначениеЗаполнено(ТабличнаяЧасть[н].СуммаВзаиморасчетов) Тогда
				МассивСумм.Добавить(Окр(ТабличнаяЧасть[н].СуммаСНДС * КоэффициентПересчетаВВалютуЗаказа, 2));
			КонецЕсли;
		КонецЦикла;
		...
		Для н=0 По ТабличнаяЧасть.Количество()-1 Цикл
			Если Не ЗначениеЗаполнено(ТабличнаяЧасть[н].СуммаВзаиморасчетов) Тогда
				ТабличнаяЧасть[н].СуммаВзаиморасчетов = МассивСумм[нМассиваСумм];
				нМассиваСумм = нМассиваСумм + 1;
			КонецЕсли;
		КонецЦикла;
...
Процедура ОтразитьДействиеСкидокНаценок(ДополнительныеСвойства,
	                                    Движения,
	                                    Отказ) Экспорт
	...
	ДвиженияДействиеСкидокНаценок.Загрузить(Таблица);
...
////////////////////////////////////////////////////////////////////////////////
// Процедуры формирования запросов проверки регистрации цен.
...
КонецПроцедуры // СформироватьЗапросНаличиеУстановленныхЦенДляРегистрацииЦен()

////////////////////////////////////////////////////////////////////////////////
// Процедуры для вывода сообщений об ошибках регистрации цен.
...
////////////////////////////////////////////////////////////////////////////////
// Процедуры формирования запросов проверки регистрации цен поставщика.
...
Процедура СформироватьЗапросВременнаяТаблицаЦенДляРегистрацииЦенПоставщика(ТекстЗапроса,
	...
	ТекстЗапроса = ТекстЗапроса + "
		...
		|	ВременнаяТаблицаТовары.Номенклатура           КАК Номенклатура,
		|	ВременнаяТаблицаТовары.Характеристика         КАК Характеристика,
		|	ВременнаяТаблицаТовары.УсловиеЦеныПоставщика  КАК УсловиеЦеныПоставщика
		|ПОМЕСТИТЬ
		|	ВременнаяТаблицаТовары
		|ИЗ
		...
		ПараметрыЗапроса.Вставить("Товары", ДокументУстановкиЦен.Товары.Выгрузить(, "НомерСтроки,НоменклатураПоставщика,Номенклатура,Характеристика,УсловиеЦеныПоставщика"));
...
Процедура СформироватьЗапросКорректностьТоваровДляРегистрацииЦенПоставщика(ТекстЗапроса)
	...
	ТекстЗапроса = ТекстЗапроса + "
		...
		|	ДокументТовары.Номенклатура          КАК Номенклатура,
		|	ДокументТовары.Характеристика        КАК Характеристика,
		|	ДокументТовары.УсловиеЦеныПоставщика КАК УсловиеЦеныПоставщика
		|ИЗ
		|	ВременнаяТаблицаТовары КАК ДокументТовары
		|ГДЕ
		|	ДокументТовары.Номенклатура <> ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
		...
		|	ДокументТовары.Номенклатура,
		|	ДокументТовары.Характеристика,
		|	ДокументТовары.УсловиеЦеныПоставщика
		|ИМЕЮЩИЕ
		|	КОЛИЧЕСТВО (*) > 1
		|;
		|";
...
// Формирует текст запроса для проверки наличия услуг в документе установке цен номенклатуры поставщика
//
// Параметры:
// ТекстЗапроса          - Строка - текстовая строка, к которой необходимо добавить текст запроса
// ПараметрыЗапроса      - Структура - структура, содержащая параметры запроса
// ДокументУстановкиЦен  - ДокументОбъект.УстановкаЦенНоменклатуры - документ, к которому необходимость сформировать запрос
//
Процедура СформироватьЗапросНаличиеУслугДляРегистрацииЦенПоставщика(ТекстЗапроса,
	                                                                ПараметрыЗапроса,
	                                                                ДокументУстановкиЦен)
	...
	ТекстЗапроса = ТекстЗапроса + "
		...
		|	И ВЫРАЗИТЬ(&Соглашение КАК Справочник.СоглашенияСПоставщиками).ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПриемНаКомиссию)
		|УПОРЯДОЧИТЬ ПО
		|	НомерСтроки ВОЗР
		|;
		|";
		
	ПараметрыЗапроса.Вставить("Соглашение", ДокументУстановкиЦен.Соглашение);
	
КонецПроцедуры // СформироватьЗапросНаличиеУслугДляРегистрацииЦенПоставщика()
...
////////////////////////////////////////////////////////////////////////////////
// Процедуры для вывода сообщений об ошибках регистрации цен поставщика.
...
Процедура СообщитьОбОшибкахКорректностьТоваровДляРегистрацииЦенПоставщика(Выборка,
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.УсловиеЦеныПоставщика) Тогда
			
			Если ЗначениеЗаполнено(Выборка.Характеристика) Тогда
				
				ТекстОшибки = НСтр("ru='Номенклатура ""%Номенклатура%"" с характеристикой ""%Характеристика%"" с условием цены ""%УсловиеЦеныПоставщика%"" повторяется'");
				ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Номенклатура%",          Выборка.Номенклатура);
				ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Характеристика%",        Выборка.Характеристика);
				ТекстОшибки = СтрЗаменить(ТекстОшибки, "%УсловиеЦеныПоставщика%", Выборка.УсловиеЦеныПоставщика);
				
			Иначе
				
				ТекстОшибки = НСтр("ru='Номенклатура ""%Номенклатура%"" с условием цены ""%УсловиеЦеныПоставщика%"" повторяется'");
				ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Номенклатура%",          Выборка.Номенклатура);
				ТекстОшибки = СтрЗаменить(ТекстОшибки, "%УсловиеЦеныПоставщика%", Выборка.УсловиеЦеныПоставщика);
...
// Отказ                 - Булево - Флаг отказа от проведения документа
//
Процедура СообщитьОбОшибкахНаличиеУслугДляРегистрацииЦенПоставщика(Выборка,
	                                                               ДокументУстановкиЦен,
	                                                               Отказ)
	
	Пока Выборка.Следующий() Цикл
		
		ТекстОшибки = НСтр("ru='Для соглашения с хозяйственной операцией ""Передача на комиссию"" нельзя указывать номенклатуру с типом ""Услуга"" (строка %НомерСтроки% списка ""Товары"")'");
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Номенклатура%", Выборка.Номенклатура);
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%НомерСтроки%",  Выборка.НомерСтроки);
...
КонецПроцедуры // СообщитьОбОшибкахНаличиеУслугДляРегистрацииЦенПоставщика()
