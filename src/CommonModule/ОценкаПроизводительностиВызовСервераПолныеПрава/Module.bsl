...
Функция ЗафиксироватьДлительностьКлючевыхОпераций(Замеры) Экспорт
	...
	Для Каждого КлючеваяОперацияЗамер из Замеры Цикл
		...
		Для Каждого ДатаДанные Из Буфер Цикл
			...
			ДатаОкончания = Данные.Получить("ВремяОкончания");
			Если Длительность = Неопределено Тогда
				// Неоконченный замер, писать его пока рано
				Продолжить;	
			КонецЕсли;
			ЗафиксироватьДлительностьКлючевойОперации(КлючеваяОперацияСсылка, 
			    Длительность, ДатаДанные.Ключ, ДатаОкончания);
		КонецЦикла;										  
	КонецЦикла;
...
Процедура ЭкспортОценкиПроизводительности(КаталогиЭкспорта) Экспорт
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	МАКСИМУМ(Замеры.ДатаНачалаЗамера) КАК ДатаЗамера
	|ИЗ 
...
Процедура ЗафиксироватьДлительностьКлючевойОперации(КлючеваяОперацияСсылка, 
	                                              Длительность, 
												  ДатаНачалаКлючевойОперации,
												  ДатаОкончанияКлючевойОперации = Неопределено) Экспорт
												  
	ЧасовойПоясКлиента = ЧасовойПоясСеанса();												  
	Запись = РегистрыСведений.ЗамерыВремени.СоздатьМенеджерЗаписи();
	Запись.КлючеваяОперация = КлючеваяОперацияСсылка;
	Запись.ДатаНачалаЗамера = УниверсальноеВремя(ДатаНачалаКлючевойОперации, ЧасовойПоясКлиента);
	...
	Запись.ДатаЗаписи = УниверсальноеВремя(ТекущаяДатаСеанса(), ЧасовойПоясКлиента);
	Если ДатаОкончанияКлючевойОперации <> Неопределено Тогда
		 Запись.ДатаОкончания = УниверсальноеВремя(ДатаОкончанияКлючевойОперации, ЧасовойПоясКлиента);
	КонецЕсли;	
	Попытка
		Запись.Записать(Ложь);
	Исключение
	 	// Происходит, когда пытаемся записать данные с неуникальным набором измерений
		// Такая ситуация может быть, если замеры происходят на сервере. В этом случае нет
		// никакой возможности контролировать уникальность измерений.
	КонецПопытки;
КонецПроцедуры
...
Функция ЗамерыСРазделениемПоКлючевымОперациям(ВерхняяГраньДатыЗамеров)
	...
	|		РегистрСведений.ЗамерыВремени КАК Замеры
	|	ГДЕ
	|		Замеры.ДатаНачалаЗамера >= &ДатаПоследнейВыгрузки И
	|       Замеры.ДатаНачалаЗамера <= &ВерхняяГраньДатыЗамеров
	|	УПОРЯДОЧИТЬ ПО
	|		Замеры.ДатаНачалаЗамера";	
...
// Формирует запрос для рассчета индекса APDEX и устанавливает значения необходимых параметров
//
// Параметры:
	...
	Пока ВыборкаАпдекс.Следующий() Цикл
		...
		Если Замеры <> Неопределено Тогда
			Для Каждого Замер Из Замеры Цикл
				...
				ЗамерXML.tUTC = Замер.ДатаНачалаЗамера;
