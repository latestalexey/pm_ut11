...
Функция ПостоянныеВидыОграниченийПравОбъектовМетаданных() Экспорт
	...
	ОграниченияПрав = "";
	СтандартныеПодсистемыПереопределяемый.ЗаполнитьВидыОграниченийПравОбъектовМетаданных(ОграниченияПрав);
...
// Только для внутреннего использования.
Функция ТипыПодпискиНаСобытие(ПодпискаНаСобытие) Экспорт
	
	ОбъектМетаданных = Метаданные.ПодпискиНаСобытия.Найти(ПодпискаНаСобытие);
	
	ТипыПодписки = Новый Соответствие;
	
	Для каждого Тип Из ОбъектМетаданных.Источник.Типы() Цикл
		ТипыПодписки.Вставить(Тип, Истина);
	КонецЦикла;
	
	Возврат ТипыПодписки;
