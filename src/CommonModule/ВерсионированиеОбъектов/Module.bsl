...
Процедура СоздатьВерсиюОбъекта(Объект, АвторВерсии = Неопределено, Знач ТипВерсииОбъекта = Неопределено, Комментарий = "",
	...
	Если Ссылка.Пустая() Тогда
		ТипВерсииОбъекта = Перечисления.ТипыВерсийОбъекта.НепринятыеДанныеПоДатеЗапретаОбъектНеСуществует;
		Если ЗначениеЗаполнено(УникальныйИдентификаторСтрокой) Тогда
