...
Процедура УведомитьИсполнителейОНовыхЗадачах() Экспорт
	...
	//{ ООО "АСТЭК"
	Задание=РегламентныеЗадания.НайтиПредопределенное(Метаданные.РегламентныеЗадания.УведомлениеИсполнителейОНовыхЗадачах);
	Интервал=Задание.Расписание.ПериодПовтораВТечениеДня;
	//}
	Если (ДатаПоследнегоУведомления = '00010101000000') 
		Или (ДатаУведомления - ДатаПоследнегоУведомления > Интервал) Тогда
		ДатаПоследнегоУведомления = ДатаУведомления - Интервал;
	КонецЕсли;
...
Процедура ПроконтролироватьЗадачи() Экспорт
	...
	АК_ТекстПисьмаНачальству = "";
	АК_КоличествоПросроченныхЗадач = 0;   
	
    ИсполнительСтарый =  NULL;
	...
	Для Каждого ЭлементПросроченныеЗадачи Из ПросроченныеЗадачи Цикл
		...
		Если ЗначениеЗаполнено(ПросроченнаяЗадача.Исполнитель) Тогда
			...
			ИсполнительНовый = ПросроченнаяЗадача.Исполнитель;
			...
			Если ПросроченнаяЗадача.Автор <> Строка(Константы.акРуководитель.Получить()) ИЛИ
				ПросроченнаяЗадача.Автор <> Строка(Константы.акЗамРуководителя.Получить())	Тогда
				    ИсполнительНовый =  ПросроченнаяЗадача.Исполнитель;
					
				Если ИсполнительСтарый = ИсполнительНовый Тогда
					АК_ТекстПисьмаНачальству = АК_ТекстПисьмаНачальству + ТекстПисьма;
				Иначе
					КолВоПросроченныхЗадачСотрудником = 0;
					Для Каждого ЗадачиИсполнителя Из ПросроченныеЗадачи Цикл
						Если ИсполнительНовый = ЗадачиИсполнителя.Исполнитель ТОГДА	
							КолВоПросроченныхЗадачСотрудником = КолВоПросроченныхЗадачСотрудником +1;
						КонецЕсли;
					КонецЦикла;	
					КолВоПросроченныхЗадачСотрудником = КолВоПросроченныхЗадачСотрудником -1;
					Если  КолВоПросроченныхЗадачСотрудником<>0 Тогда
						АК_ТекстПисьмаНачальству = АК_ТекстПисьмаНачальству + Символы.ПС + Символы.ПС + "Задачи, просроченные ("+ Строка(ИсполнительНовый) + ")   Количество : " + Строка(КолВоПросроченныхЗадачСотрудником) + Символы.ПС + Символы.ПС;
						КолВоПросроченныхЗадачСтрудником = 0;
					КонецЕсли;
				КонецЕсли;				
				АК_КоличествоПросроченныхЗадач = АК_КоличествоПросроченныхЗадач + 1;
			КонецЕсли;
			
			ИсполнительСтарый = ИсполнительНовый;	
		Иначе
	...
	НайтиПисьмоИДобавитьТекст(НаборПисемПоАдресатам, ПолучательПисьма, Константы.акЗамРуководителя.получить(), АК_ТекстПисьмаНачальству, "Начальству");
	//
	//   //Письмо Для Руководителя
	 ПолучательПисьма = "";
	СтандартныеПодсистемыПереопределяемый.ПолучитьАдресЭлектроннойПочты(Константы.акРуководитель.получить(), ПолучательПисьма);
	НайтиПисьмоИДобавитьТекст(НаборПисемПоАдресатам, ПолучательПисьма, Константы.акРуководитель.получить(), АК_ТекстПисьмаНачальству, "Начальству");
	...
	Для Каждого ПисьмоИзНабора Из НаборПисемПоАдресатам Цикл
		...
		ИначеЕсли ПисьмоИзНабора.ТипПисьма = "Координатору" Тогда
			...
			ТекстТемыПисьма = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				Строка(ПисьмоИзНабора.КоличествоЗадач));
			ПараметрыПисьма.Вставить("Тема", ТекстТемыПисьма);
		ИначеЕсли ПисьмоИзНабора.ТипПисьма = "Начальству" Тогда
			
			ТекстТелаПисьма = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'По введенным задачам прошел крайний срок (" +  Строка(АК_КоличествоПросроченныхЗадач)  +") : " + символы.ПС + "%1'" ) , ПисьмоИзНабора.ТекстПисьма);
			ПараметрыПисьма.Вставить("Тело", ТекстТелаПисьма);
			
			
			ТекстТемыПисьма = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'По задачам истек контрольный срок: %1'") ,    
				Строка(АК_КоличествоПросроченныхЗадач));
			ПараметрыПисьма.Вставить("Тема", ТекстТемыПисьма);
...
Функция ОтправитьУведомлениеОНовыхЗадачах(Исполнитель, ЗадачиПоИсполнителю)
	...
	Для Каждого СтрокаЗадача Из ЗадачиПоИсполнителю.Строки Цикл
 		    ТекстПисьма = ТекстПисьма + СформироватьПредставлениеЗадачи(СтрокаЗадача);  
	КонецЦикла;
...
Функция СформироватьПредставлениеЗадачи(ЗадачаСтруктура)
	...
	Если ЗначениеЗаполнено(ЗадачаСтруктура.ссылка) Тогда
		ШаблонСтроки = ШаблонСтроки + Символы.ПС + НСтр("ru = '%9'") + Символы.ПС;     
	КонецЕсли;
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСтроки,
		...
		ЗадачаСтруктура.Описание,ЗадачаСтруктура.ссылка);
