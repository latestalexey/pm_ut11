...
//
Функция ПолучитьСообщениеОбменаВоВременныйКаталог(Отказ, УзелИнформационнойБазы, ВидТранспортаСообщенийОбмена, МенеджерНастроекТранспорта = "", ВыводитьСообщения = Истина) Экспорт
	...
	СтруктураНастроекОбмена = ОбменДаннымиПовтИсп.ПолучитьСтруктуруНастроекТранспорта(УзелИнформационнойБазы, ВидТранспортаСообщенийОбмена, МенеджерНастроекТранспорта);
...
Функция СтруктураНастроекОбменаЧерезВнешнееСоединение(Структура) Экспорт
	...
	СтруктураНастроекОбменаВнешнееСоединение.Вставить("ИмяФайлаПротоколаОбмена",          ДобавитьЛитералКИмениФайла(Структура.ИмяФайлаПротоколаОбмена, "ВнешнееСоединение"));
...
Процедура ВыполнитьДействиеОбменаДляУзлаИнформационнойБазы(
	...
	СтруктураНастроекОбмена = ОбменДаннымиПовтИсп.ПолучитьСтруктуруНастроекОбменаДляУзлаИнформационнойБазы(
		УзелИнформационнойБазы, ДействиеПриОбмене, ВидТранспортаСообщенийОбмена,, МенеджерНастроекТранспорта
	);
...
Процедура ВыполнитьСтандартнуюЗагрузкуИзмененийДляУзла(
	...
	Попытка
		...
		Пока ВозможностьЧтенияXML(ЧтениеXML) Цикл
			...
			Отказ = Ложь;
			СообщениеОбОшибке = "";
			СтандартныеПодсистемыПереопределяемый.ПередЗаписьюДанныхИзСообщенияОбмена(
				Данные, УзелИнформационнойБазы, Отказ, СообщениеОбОшибке);
...
Функция ТаблицаМонитораОбменаДанными(ПланыОбмена, Знач ДополнительныеСвойстваПланаОбмена = "", ТолькоОшибочные = Ложь) Экспорт
	...
	Если ТолькоОшибочные Тогда
		...
		Отбор = "ГДЕ
				|	ЕСТЬNULL(СостоянияОбменовДаннымиВыгрузка.РезультатВыполненияОбмена, 0) <>0 ИЛИ ЕСТЬNULL(СостоянияОбменовДаннымиЗагрузка.РезультатВыполненияОбмена, 0) <> 0";
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "[ТолькоОшибочные]", Отбор);
...
Процедура ЗафиксироватьЗавершениеОбменаВРегистреСведений(СтруктураНастроекОбмена)
	...
	Если СтруктураНастроекОбмена.ДействиеПриОбмене = Перечисления.ДействияПриОбмене.ЗагрузкаДанных
		И СтруктураНастроекОбмена.РезультатВыполненияОбмена = Перечисления.РезультатыВыполненияОбмена.Выполнено Тогда
		
		Если ЭтоРазделенныйПланОбменаБСП(СтруктураНастроекОбмена.ИмяПланаОбмена) Тогда
			
			КоличествоПроблем = РегистрыСведений.РезультатыОбменаДанными.КоличествоПроблем( , СтруктураНастроекОбмена.УзелИнформационнойБазы);
			
			Если КоличествоПроблем > 0 Тогда
				
				СтруктураЗаписи.РезультатВыполненияОбмена = Перечисления.РезультатыВыполненияОбмена.ВыполненоСПредупреждениями;
...
Процедура ОбработчикПриВыгрузкеДанныхБСП(СтандартнаяОбработка,
	...
	СтандартныеПодсистемыПереопределяемый.ПриВыгрузкеДанных(СтандартнаяОбработка,
											Получатель,
											ИмяФайлаСообщения,
											ДанныеСообщения,
											КоличествоЭлементовВТранзакции,
...
Процедура ОбработчикПриЗагрузкеДанныхБСП(СтандартнаяОбработка,
	...
	СтандартныеПодсистемыПереопределяемый.ПриЗагрузкеДанных(СтандартнаяОбработка,
											Отправитель,
											ИмяФайлаСообщения,
											ДанныеСообщения,
											КоличествоЭлементовВТранзакции,
...
Функция ПолучитьСтруктуруНастроекОбменаДляУзлаИнформационнойБазы(
	...
	ВыполнитьИнициализациюСтруктурыНастроекОбменаДляУзлаИнформационнойБазы(СтруктураНастроекОбмена, ИспользоватьНастройкиТранспорта, МенеджерНастроекТранспорта);
...
//
Функция ПолучитьСтруктуруНастроекТранспорта(УзелИнформационнойБазы, ВидТранспортаСообщенийОбмена, МенеджерНастроекТранспорта = "") Экспорт
	...
	ВыполнитьИнициализациюСтруктурыНастроекОбменаДляУзлаИнформационнойБазы(СтруктураНастроекОбмена, Истина, МенеджерНастроекТранспорта);
...
Процедура ВыполнитьИнициализациюСтруктурыНастроекОбменаДляУзлаИнформационнойБазы(
		...
		Знач МенеджерНастроекТранспорта = ""
	)
	...
	Если ПустаяСтрока(МенеджерНастроекТранспорта) Тогда
		СтруктураНастроекОбмена.НастройкиТранспорта = РегистрыСведений.НастройкиТранспортаОбмена.НастройкиТранспорта(СтруктураНастроекОбмена.УзелИнформационнойБазы);
	Иначе
		СтруктураНастроекОбмена.НастройкиТранспорта = РегистрыСведений[МенеджерНастроекТранспорта].НастройкиТранспорта(СтруктураНастроекОбмена.УзелИнформационнойБазы);
	КонецЕсли;
...
//
Функция ВыгрузкаСсылкиРазрешена(УзелОбмена, Ссылка) Экспорт
...
Процедура ИнтерактивноеИзменениеВыгрузки_СформироватьТабличныйДокументПользователя(СтруктураОбработки, АдресРезультата, ПолноеИмяМетаданных, Представление) Экспорт
    ...
    ЗаполнитьЗначенияСвойств(ОбъектОтчета, СтруктураОбработки);
