...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	|ВЫБРАТЬ
	|	Участники.Партнер,
	|	Участники.РольПартнера,
	|	Участники.РольКонтактногоЛица,
	|	Участники.Комментарий,
	|	Участники.ВыигралСделку,
	|	Участники.ИсходныйНомерСтроки,
	|	Участники.КонтактноеЛицо
	|ПОМЕСТИТЬ Участники
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	УчастникиСделки.Партнер,
	|	УчастникиСделки.РольПартнера,
	|	УчастникиСделки.КонтактноеЛицо,
	|	УчастникиСделки.РольКонтактногоЛица,
	|	УчастникиСделки.Комментарий,
	|	УчастникиСделки.ВыигралСделку,
	|	УчастникиСделки.ИсходныйНомерСтроки
	|ИЗ
	|	Участники КАК УчастникиСделки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Партнеры КАК Партнеры
	|		ПО УчастникиСделки.Партнер = Партнеры.Ссылка";
	
	Запрос.УстановитьПараметр("Участники",ПолучитьИзВременногоХранилища(Параметры.АдресУчастники).Выгрузить());
	
	Результат = Запрос.Выполнить();
	Если Не Результат.Пустой() Тогда
		Участники.Загрузить(Результат.Выгрузить());
	КонецЕсли;
...
////////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ЭЛЕМЕНТОВ ТАБЛИЦЫ ФОРМЫ ДЕРЕВО СВЯЗЕЙ
...
Процедура ОткрытьВыполнить()
	...
	ОткрытьЗначение(Элементы.ДеревоСвязей.ТекущиеДанные.Ссылка);
...
Процедура СоздатьУзелПартнера(Родитель, Партнер, ПартнерыРодителя, мКонтакты, УровеньИерархии, Роль = Неопределено)
	...
	УзелПартнера.Представление = ?(
		Не ЗначениеЗаполнено(Роль) , Партнер.Наименование,"(" + Роль + ") " + Партнер.Наименование
