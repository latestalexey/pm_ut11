...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		ДействияСПользователемСервиса = СтандартныеПодсистемыПереопределяемый.ПолучитьДействияСПользователемСервиса(
			Объект.Ссылка);
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ)
	...
	Если СтандартныеПодсистемыКлиентПовтИсп.ПараметрыРаботыКлиента().РазделениеВключено
		...
		СтандартныеПодсистемыКлиентПереопределяемый.ЗапроситьПарольДляАутентификацииВСервисе(
...
Процедура ФизическоеЛицоПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.ФизическоеЛицо) Тогда
		...
		ИначеЕсли Объект.Наименование <> Строка(Объект.ФизическоеЛицо) Тогда
			ТекстВопроса = НСтр("ru='Полное имя пользователя отличается от ФИО физического лица. Заменить имя пользователя?'");
...
&НаКлиенте
Процедура ИзменитьEMail(Команда)
	...
	Если Форма.ДействияСПользователемСервиса <> Неопределено Тогда
		...
		Для каждого СтрокаКИ Из Форма.КонтактнаяИнформацияОписаниеДополнительныхРеквизитов Цикл
			...
			Если СтрокаКИ.Вид = ПредопределенноеЗначение("Справочник.ВидыКонтактнойИнформации.EmailПользователя") Тогда
				...
				Элементы[СтрокаКИ.ИмяРеквизита].ТолькоПросмотр = НЕ Форма.Объект.Ссылка.Пустая();
				
				Элементы.ИзменитьEMail.Доступность = НЕ Форма.Объект.Ссылка.Пустая()
					И ДействияВидаКИ.Изменение;
					
				ЗаполненEMail = ЗначениеЗаполнено(Форма[СтрокаКИ.ИмяРеквизита]);
			Иначе
				Элементы[СтрокаКИ.ИмяРеквизита].ТолькоПросмотр = Элементы[СтрокаКИ.ИмяРеквизита].ТолькоПросмотр
...
Процедура АдаптироватьРедактированиеКонтактнойИнформацииВСервисе()
	...
	ТаблицаКИ = ЭтаФорма.КонтактнаяИнформацияОписаниеДополнительныхРеквизитов;
	СтрокаEMail = ТаблицаКИ.НайтиСтроки(Новый Структура("Вид", Справочники.ВидыКонтактнойИнформации.EmailПользователя))[0];
	...
	ГруппаEMail = Элементы.Вставить("ГруппаEMail", Тип("ГруппаФормы"), Элементы.КонтактнаяИнформация, ЭлементEMail);
	...
	ГруппаEMail.Заголовок = НСтр("ru = 'Группа e-mail'");
	
	Элементы.Переместить(ЭлементEMail, ГруппаEMail);
	
	ГруппаИзменитьEMail = Элементы.Добавить("ГруппаИзменитьEMail", Тип("ГруппаФормы"), ГруппаEMail);
	ГруппаИзменитьEMail.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	ГруппаИзменитьEMail.ОтображатьЗаголовок = Ложь;
	ГруппаИзменитьEMail.Отображение = ОтображениеОбычнойГруппы.Нет;
	ГруппаИзменитьEMail.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	ГруппаИзменитьEMail.Заголовок = НСтр("ru = 'Группа изменить e-mail'");
	
	ДекорацияИзменитьEMail = Элементы.Добавить("ДекорацияИзменитьEMail", Тип("ДекорацияФормы"), ГруппаИзменитьEMail);
	ДекорацияИзменитьEMail.Заголовок = " ";
	
	КнопкаИзменитьEMail = Элементы.Добавить("ИзменитьEMail", Тип("КнопкаФормы"), ГруппаИзменитьEMail);
	КнопкаИзменитьEMail.Вид = ВидКнопкиФормы.ОбычнаяКнопка;
	КнопкаИзменитьEMail.ИмяКоманды = "ИзменитьEMail";
	
	СтрокаТелефон = ТаблицаКИ.НайтиСтроки(Новый Структура("Вид", Справочники.ВидыКонтактнойИнформации.ТелефонПользователя))[0];
