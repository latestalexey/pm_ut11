...
Функция ОпределитьКоординатыПоля(ДанныеДорожки, Префикс, Суффикс, Разделитель, НКол, ККол)
	...
	НайденРезделитель = Найти(Сред(ДанныеСтрока, НКол, ККол-НКол), Разделитель);
	Если НЕ ПустаяСтрока(Разделитель) И НайденРезделитель > 0 Тогда
...
Процедура ПроверитьШаблон(Команда)
	...
	ОткрытьФормуМодально("Справочник.ШаблоныМагнитныхКарт.Форма.ФормаПроверкиШаблона"
		, Новый Структура("ДанныеШаблона", ДанныеШаблона), ЭтаФорма);      
