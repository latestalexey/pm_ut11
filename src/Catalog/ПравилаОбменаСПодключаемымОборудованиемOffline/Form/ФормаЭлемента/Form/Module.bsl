...
&НаКлиенте
Процедура ТоварыКодНачалоВыбораИзСписка(Элемент, СтандартнаяОбработка)
...
Процедура ТоварыПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если Не ОтменаРедактирования Тогда
		...
		Если НайденнаяСтрока <> Неопределено Тогда
			...
			Если Не ЗначениеЗаполнено(НайденнаяСтрока.Номенклатура) ИЛИ Вопрос(ОписаниеОшибки, РежимДиалогаВопрос.ДаНет) = КодВозвратаДиалога.Да Тогда
...
Процедура ТоварыПередУдалением(Элемент, Отказ)
	...
	Для Каждого ВыделеннаяСтрока Из Элементы.Товары.ВыделенныеСтроки Цикл
		...
		Если ЗначениеЗаполнено(ТекущиеДанные.Номенклатура) И ТекущиеДанные.Используется Тогда
			...
			Результат = Вопрос(НСтр("ru = 'Удаляемой номенклатуре, соответствующей заданному отбору после удаления будет заново назначены коды товаров. Продолжить?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура УдалитьСвободныеКодыТоваровВКонцеСписка(Команда)
	...
	Если Товары.Количество() > 0 Тогда
		...
		Иначе
			Предупреждение(НСтр("ru = 'Нет данных для удаления'"));
		КонецЕсли;
	Иначе
		Предупреждение(НСтр("ru = 'Нет данных для удаления'"));
	КонецЕсли;
...
Функция ТаблицаТоваров(Товары, ВидЦены)
	...
	|	(ЕСТЬNULL(КодыТоваров.Упаковка.Коэффициент, 1)
	|		/ ЕСТЬNULL(ЦеныНоменклатурыСрезПоследних.Упаковка.Коэффициент, 1)) *
	|	ЕСТЬNULL(ЦеныНоменклатурыСрезПоследних.Цена, 0) КАК Цена,
	|	ВЫБОР
	|		КОГДА КодыТоваров.Упаковка = ЗНАЧЕНИЕ(Справочник.УпаковкиНоменклатуры.ПустаяСсылка)
	|			ТОГДА ВЫБОР КОГДА КодыТоваров.Номенклатура.ЕдиницаИзмерения.ТипЕдиницыИзмерения = ЗНАЧЕНИЕ(Перечисление.ТипыЕдиницИзмерения.Весовая) ТОГДА Истина Иначе Ложь КОНЕЦ
	|		ИНАЧЕ ВЫБОР КОГДА КодыТоваров.Упаковка.ЕдиницаИзмерения.ТипЕдиницыИзмерения = ЗНАЧЕНИЕ(Перечисление.ТипыЕдиницИзмерения.Весовая) ТОГДА Истина Иначе Ложь КОНЕЦ
	|	КОНЕЦ КАК Весовой
	|ИЗ
