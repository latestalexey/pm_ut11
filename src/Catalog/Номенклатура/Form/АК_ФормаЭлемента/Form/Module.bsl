...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//		Объект.акИспользоватьГрузовыеМеста=ЛОЖЬ;
...
Процедура ГрузовыеМестаПослеУдаления(Элемент)
	ПроверитьГрузовыеМеста();
КонецПроцедуры
 
&НаСервере
Процедура ПроверитьГрузовыеМеста()
	Объект.акКоличествоГрузовыхМест  = Объект.акГрузовыеМеста.Итог("Количество");
КонецПроцедуры

&НаКлиенте
Процедура акГрузовыеМестаГрузовоеМестоОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)	
	
	ТекущийЭлемент.ТекущиеДанные.Количество = ПолучитьГрузовоеМесто(ВыбранноеЗначение);
	
КонецПроцедуры

&НаСервере
Функция ПолучитьГрузовоеМесто(ГрузовоеМесто)
	
	ГрузовоеМестоОбъект =  ГрузовоеМесто.получитьОбъект();
	Возврат ГрузовоеМестоОбъект.количество;
	
КонецФункции

&НаКлиенте
Процедура акГрузовыеМестаПослеУдаления(Элемент)
	ПроверитьГрузовыеМеста();
КонецПроцедуры

&НаКлиенте
Процедура акГрузовыеМестаПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	ПроверитьГрузовыеМеста();
КонецПроцедуры

//{ ООО "АСТЭК" Разработчик: Бурыкин Александр  26.11.2013
&НаКлиенте
Процедура акИспользоватьГрузовыеМестаПриИзменении(Элемент)
	
	Если объект.акИспользоватьГрузовыеМеста = Истина Тогда
		Элементы.ГруппаГрузовыеМеста.Видимость = Истина;		
		Элементы.акКоличествоГрузовыхМест.Видимость = Истина;
	Иначе 
        Элементы.ГруппаГрузовыеМеста.Видимость = Ложь;		
		Объект.акКоличествоГрузовыхМест=1; 
		Объект.акГрузовыеМеста.Очистить();
		Элементы.акКоличествоГрузовыхМест.Видимость = Ложь;
	КонецЕсли;
