...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	АвтоЗаголовок = Ложь;
	Заголовок = НСтр("ru = 'Анализ движений'");
	
	ОтборНоменклатура = Параметры.Ключ;
	
	// Если по товару не ведется учет по сериям, то скрыть закладку "Движения серий"
	// и не формировать отчет по сериям.
	...
	ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СтраницаДвиженияСерийТоваров", "Видимость", ИспользоватьСерии);
	
	СформироватьОтчет("ВедомостьПоТоварамНаСкладах", "ОстаткиИОборотыСГруппировкойПоСкладам");
	
	Если ИспользоватьСерии Тогда
		
		СформироватьОтчет("ДвиженияСерийТоваров", "ДвиженияСерийНоменклатурыКонтекст");
...
&НаКлиенте
Процедура СформироватьОтчетВедомостьПоТоварамНаСкладах(Команда)
	
	ОбновитьОтчет("ВедомостьПоТоварамНаСкладах");
	
КонецПроцедуры

&НаКлиенте
Процедура ПеречитатьОтчетДвиженияТоваров(Команда)
...
&НаКлиенте
Процедура СформироватьОтчетДвижениеСерийТоваров(Команда)
	
	ОбновитьОтчет("ДвиженияСерийТоваров");
	
КонецПроцедуры

&НаКлиенте
Процедура ПеречитатьОтчетДвиженияСерийТоваров(Команда)
...
Процедура ОбновитьОтчет(ИмяОтчета)
	...
	Если ИмяОтчета = "ВедомостьПоТоварамНаСкладах" Тогда
		АдресСхемы = АдресСхемыДвиженияТовара;
		АдресДанныхРасшифровки = АдресДанныхРасшифровкиДвиженияТовара;
	Иначе
		АдресСхемы = АдресСхемыДвиженияСерий;
		АдресДанныхРасшифровки = АдресДанныхРасшифровкиДвиженияСерий;
	КонецЕсли; 
...
Процедура СформироватьОтчет(ИмяОтчета, ИмяВариантаНастроек = "ПоНоменклатуреКонтекст")
	...
	Если ИмяОтчета = "ВедомостьПоТоварамНаСкладах" Тогда
		АдресСхемыДвиженияТовара = АдресСхемы;
		АдресДанныхРасшифровкиДвиженияТовара = АдресДанныхРасшифровки;
	Иначе
		АдресСхемыДвиженияСерий = АдресСхемы;
		АдресДанныхРасшифровкиДвиженияСерий = АдресДанныхРасшифровки;
	КонецЕсли; 
	
КонецПроцедуры

&НаКлиенте
Процедура ТаблицаОтчетаВедомостьПоТоварамНаСкладахОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если НЕ (ПараметрВыполненногоДействия = Неопределено) Тогда
		...
		Иначе
			...
			ОбработатьРасшифровкуДвиженияТоваров("ВедомостьПоТоварамНаСкладах", ОписаниеОбработкиРасшифровки);
...
&НаКлиенте
Процедура ТаблицаОтчетаДвиженияСерийТоваровОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если НЕ (ПараметрВыполненногоДействия = Неопределено) Тогда
		...
		Иначе
			...
			ОбработатьРасшифровкуДвиженияТоваров("ДвиженияСерийТоваров", ОписаниеОбработкиРасшифровки);
...
&НаСервере
Процедура ОбработатьРасшифровкуДвиженияТоваров(ИмяОтчета, ОписаниеОбработкиРасшифровки)
