...
Процедура ДобавитьДополнительныйОтчет(Команда)
	СтандартныеПодсистемыКлиентПереопределяемый.РассылкаОтчетовПодборДопОтчета(Элементы.Отчеты);
КонецПроцедуры
...
Процедура ПрочитатьНастройкиОтчетовОбъектаКопирования()
	...
	Для ОбратныйИндекс = 1 По КоличествоСтрок Цикл
		...
		//ПараметрыОтчета = ИнициализироватьОтчет(СтрокаОтчеты, Истина, НастройкиОтчета);
		//
		...
		//Иначе
		//	Сообщение = Новый СообщениеПользователю;
		//	Сообщение.Текст = ПараметрыОтчета.Ошибки;
		//	Сообщение.Сообщить();
		//КонецЕсли;
	КонецЦикла;
КонецПроцедуры
