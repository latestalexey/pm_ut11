...
Процедура ЗаполнитьЗаголовкиГиперссылокСтраницыСертификата()
	...
	Иначе
		ТекстГиперссылкиПользователя = "Доступен пользователю: "
									+ ?(ЗначениеЗаполнено(Пользователь), Пользователь, "<Выбрать пользователя>");
	КонецЕсли;
...
Процедура ЗаполнитьРеквизитыСертификатаНаСервере(СертификатСсылка)
	...
	РеквизитыСертификата = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СертификатСсылка,
		...
		|ПарольПользователя, ВидыДокументов, Наименование, ФайлСертификата, Пользователь");
...
Процедура СохранитьПараметры(ДанныеСохранены)
	...
	Попытка
		...
		Если ВариантВыбораСертификата = "Добавить новый" Тогда
			НовоеСоглашение.СертификатАбонента   = НовыйСертификатСсылка;
		Иначе
			НовоеСоглашение.СертификатАбонента   = СертификатКриптографии;
		КонецЕсли;
...
Процедура ТестСвязиСТакском(ТестПройден)
	...
	Если ВариантВыбораСертификата = "Добавить новый" Тогда
		СтруктураПараметровЗапросаМаркера = Новый Структура("НаименованиеСоглашения, ЗапомнитьПарольКСертификату, ПарольПользователя, ФайлСертификата, СертификатАбонента",
															НаименованиеСоглашения,
															Ложь,
															ПарольПользователя,
	...
	Иначе
		...
		СтруктураПараметровЗапросаМаркера.Вставить("НаименованиеСоглашения", НаименованиеСоглашения);
	КонецЕсли;
	
	Если ЭлектронныеДокументыСлужебныйВызовСервера.ВыполнятьАвторизациюНаСервере() Тогда
		ТестПройден = ЭлектронныеДокументыСлужебныйВызовСервера.ТестСвязиСОператоромЭДО(СтруктураПараметровЗапросаМаркера,
																						ПарольПользователя,
																						Ложь);
	Иначе
		ТестПройден = ЭлектронныеДокументыСлужебныйКлиент.ТестСвязиСОператоромЭДО(
...
Функция СтруктураПараметровЗапросаМаркера(Сертификат)
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	СертификатАбонента.ЗапомнитьПарольКСертификату КАК ЗапомнитьПарольКСертификату,
	|	СертификатАбонента.ПарольПользователя КАК ПарольПользователя,
	|	СертификатАбонента.ФайлСертификата КАК ФайлСертификата,
	|	СертификатАбонента.Ссылка КАК СертификатАбонента
	|ИЗ
	|	Справочник.СертификатыЭЦП КАК СертификатАбонента
	|ГДЕ
	|	СертификатАбонента.Ссылка = &Ссылка";
...
Процедура СохранитьНастройкиКриптосредстваИСертификата(ДанныеСохранены)
	...
	Попытка
		...
		Если ВариантВыбораСертификата = "Добавить новый" Тогда
			...
			НовыйСертификат.ФайлСертификата = Новый ХранилищеЗначения(ПолучитьИзВременногоХранилища(СсылкаНаСертификат),
