...
&НаКлиенте
Процедура ПроверитьДоступностьКаталога(ПутьККаталогу, Отказ = Ложь)
	...
	Если ЗначениеЗаполнено(ПутьККаталогу) Тогда
		...
		Если НЕ ПрочитатьТестовыйФайлНаСервере(ПолноеИмяТестовогоФайла) Тогда
...
Процедура КаталогОбмена(ПутьККаталогу)
	...
	Если ДиалогКаталога.Выбрать() Тогда
		...
		ПроверитьДоступностьКаталога(ПутьККаталогу);
	КонецЕсли;
...
Процедура ИзменитьНадписьНаправлениеДокументов()
	...
	Если Объект.ЭтоТиповое Тогда
		Элементы.НадписьНаправлениеДокументовВх.Заголовок  = НСтр("ru='Кому: '") + Объект.Организация;
		Элементы.НадписьНаправлениеДокументовИсх.Заголовок = НСтр("ru='От кого: '") + Объект.Организация;
	Иначе
		Элементы.НадписьНаправлениеДокументовВх.Заголовок  = НСтр("ru='От кого: '")
			+ Объект.Контрагент + НСтр("ru=', кому: '") + Объект.Организация;
		Элементы.НадписьНаправлениеДокументовИсх.Заголовок = НСтр("ru='От кого: '")
			+ Объект.Организация + НСтр("ru=', кому: '") + Объект.Контрагент;
	КонецЕсли
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если НЕ Объект.ЭтоТиповое Тогда
		...
		Если Объект.СтатусСоглашения = ПредопределенноеЗначение("Перечисление.СтатусыСоглашенийЭД.Действует") Тогда
			...
			КонецЕсли;
			ПроверитьДоступностьКаталога(КаталогВходящихДокументов, Отказ);
			ПроверитьДоступностьКаталога(КаталогИсходящихДокументов, Отказ);
		КонецЕсли;
