...
Процедура ЗарегистрироватьИзменения(Команда)
	...
	Если МассивКодов.Количество() > 0 Тогда
		...
		Иначе
			Предупреждение(НСтр("ru = 'В процессе регистрации изменений произошла ошибка:'" + " " + ОписаниеОшибки));
		КонецЕсли;
	Иначе
		Предупреждение(НСтр("ru = 'Не выбраны строки для регистрации изменений'"));
	КонецЕсли;
...
Процедура ТоварыОчистить(Команда)
	...
	Если ТипОборудования = ПредопределенноеЗначение("Перечисление.ТипыПодключаемогоОборудования.ККМOffline") Тогда
		Выполнено = ПодключаемоеОборудованиеOfflineКлиент.ОчиститьТоварыВККМOffline(Устройства);
	ИначеЕсли ТипОборудования = ПредопределенноеЗначение("Перечисление.ТипыПодключаемогоОборудования.ВесыСПечатьюЭтикеток") Тогда
		Выполнено = ПодключаемоеОборудованиеOfflineКлиент.ОчиститьТоварыВВесах(Устройства);
	КонецЕсли;
...
Процедура ТоварыПерезагрузить(Команда)
	...
	Выполнено = 0;
	Если ТипОборудования = ПредопределенноеЗначение("Перечисление.ТипыПодключаемогоОборудования.ККМOffline") Тогда
		Выполнено = ПодключаемоеОборудованиеOfflineКлиент.ВыгрузитьПолныйСписокТоваровВККМOffline(Устройства);
	ИначеЕсли ТипОборудования = ПредопределенноеЗначение("Перечисление.ТипыПодключаемогоОборудования.ВесыСПечатьюЭтикеток") Тогда
		Выполнено = ПодключаемоеОборудованиеOfflineКлиент.ВыгрузитьПолныйСписокТоваровВВесы(Устройства);
	КонецЕсли;
...
Процедура ТоварыВыгрузить(Команда)
	...
	Выполнено = 0;
	Если ТипОборудования = ПредопределенноеЗначение("Перечисление.ТипыПодключаемогоОборудования.ККМOffline") Тогда
		Выполнено = ПодключаемоеОборудованиеOfflineКлиент.ВыгрузитьТоварыВККМOffline(Устройства);
	ИначеЕсли ТипОборудования = ПредопределенноеЗначение("Перечисление.ТипыПодключаемогоОборудования.ВесыСПечатьюЭтикеток") Тогда
		Выполнено = ПодключаемоеОборудованиеOfflineКлиент.ВыгрузитьТоварыВВесы(Устройства);
	КонецЕсли;
...
Процедура УдалитьРегистрациюИзмененийДляВыделенныхСтрок(Команда)
	...
	Если МассивКодов.Количество() > 0 Тогда
		...
		Если Результат Тогда
			...
			Оповестить("Запись_КодыТоваровПодключаемогоОборудования", Новый Структура, Неопределено);
		Иначе
			Предупреждение(НСтр("ru = 'В процессе удаления регистрации изменений произошла ошибка:'" + " " + ОписаниеОшибки));
		КонецЕсли;
	Иначе
		Предупреждение(НСтр("ru = 'Не выбраны строки для удаления регистрации изменений'"));
	КонецЕсли;
...
Функция ПолучитьДанныеДляПечатиЦенников()
	...
	ТаблицаТовары.Колонки.Добавить("Упаковка",       Новый ОписаниеТипов("СправочникСсылка.УпаковкиНоменклатуры"));
