...
Функция КомплектПечатныхФорм(ТипОбъекта, Знач МассивСсылок = Неопределено, ВариантИспользования = 0) Экспорт
	...
	КомплектПоУмолчанию.Колонки.Добавить("Ссылка");
	КоллекцияПечатныхФорм = ПодготовитьКомплектПечатныхФорм();
	КоллекцияПечатныхФорм.Колонки.Добавить("Ссылка");
	...
	ТаблицаКоманд = ДополнительныеОтчетыИОбработки.ПолучитьДоступныеКоманды(Истина, ТипОбъекта, Истина, Неопределено,
	...
	Для Каждого ОбъектСсылка из МассивСсылок Цикл
		...
		КонецЕсли;
		НастройкиПечати.ЗаполнитьЗначения(Выборка.Ссылка,"Ссылка");
...
// КомплектПечатныхФорм - ТаблицаЗначений возвращаемая функций ПодготовитьКомплектПечатныхФорм()
// Имя - Строка - внутреннее имя печатной формы
// Представление - Число - количество экземпляров, выводимых на печать
// Экземпляров - Число - количество экземпляров, выводимых на печать
//
// Возвращаемое значение:
...
Процедура ДополнитьКомплектВнешнимиПечатнымиФормами(ТипОбъекта, ДополняемыйКомплект) Экспорт
	...
	ПрефиксВнешнихПечатныхФорм = "ВнешняяПечатнаяФорма.";
	КоллекцияВнешнихПечатныхФорм = ПодготовитьКомплектПечатныхФорм();
	ТаблицаКоманд = ДополнительныеОтчетыИОбработки.ПолучитьДоступныеКоманды(
		Истина, 
		ТипОбъекта, 
		Истина, 
		Неопределено,
...
Процедура СформироватьКомплектВнешнихПечатныхФорм(ИмяМенеджераПечати, МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати) Экспорт
	...
	Для каждого ПечатнаяФорма Из КомплектПечатныхФорм Цикл
		...
		Если ТекущийКомплект.Количество() > 0 И КоллекцияПечатныхФорм.Найти(ТекущийКомплект[0].Имя, "ИмяМакета") = Неопределено Тогда
			Если ТекущийКомплект.Колонки.Найти("Ссылка") <> Неопределено И ЗначениеЗаполнено(ТекущийКомплект[0].Ссылка) Тогда
				...
				ТекущаяСтруктураТипов.Вставить(ИмяМенеджераПечати, ТаблицаСсылок.ВыгрузитьКолонку("Ссылка"));
				...
				Для каждого ПечатнаяФорма Из ТекущийКомплект Цикл
					...
					Если ПечатнаяФорма.Экземпляров > 1 Тогда
						ОбластьКопирования = ТабличныйДокумент.ПолучитьОбласть(1,,ТабличныйДокумент.ВысотаТаблицы);
						Пока ПечатнаяФорма.Экземпляров > 1 Цикл
...
Функция ВнешняяПечатнаяФорма(Идентификатор, ПолноеИмяОбъектаМетаданных)
	...
	СтандартныеПодсистемыПереопределяемый.ПриПолученииВнешнейПечатнойФормы(Идентификатор, ПолноеИмяОбъектаМетаданных, ВнешняяПечатнаяФормаСсылка);
