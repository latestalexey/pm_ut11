...
Функция ТекстЗапросаОПрибыли(СоответствиеЗапросыДанные,НомерПоследнегоПакета,СтруктураПериодовФормированияОтчета)
	...
	Если СтруктураПериодовФормированияОтчета.ЕстьДополнительныйПериод Тогда
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ЕСТЬNULL(ВложенныйЗапрос.Менеджер, ""Не указан"") КАК Менеджер,
...
Функция ТекстЗапросаОПоступленииДС(СоответствиеЗапросыДанные,НомерПоследнегоПакета, СтруктураПериодовФормированияОтчета)
	...
	Если СтруктураПериодовФормированияОтчета.ЕстьДополнительныйПериод Тогда
		...
		СоответствиеЗапросыДанные.Вставить("ПоступлениеДС",НомерПоследнегоПакета + 3);
		НомерПоследнегоПакета = НомерПоследнегоПакета + 3;
		
		Возврат "ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	РасчетыСКлиентамиОбороты.ЗаказКлиента.Менеджер КАК Менеджер,
		|	СУММА(ВЫБОР
		|			КОГДА РасчетыСКлиентамиОбороты.Регистратор.Валюта = Константы.ВалютаУправленческогоУчета
		|				ТОГДА РасчетыСКлиентамиОбороты.СуммаРасход
		|			ИНАЧЕ ВЫБОР
		|					КОГДА ЕСТЬNULL(КурсВалютыУправленческогоУчета.СредневзвешенныйКурс, 0) = 0
		|						ТОГДА 0
		|					ИНАЧЕ РасчетыСКлиентамиОбороты.СуммаРасход * ЕСТЬNULL(КурсыПрочиеВалюты.СредневзвешенныйКурс, 0) / ЕСТЬNULL(КурсВалютыУправленческогоУчета.СредневзвешенныйКурс, 0)
		|				КОНЕЦ
		|		КОНЕЦ) КАК Показатель
		|ПОМЕСТИТЬ ТекущийПериодПоступлениеДС
		|ИЗ
		|	РегистрНакопления.РасчетыСКлиентами.Обороты(&НачалоПериода, &ОкончаниеПериода, Регистратор, АналитикаУчетаПоПартнерам.Партнер <> ЗНАЧЕНИЕ(Справочник.Партнеры.РозничныйПокупатель) И ЗаказКлиента.Менеджер В (ВЫБРАТЬ ОтборПоМенеджерам.Ссылка ИЗ ОтборПоМенеджерам КАК ОтборПоМенеджерам)) КАК РасчетыСКлиентамиОбороты
		|		ЛЕВОЕ СОЕДИНЕНИЕ СреднеВзвешенныеКурсыВалютЗаТекущийПериод КАК КурсыПрочиеВалюты
		|		ПО РасчетыСКлиентамиОбороты.Регистратор.Валюта = КурсыПрочиеВалюты.Валюта,
		|	Константы КАК Константы
		...
		|ГДЕ
		|	(РасчетыСКлиентамиОбороты.Регистратор ССЫЛКА Документ.РасходныйКассовыйОрдер
		|			ИЛИ РасчетыСКлиентамиОбороты.Регистратор ССЫЛКА Документ.ВзаимозачетЗадолженности
		|			ИЛИ РасчетыСКлиентамиОбороты.Регистратор ССЫЛКА Документ.ОперацияПоПлатежнойКарте
		|			ИЛИ РасчетыСКлиентамиОбороты.Регистратор ССЫЛКА Документ.ПоступлениеБезналичныхДенежныхСредств
		|			ИЛИ РасчетыСКлиентамиОбороты.Регистратор ССЫЛКА Документ.ПриходныйКассовыйОрдер
		|			ИЛИ РасчетыСКлиентамиОбороты.Регистратор ССЫЛКА Документ.РасходныйКассовыйОрдер
		|			ИЛИ РасчетыСКлиентамиОбороты.Регистратор ССЫЛКА Документ.СписаниеБезналичныхДенежныхСредств
		|			ИЛИ РасчетыСКлиентамиОбороты.Регистратор ССЫЛКА Документ.СписаниеЗадолженности)
		|	И КурсВалютыУправленческогоУчета.Валюта = Константы.ВалютаУправленческогоУчета
		|
		|СГРУППИРОВАТЬ ПО
		...
		|ИЗ
		|	РегистрНакопления.РасчетыСКлиентами.Обороты(&ДатаНачалаДополнительныйПериод,&ДатаОкончанияДополнительныйПериод, Регистратор, АналитикаУчетаПоПартнерам.Партнер <> ЗНАЧЕНИЕ(Справочник.Партнеры.РозничныйПокупатель) И ЗаказКлиента.Менеджер В (ВЫБРАТЬ ОтборПоМенеджерам.Ссылка ИЗ ОтборПоМенеджерам КАК ОтборПоМенеджерам)) КАК РасчетыСКлиентамиОбороты
	...
	Иначе
		...
		СоответствиеЗапросыДанные.Вставить("ПоступлениеДС",НомерПоследнегоПакета + 1);
		НомерПоследнегоПакета = НомерПоследнегоПакета + 1;
...
Функция ТекстЗапросаОПоступленииДС_МинусСебестоимость(СоответствиеЗапросыДанные,НомерПоследнегоПакета, СтруктураПериодовФормированияОтчета)
	...
	Если СтруктураПериодовФормированияОтчета.ЕстьДополнительныйПериод Тогда
		...
		СоответствиеЗапросыДанные.Вставить("ПоступлениеДС_МинусСебестоимость",НомерПоследнегоПакета + 3);
		НомерПоследнегоПакета = НомерПоследнегоПакета + 3;
		...
		|
		|СГРУППИРОВАТЬ ПО
		|	ЕСТЬNULL(ВложенныйЗапрос.Менеджер, ""Не указан"")
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	Иначе
		...
		СоответствиеЗапросыДанные.Вставить("ПоступлениеДС_МинусСебестоимость",НомерПоследнегоПакета + 1);
		НомерПоследнегоПакета = НомерПоследнегоПакета + 1;
		...
		|
		|УПОРЯДОЧИТЬ ПО
		|	ПоступлениеДС_МинусСебестоимость
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|";
...
Функция ТекстЗапросаОПотеряхВРезультатеПросроченнойДебиторки(СоответствиеЗапросыДанные,НомерПоследнегоПакета, СтруктураПериодовФормированияОтчета)
	...
	Если СтруктураПериодовФормированияОтчета.ЕстьДополнительныйПериод Тогда
		...
		Возврат "ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ЕСТЬNULL(ВложенныйЗапрос.ЗаказКлиентаМенеджер, ""Не указан"") КАК Менеджер,
		|	СУММА(ВложенныйЗапрос.Показатель) КАК Показатель
		...
		|				КОНЕЦ
		|		КОНЕЦ * РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(РасчетыСКлиентамиПоДокументамОстатки.ДатаПлатежа, ДЕНЬ), &ОкончаниеПериода, ДЕНЬ) * &ПроцентнаяСтавкаСтоимостиДенег / 36500
		|	ИЗ
		|		РегистрНакопления.РасчетыСКлиентамиПоДокументам.Остатки(&ОкончаниеПериода,ЗаказКлиента.Менеджер В (ВЫБРАТЬ ОтборПоМенеджерам.Ссылка ИЗ ОтборПоМенеджерам КАК ОтборПоМенеджерам) И ДатаПлатежа < &ОкончаниеПериода) КАК РасчетыСКлиентамиПоДокументамОстатки
		|			ЛЕВОЕ СОЕДИНЕНИЕ СреднеВзвешенныеКурсыВалютЗаТекущийПериод КАК КурсыПрочиеВалюты
		...
		|
		|СГРУППИРОВАТЬ ПО
		|	ЕСТЬNULL(ВложенныйЗапрос.ЗаказКлиентаМенеджер, ""Не указан"")
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|
		...
		|					ИНАЧЕ РасчетыСКлиентамиПоДокументамОстатки.ДолгОстаток * ЕСТЬNULL(КурсыПрочиеВалюты.СредневзвешенныйКурс, 0) / ЕСТЬNULL(КурсВалютыУправленческогоУчета.СредневзвешенныйКурс, 0)
		|				КОНЕЦ
		|		КОНЕЦ * РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(РасчетыСКлиентамиПоДокументамОстатки.ДатаПлатежа, ДЕНЬ), &ДатаОкончанияДополнительныйПериод, ДЕНЬ) * &ПроцентнаяСтавкаСтоимостиДенег / 36500
		|	ИЗ
		|		РегистрНакопления.РасчетыСКлиентамиПоДокументам.Остатки(&ДатаОкончанияДополнительныйПериод,ЗаказКлиента.Менеджер В (ВЫБРАТЬ ОтборПоМенеджерам.Ссылка ИЗ ОтборПоМенеджерам КАК ОтборПоМенеджерам)И ДатаПлатежа < &ДатаОкончанияДополнительныйПериод) КАК РасчетыСКлиентамиПоДокументамОстатки
		|			ЛЕВОЕ СОЕДИНЕНИЕ СреднеВзвешенныеКурсыВалютЗаДополнительныйПериод КАК КурсыПрочиеВалюты
		|			ПО РасчетыСКлиентамиПоДокументамОстатки.Валюта = КурсыПрочиеВалюты.Валюта,
		|		Константы КАК Константы
		|			ЛЕВОЕ СОЕДИНЕНИЕ СреднеВзвешенныеКурсыВалютЗаДополнительныйПериод КАК КурсВалютыУправленческогоУчета
...
Функция ТекстЗапросаОПродажахНаОднуСделку(СоответствиеЗапросыДанные,НомерПоследнегоПакета, СтруктураПериодовФормированияОтчета)
	...
	Если СтруктураПериодовФормированияОтчета.ЕстьДополнительныйПериод Тогда
		...
		|		ВЫБРАТЬ
		|			0,
		|			ЕСТЬNULL(ВыручкаИСебестоимостьПродажОбороты.ЗаказКлиента.Менеджер, ""Не указан""),
