...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	ЭтоРасшифровка = ВыбранныйВариант <> "Анализ исполнения ассортимента";
...
Функция СформироватьОтчет()
	...
	ОбластьЗаголовок.Параметры.ЗаголовокОтчета = НСтр("ru='Анализ исполнения ассортимента" + ?(ЭтоРасшифровка, " (детально по номенклатуре)", "") +"'");
	...
	Если ЭлементыОтбора.Количество() > 0 Тогда
		...
		Для Каждого ЭлементОтбора ИЗ ЭлементыОтбора Цикл
			...
			Если ЛевоеЗначение = "Ассортимент исполняется" Тогда
...
Функция ТекстНачальногоЗапроса()
	...
	|	ВЫБОР КОГДА Квоты.Марка = ЗНАЧЕНИЕ(Справочник.Марки.ПустаяСсылка) ТОГДА ""Прочие марки"" ИНАЧЕ Квоты.Марка КОНЕЦ КАК Марка,
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Склады КАК Магазины
	|		ПО (Магазины.ФорматМагазина = втТекущиеКвотыБезМагазинов.ФорматМагазина)
	|		И (Магазины.ТипСклада = ЗНАЧЕНИЕ(Перечисление.ТипыСкладов.РозничныйМагазин))
	...
	|	СправочникНоменклатура.ТоварнаяКатегория.Владелец КАК ВидНоменклатуры,
	...
	|	Ассортимент.ДатаРегистратора КАК ДатаРегистратора,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТоварыНаСкладахОстатки.Склад.ФорматМагазина КАК ФорматМагазина,
	...
	|		ВЫБОР КОГДА ТекущиеКвоты.Марка.Наименование ЕСТЬ NULL ТОГДА ""Прочие марки"" ИНАЧЕ ТекущиеКвоты.Марка КОНЕЦ КАК Марка,
	|		ВЫБОР
	...
	|		ВЫБОР КОГДА ТекущиеКвоты.Марка.Наименование ЕСТЬ NULL ТОГДА ""Прочие марки"" ИНАЧЕ ТекущиеКвоты.Марка КОНЕЦ,
