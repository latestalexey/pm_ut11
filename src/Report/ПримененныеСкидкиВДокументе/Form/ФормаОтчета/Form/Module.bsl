...
Процедура Сформировать()
	...
	Для каждого СтрокаДерева Из МассивСкидок Цикл
		...
		ОбластьШапкаСкидкаНаценка.Параметры.СкидкаНаценка = СтрокаДерева.СкидкаНаценка;
	...
	Для каждого СтрокаТовары Из ДокументОбъект[ИмяТЧ] Цикл
		...
		Для каждого СтрокаДерева Из МассивСкидок Цикл
			...
			// Сумма скидки
			НайденнаяСтрока = СтрокаДерева.ТаблицаДанных.Найти(СтрокаТовары.КлючСвязи, "КлючСвязи");
			...
			Если ВсеУсловияВыполнены Тогда
				...
				ИначеЕсли СтрокаДерева.Управляемая И СтрокаДерева.НазначенаПользователем Тогда
					Если ДокументОбъект.СкидкиНаценки.НайтиСтроки(Новый Структура("КлючСвязи, СкидкаНаценка", СтрокаТовары.КлючСвязи, СтрокаДерева.СкидкаНаценка)).Количество() > 0 Тогда
	...
	Для каждого СтрокаДерева Из МассивСкидок Цикл
		...
		// Сумма скидки
		СуммаСкидки = 0;
		НайденныеСтроки = ДокументОбъект.СкидкиНаценки.НайтиСтроки(Новый Структура("СкидкаНаценка", СтрокаДерева.СкидкаНаценка));
