...
Процедура Настройки(Команда)
	...
	Результат = ОткрытьФормуМодально("Отчет.КалендарьСобытий.Форма.ФормаНастроек",ПараметрыДляФормыНастроек,ЭтаФорма);
...
Процедура ЗаполнитьДеревоСобытий()
	...
	ЕстьПравоДоверенностьПолучениеТоваров = ПравоДоступа("Чтение",Метаданные.Документы.ДоверенностьНаПолучениеТоваров);
	...
	Если ЕстьПравоУстановкаЦен Тогда
		ДобавленноеВДеревоВидСобытия(УзелДерева, "УстановкаЦен", НСтр("ru = 'Установка цен номенклатуры'"));
	КонецЕсли;
	...
	Если ЕстьПравоДоверенностьПолучениеТоваров Тогда
		ДобавленноеВДеревоВидСобытия(УзелДерева, "ДоверенностьПолучениеТоваров", НСтр("ru = 'Доверенность на получение товаров'"));
	КонецЕсли;
	УдалитьРодительскийУзелДереваЕслиПустой(УзелДерева);
	
	УзелДерева = ДобавленноеВДеревоВидСобытия(ВидыСобытий,"Финансы", НСтр("ru = 'Финансы'"));
	Если ЕстьПравоЗаявкаНаРасходованиеДС И ИспользоватьПланированиеДенежныхСредств Тогда
	...
	Если ЕстьПраваНаДоговора Тогда
		ДобавленноеВДеревоВидСобытия(УзелДерева, "ДоговораКонтрагентов", НСтр("ru = 'Договора контрагентов'"));
	КонецЕсли;
