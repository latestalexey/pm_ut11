...
Процедура УстановитьЗначенияПараметровЗапроса(Запрос,СтруктураФормированияОтчета)
	...
	Запрос.УстановитьПараметр("ОбеспечениеЗаказа",НСтр("ru = 'Обеспечить заказ'"));
	...
	Запрос.УстановитьПараметр("НеИспользоватьСтатусыЗаказовКлиента", НЕ ПолучитьФункциональнуюОпцию("ИспользоватьСтатусыЗаказовКлиентов"));
	...
	Запрос.УстановитьПараметр("ПодготовитьЗаявкуКВозврату",НСтр("ru = 'Подготовить заявку к возврату'"));
	Запрос.УстановитьПараметр("ОжиданиеОплатыДоОбеспеченияЗаявкаНаВозврат",НСтр("ru = 'Получить оплату до обеспечения'"));
	Запрос.УстановитьПараметр("ОбеспечениеПоЗаявкеНаВозврат",НСтр("ru = 'Обеспечить замену'"));
	...
	Запрос.УстановитьПараметр("ПеревестиВСтатусОтгружено",НСтр("ru = 'Перевести в статус отгружено'"));
	Запрос.УстановитьПараметр("ПолучитьОплатуПоРеализации",НСтр("ru = 'Получить оплату'"));
	Запрос.УстановитьПараметр("ПеревестиВСтатусКОтгрузке",НСтр("ru = 'Перевести в статус к отгрузке'"));
КонецПроцедуры
...
Функция ДатаОкончанияПериодаФормирования(ПериодФормирования)
	...
	ИначеЕсли ПериодФормирования = "На три месяца вперед" Тогда
		
		Возврат НачалоДня(ДобавитьМесяц(ТекущаяДата(),3) + СекундВДне);
		
	ИначеЕсли ПериодФормирования = "На полугодие вперед" Тогда
...
Функция ТекстЗапросаРезультирующий(СтруктураФормированияОтчета)
	...
	Иначе
		...
		Если СтруктураФормированияОтчета.ЕстьОбращенияКРегистрамРасчетыПоДокументамКлиенты Тогда
			...
			|			ИЛИ События.ТипСобытия = &ОбеспечениеЗаказа
			|			ИЛИ События.ТипСобытия = &ОжиданиеОплатыДоОбеспеченияЗаявкаНаВозврат
			|			ИЛИ События.ТипСобытия = &ОбеспечениеПоЗаявкеНаВозврат
			...
			|ГДЕ
			|	ПроведениеПоРасчетамСКлиентами.АналитикаУчетаПоПартнерам В
			|			(ВЫБРАТЬ
			...
			|	Границы.АналитикаУчетаПоПартнерам.Партнер КАК Партнер
			...
			|	Границы.АналитикаУчетаПоПартнерам.Партнер
			|
			|ИМЕЮЩИЕ
		...
		Если СтруктураФормированияОтчета.ЕстьОбращенияКРегистрамРасчетыПоДокументамПоставщики Тогда
			...
			|ПОМЕСТИТЬ НачальныеДвиженияПоставщики
			|ИЗ
			|	Последовательность.ПроведениеПоРасчетамСПоставщиками КАК ПроведениеПоРасчетамСПоставщиками
			|ГДЕ
			|	ПроведениеПоРасчетамСПоставщиками.АналитикаУчетаПоПартнерам В
			|			(ВЫБРАТЬ
			|				ОтборАналитикиПартнеров.КлючАналитики
			|			ИЗ
			...
			|	ИЗ
			|		НачальныеДвиженияПоставщики КАК НачальныеДвижения
...
Функция ТекстЗапросаПрочиеЗадачиИсполнителя(Запрос,СтруктураФормированияОтчета)
	...
	|		И НЕ ЗадачаИсполнителя.Предмет ССЫЛКА Документ.ДоверенностьНаПолучениеТоваров
...
Функция ТекстЗапросаВнутреннееПотребление(Запрос,СтруктураФормированияОтчета)
	...
	|	И ВнутреннееПотреблениеТоваров.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыВнутреннихПотребленийТоваров.КОтгрузке)
	|	И ВЫБОР
	...
	|	ВнутреннееПотреблениеТоваров.Ссылка КАК СсылкаНаОбъектСобытия,
	|	ВнутреннееПотреблениеТоваров.Ссылка КАК Предмет,
	|	&ПеревестиВСтатусОтгруженоВнутреннееПотребление КАК ТипСобытия,
	|	ВнутреннееПотреблениеТоваров.Дата КАК ДатаОбъектаСобытия,
	|	КОНЕЦПЕРИОДА(&ТекущаяДата, ДЕНЬ) КАК ДатаСобытия,
	|	ВнутреннееПотреблениеТоваров.Представление КАК Описание,
	|	NULL КАК ДопРеквизитОписание1,
	...
	Запрос.УстановитьПараметр("ПроконтролироватьОтгрузкуВнутреннееПотребление",НСтр("ru = 'Проконтроллировать отгрузку'"));
	Запрос.УстановитьПараметр("ПеревестиВСтатусОтгруженоВнутреннееПотребление",НСтр("ru = 'Перевести в стаутус отгружено'"));
...
Функция ТекстЗапросаДоверенностьПолучениеТоваров(Запрос,СтруктураФормированияОтчета)
	...
	ТекстЗапроса = "
	|ВЫБРАТЬ
	|	ДоверенностьНаПолучениеТоваров.Ссылка КАК СсылкаНаОбъектСобытия,
	|	ДоверенностьНаПолучениеТоваров.Ссылка КАК Предмет,
	|	&ПроконтролироватьСдачуДоверенности КАК ТипСобытия,
	|	ДоверенностьНаПолучениеТоваров.Дата КАК ДатаОбъектаСобытия,
	|	ВЫБОР
	|		КОГДА ДоверенностьНаПолучениеТоваров.ДатаОкончанияДействия = ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА КОНЕЦПЕРИОДА(&ТекущаяДата, ДЕНЬ)
	|		ИНАЧЕ КОНЕЦПЕРИОДА(ДоверенностьНаПолучениеТоваров.ДатаОкончанияДействия,ДЕНЬ)
	|	КОНЕЦ КАК ДатаСобытия,
	|	ДоверенностьНаПолучениеТоваров.Представление КАК Описание,
	|	NULL КАК ДопРеквизитОписание1,
	|	NULL КАК ДопРеквизитОписание2,
	|	ВЫБОР
	|		КОГДА ДоверенностьНаПолучениеТоваров.ДатаОкончанияДействия = ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА &Текущее
	|		ИНАЧЕ ВЫБОР
	|				КОГДА НАЧАЛОПЕРИОДА(&ТекущаяДата, ДЕНЬ) < ДоверенностьНаПолучениеТоваров.ДатаОкончанияДействия
	|					ТОГДА &Предстоящее
	|				КОГДА НАЧАЛОПЕРИОДА(&ТекущаяДата, ДЕНЬ) = ДоверенностьНаПолучениеТоваров.ДатаОкончанияДействия
	|					ТОГДА &Текущее
	|				ИНАЧЕ &Просроченное
	|			КОНЕЦ
	|	КОНЕЦ КАК Статус,
	|	ДоверенностьНаПолучениеТоваров.Партнер КАК Партнер,
	|	ДоверенностьНаПолучениеТоваров.Ответственный КАК Ответственный
	|%ТекстПоместить%
	|ИЗ
	|	Документ.ДоверенностьНаПолучениеТоваров КАК ДоверенностьНаПолучениеТоваров
	|ГДЕ
	|	НЕ ДоверенностьНаПолучениеТоваров.ПометкаУдаления
	|	И ДоверенностьНаПолучениеТоваров.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыДоверенностейНаПолучениеТоваров.Выдана)
	|	И ВЫБОР
	|			КОГДА ДоверенностьНаПолучениеТоваров.ДатаОкончанияДействия = ДАТАВРЕМЯ(1, 1, 1)
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ДоверенностьНаПолучениеТоваров.ДатаОкончанияДействия < &ДатаОкончания
	|		КОНЕЦ 
	|	%ТекстУсловияОтбор%";
	
	Запрос.УстановитьПараметр("ПроконтролироватьСдачуДоверенности",НСтр("ru = 'Проконтроллировать сдачу доверенности'"));
	
	ДобавитьЗапросПоЗадачам(ТекстЗапроса,"Документ.ДоверенностьНаПолучениеТоваров",СтруктураФормированияОтчета);
	
	Если НЕ ПустаяСтрока(СтруктураФормированияОтчета.ТекстУсловияПоПользователям) Тогда
		
		ТекстУсловияОтбор ="
		|И ДоверенностьНаПолучениеТоваров.Ответственный" + СтруктураФормированияОтчета.ТекстУсловияПоПользователям;
	...
	ДобавитьВМассивПроверяемыхНаПредметДоступаТипов(СтруктураФормированияОтчета,"Документ.ДоверенностьНаПолучениеТоваров");
...
Функция ТекстЗапросаВозвратыПоставщику(Запрос,СтруктураФормированияОтчета)
	...
	|	И ВозвратТоваровПоставщику.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыВозвратовТоваровПоставщикам.КОтгрузке)
	|	И ВЫБОР
	|			КОГДА Склады.ИспользоватьОрдернуюСхемуПриОтгрузке
	|					ИЛИ Склады.ИспользоватьОрдернуюСхемуПриОтраженииИзлишковНедостач
	|					ИЛИ Склады.ИспользоватьОрдернуюСхемуПриПоступлении
	...
	|	ВозвратТоваровПоставщику.Ссылка КАК СсылкаНаОбъектСобытия,
	|	ВозвратТоваровПоставщику.Ссылка КАК Предмет,
	|	&ПеревестиВСтатусОтгруженоВозвратПоставщику КАК ТипСобытия,
	|	ВозвратТоваровПоставщику.Дата КАК ДатаОбъектаСобытия,
	|	КОНЕЦПЕРИОДА(&ТекущаяДата, ДЕНЬ) КАК ДатаСобытия,
	|	ВозвратТоваровПоставщику.Представление КАК Описание,
	|	NULL КАК ДопРеквизитОписание1,
	...
	Запрос.УстановитьПараметр("ПеревестиВСтатусОтгруженоВозвратПоставщику",НСтр("ru = 'Перевести в статус отгружено'"));
...
Функция ТекстЗапросаРеализацииТоваровУслуг(Запрос,СтруктураФормированияОтчета)
	...
	|				И НЕ ЗаказКлиента.РеализацияПоЗаказам
	...
	|				%ТекстУсловияОтборЗаказ%) КАК РасчетыСКлиентамиПоДокументамОстатки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.РеализацияТоваровУслуг КАК РеализацияТоваровУслуг
	|		ПО РасчетыСКлиентамиПоДокументамОстатки.ЗаказКлиента = РеализацияТоваровУслуг.Ссылка
	|ГДЕ
	|	РасчетыСКлиентамиПоДокументамОстатки.КОплатеОстаток > 0";
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСтатусыРеализацийТоваровУслуг") Тогда
		...
		|	РеализацияТоваровУслуг.Ссылка КАК СсылкаНаОбъектСобытия,
		|	РеализацияТоваровУслуг.Ссылка КАК Предмет,
		|	&ПеревестиВСтатусКОтгрузке КАК ТипСобытия,
		|	РеализацияТоваровУслуг.Дата КАК ДатаОбъектаСобытия,
		|	КОНЕЦПЕРИОДА(&ТекущаяДата, ДЕНЬ) КАК ДатаСобытия,
		|	РеализацияТоваровУслуг.Представление КАК Описание,
		|	NULL КАК ДопРеквизитОписание1,
		...
		|			,
		|			ЗаказКлиента ССЫЛКА Документ.РеализацияТоваровУслуг
		|				И ЗаказКлиента.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыРеализацийТоваровУслуг.КПредоплате) %ТекстУсловияОтборЗаказ%) КАК РасчетыСКлиентамиПоДокументамОстатки
		|		ПО РеализацияТоваровУслуг.Ссылка = РасчетыСКлиентамиПоДокументамОстатки.ЗаказКлиента
		|ГДЕ
		|	РеализацияТоваровУслуг.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыРеализацийТоваровУслуг.КПредоплате) 
		|	И РеализацияТоваровУслуг.СуммаВзаиморасчетов - РеализацияТоваровУслуг.СуммаПредоплаты >= ЕСТЬNULL(РасчетыСКлиентамиПоДокументамОстатки.КОплатеОстаток, 0)";
	...
	|				ТОГДА 1
	|			КОГДА ЕСТЬNULL(Склады.ИспользоватьОрдернуюСхемуПриОтраженииИзлишковНедостач, ЛОЖЬ)
	|				ТОГДА 1
	|			КОГДА ЕСТЬNULL(Склады.ИспользоватьОрдернуюСхемуПриПоступлении, ЛОЖЬ)
	|				ТОГДА 1
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК ЕстьОрдерныеСклады,
	|	СУММА(ВЫБОР
	|			КОГДА ЕСТЬNULL(Склады.ИспользоватьОрдернуюСхемуПриОтгрузке, ЛОЖЬ) = ЛОЖЬ
	|					И ЕСТЬNULL(Склады.ИспользоватьОрдернуюСхемуПриОтраженииИзлишковНедостач, ЛОЖЬ) = ЛОЖЬ
	|					И ЕСТЬNULL(Склады.ИспользоватьОрдернуюСхемуПриПоступлении, ЛОЖЬ) = ЛОЖЬ
	|				ТОГДА 1
	|			ИНАЧЕ 0
...
Функция ТекстЗапросаЗаявкиНаВозврат(Запрос,СтруктураФормированияОтчета)
	...
	|			ИЛИ &НеИспользоватьСтатусыЗаявокНаВозврат)
	...
	|	ДокументЗаявкаНаВозвратТоваровОтКлиента.Ссылка КАК СсылкаНаОбъектСобытия,
	|	ДокументЗаявкаНаВозвратТоваровОтКлиента.Ссылка КАК Предмет,
	|	&ПодготовитьЗаявкуКВозврату КАК ТипСобытия,
	|	ДокументЗаявкаНаВозвратТоваровОтКлиента.Дата КАК ДатаОбъектаСобытия,
	|	ВЫБОР
	|		КОГДА ЗаявкаНаВозвратТоваровОтКлиентаВозвращаемыеТовары.ДатаПоступления = ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА КОНЕЦПЕРИОДА(&ТекущаяДата, ДЕНЬ)
	|		ИНАЧЕ КОНЕЦПЕРИОДА(ЗаявкаНаВозвратТоваровОтКлиентаВозвращаемыеТовары.ДатаПоступления,ДЕНЬ)
	|	КОНЕЦ КАК ДатаСобытия,
	|	ДокументЗаявкаНаВозвратТоваровОтКлиента.Представление КАК Описание,
	|	NULL КАК ДопРеквизитОписание1,
	|	NULL КАК ДопРеквизитОписание2,
	...
	|ГДЕ
	|	ДокументЗаявкаНаВозвратТоваровОтКлиента.Проведен
	|	И ДокументЗаявкаНаВозвратТоваровОтКлиента.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.Согласована)
	|	И ЗаявкаНаВозвратТоваровОтКлиентаВозвращаемыеТовары.ДатаПоступления < &ДатаОкончания %ТекстУсловияОтборДокумент%
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.РасчетыСКлиентамиПоДокументам.Остатки(, ДатаПлатежа < &ДатаОкончания И ЗаказКлиента ССЫЛКА Документ.ЗаявкаНаВозвратТоваровОтКлиента %ТекстУсловияЗаказ%) КАК РасчетыСКлиентамиПоДокументамОстатки
	|		ПО ЗаявкаНаВозвратТоваровОтКлиентаЭтапыГрафикаОплаты.Ссылка = РасчетыСКлиентамиПоДокументамОстатки.ЗаказКлиента
	|			И ЗаявкаНаВозвратТоваровОтКлиентаЭтапыГрафикаОплаты.ДатаПлатежа = РасчетыСКлиентамиПоДокументамОстатки.ДатаПлатежа
	|ГДЕ
	|	ДокументЗаявкаНаВозвратТоваровОтКлиента.Проведен
	|	И (ДокументЗаявкаНаВозвратТоваровОтКлиента.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.Согласована)
	...
	|	ЕСТЬNULL(СУММА(РасчетыСКлиентамиПоДокументамОстатки.КОплатеОстаток), 0) > 0
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ДокументЗаявкаНаВозвратТоваровОтКлиента.Ссылка КАК СсылкаНаОбъектСобытия,
	|	ДокументЗаявкаНаВозвратТоваровОтКлиента.Ссылка КАК Предмет,
	|	&ОбеспечениеПоЗаявкеНаВозврат КАК ТипСобытия,
	|	ДокументЗаявкаНаВозвратТоваровОтКлиента.Дата КАК ДатаОбъектаСобытия,
	|	ВЫБОР
	...
	|			ТОГДА &Текущее
	|		ИНАЧЕ ВЫБОР
	|				КОГДА ДокументЗаявкаНаВозвратТоваровОтКлиента.ДатаОтгрузки = НАЧАЛОПЕРИОДА(&ТекущаяДата, ДЕНЬ)
	|					ТОГДА &Текущее
	|				КОГДА ДокументЗаявкаНаВозвратТоваровОтКлиента.ДатаОтгрузки > НАЧАЛОПЕРИОДА(&ТекущаяДата, ДЕНЬ)
	|					ТОГДА &Предстоящее
	|				ИНАЧЕ &Просроченное
	...
	|					И ВЫБОР
	|						КОГДА ЗаказКлиента.ДатаОтгрузки <> ДАТАВРЕМЯ(1, 1, 1)
	|							ТОГДА ЗаказКлиента.ДатаОтгрузки < &ДатаОкончания
	|						ИНАЧЕ ИСТИНА
	|					КОНЕЦ %ТекстУсловияЗаказ%) КАК РасчетыСКлиентамиПоДокументамОстатки
	|		ПО ДокументЗаявкаНаВозвратТоваровОтКлиента.Ссылка = РасчетыСКлиентамиПоДокументамОстатки.ЗаказКлиента
	|ГДЕ
	|	ДокументЗаявкаНаВозвратТоваровОтКлиента.Проведен
	|	И ДокументЗаявкаНаВозвратТоваровОтКлиента.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.КВозврату)
	|	И ДокументЗаявкаНаВозвратТоваровОтКлиента.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказамНакладным)
	|	И (ДокументЗаявкаНаВозвратТоваровОтКлиента.СуммаАвансаДоОбеспечения = 0 ИЛИ (ДокументЗаявкаНаВозвратТоваровОтКлиента.СуммаАвансаДоОбеспечения > 0
	|	И ДокументЗаявкаНаВозвратТоваровОтКлиента.СуммаЗамены - ЕСТЬNULL(РасчетыСКлиентамиПоДокументамОстатки.КОплатеОстаток, 0) >= ДокументЗаявкаНаВозвратТоваровОтКлиента.СуммаАвансаДоОбеспечения))
	|	И ВЫБОР
	|			КОГДА ДокументЗаявкаНаВозвратТоваровОтКлиента.ДатаОтгрузки <> ДАТАВРЕМЯ(1, 1, 1)
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.РасчетыСКлиентамиПоДокументам.Остатки(
	|				,
	|				ДатаПлатежа < &ДатаОкончания
	|					И ЗаказКлиента ССЫЛКА Документ.ЗаявкаНаВозвратТоваровОтКлиента %ТекстУсловияЗаказ%) КАК РасчетыСКлиентамиПоДокументамОстатки
	|		ПО ЗаявкаНаВозвратТоваровОтКлиентаЭтапыГрафикаОплаты.Ссылка = РасчетыСКлиентамиПоДокументамОстатки.ЗаказКлиента
	|			И ЗаявкаНаВозвратТоваровОтКлиентаЭтапыГрафикаОплаты.ДатаПлатежа = РасчетыСКлиентамиПоДокументамОстатки.ДатаПлатежа
	|ГДЕ
	|	ДокументЗаявкаНаВозвратТоваровОтКлиента.Проведен
	...
	|		ДокументЗаявкаНаВозвратТоваровОтКлиента.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.Согласована) ИЛИ
	|		ДокументЗаявкаНаВозвратТоваровОтКлиента.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.КОбеспечению) ИЛИ
	...
	|			ИЛИ &НеИспользоватьСтатусыЗаявокНаВозврат)
	|	И ДокументЗаявкаНаВозвратТоваровОтКлиента.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказамНакладным)
	|	И ДокументЗаявкаНаВозвратТоваровОтКлиента.СуммаЗамены > 0
	|	И (ДокументЗаявкаНаВозвратТоваровОтКлиента.СуммаПредоплатыДоОтгрузки = 0
	|			ИЛИ ДокументЗаявкаНаВозвратТоваровОтКлиента.СуммаАвансаДоОбеспечения + ДокументЗаявкаНаВозвратТоваровОтКлиента.СуммаПредоплатыДоОтгрузки > 0
	|				И ДокументЗаявкаНаВозвратТоваровОтКлиента.СуммаЗамены - ЕСТЬNULL(РасчетыСКлиентамиПоДокументамОстатки.КОплатеОстаток, 0) >= ДокументЗаявкаНаВозвратТоваровОтКлиента.СуммаАвансаДоОбеспечения + ДокументЗаявкаНаВозвратТоваровОтКлиента.СуммаПредоплатыДоОтгрузки)
	|	И ВЫБОР
	|			КОГДА ДокументЗаявкаНаВозвратТоваровОтКлиента.ДатаОтгрузки <> ДАТАВРЕМЯ(1, 1, 1)
	|				ТОГДА ДокументЗаявкаНаВозвратТоваровОтКлиента.ДатаОтгрузки < &ДатаОкончания
	|			ИНАЧЕ ИСТИНА
	...
	|				И (ЗаказКлиента.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.КОтгрузке) ИЛИ &НеИспользоватьСтатусыЗаявокНаВозврат) %ТекстУсловияЗаказ%) КАК ЗаказыКлиентовОстатки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаявкаНаВозвратТоваровОтКлиента КАК ДокументЗаявкаНаВозвратТоваровОтКлиента
	|		ПО ЗаказыКлиентовОстатки.ЗаказКлиента = ДокументЗаявкаНаВозвратТоваровОтКлиента.Ссылка
	...
	|		ПО ЗаказыКлиентовОстатки.КодСтроки = ЗаявкаНаВозвратТоваровОтКлиентаТовары.КодСтроки
	|			И ЗаказыКлиентовОстатки.ЗаказКлиента = ЗаявкаНаВозвратТоваровОтКлиентаТовары.Ссылка
	|ГДЕ
	|	ЗаказыКлиентовОстатки.КОформлениюОстаток > 0 %ТекстУсловияОтборДокумент%
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|			,
	|			ЗаказКлиента ССЫЛКА Документ.ЗаявкаНаВозвратТоваровОтКлиента
	|				И ЗаказКлиента.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.КОтгрузке) %ТекстУсловияЗаказ%) КАК РасчетыСКлиентамиПоДокументамОстатки
	|	ПО ДокументЗаявкаНаВозвратТоваровОтКлиента.Ссылка = РасчетыСКлиентамиПоДокументамОстатки.ЗаказКлиента
	|ГДЕ
	|	ДокументЗаявкаНаВозвратТоваровОтКлиента.Проведен
	|	И ДокументЗаявкаНаВозвратТоваровОтКлиента.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.КОтгрузке)
	|	И ДокументЗаявкаНаВозвратТоваровОтКлиента.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказамНакладным)
	|	И ЕСТЬNULL(РасчетыСКлиентамиПоДокументамОстатки.КОтгрузкеОстаток, 0) = 0
	|	И ЕСТЬNULL(РасчетыСКлиентамиПоДокументамОстатки.КОплатеОстаток, 0) = 0 %ТекстУсловияОтборДокумент%";
...
Функция ТекстЗапросаЗаказыПоставщикам(Запрос,СтруктураФормированияОтчета)
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.РасчетыСПоставщикамиПоДокументам.Остатки(, ДатаПлатежа < &ДатаОкончания %ТекстУсловияОтбор%) КАК РасчетыСПоставщикамиПоДокументамОстатки
	|		ПО ЗаказПоставщикуЭтапыГрафикаОплаты.Ссылка = РасчетыСПоставщикамиПоДокументамОстатки.ЗаказПоставщику
	|			И ЗаказПоставщикуЭтапыГрафикаОплаты.ДатаПлатежа = РасчетыСПоставщикамиПоДокументамОстатки.ДатаПлатежа
	|ГДЕ
	...
	|	ЕСТЬNULL(СУММА(РасчетыСПоставщикамиПоДокументамОстатки.КОплатеОстаток), 0) < 0
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.РасчетыСПоставщикамиПоДокументам.Остатки(
	|			,
	...
	|					ИНАЧЕ ИСТИНА
	|				КОНЕЦ %ТекстУсловияОтбор%) КАК РасчетыСПоставщикамиПоДокументамОстатки
	|			ПО ДокументЗаказПоставщику.Ссылка = РасчетыСПоставщикамиПоДокументамОстатки.ЗаказПоставщику
	|ГДЕ
	|	ДокументЗаказПоставщику.Проведен
	|	И ДокументЗаказПоставщику.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПоставщикам.Согласован)
	|	И ДокументЗаказПоставщику.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказамНакладным)
	|	И ДокументЗаказПоставщику.СуммаАвансаДоПодтверждения > 0
	|	И ДокументЗаказПоставщику.СуммаДокумента + ЕСТЬNULL(РасчетыСПоставщикамиПоДокументамОстатки.КОплатеОстаток, 0) >= ДокументЗаказПоставщику.СуммаАвансаДоПодтверждения
	|	И ВЫБОР
	|		КОГДА ДокументЗаказПоставщику.ДатаПервогоПоступления <> ДАТАВРЕМЯ(1, 1, 1)
	...
	|	И ДокументЗаказПоставщику.СуммаДокумента + ЕСТЬNULL(РасчетыСПоставщикамиПоДокументамОстатки.КОплатеОстаток, 0) >= (ДокументЗаказПоставщику.СуммаАвансаДоПодтверждения + ДокументЗаказПоставщику.СуммаПредоплатыДоПоступления)
	|	И ВЫБОР
	|		КОГДА ДокументЗаказПоставщику.ДатаПервогоПоступления <> ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА ДокументЗаказПоставщику.ДатаПервогоПоступления < &ДатаОкончания
	|		ИНАЧЕ ИСТИНА
	...
	|			,
	|			ЗаказПоставщику ССЫЛКА Документ.ЗаказПоставщику
	|			И ЗаказПоставщику.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПоставщикам.КПоступлению) %ТекстУсловияОтбор%) КАК РасчетыСПоставщикамиПоДокументамОстатки
	|			ПО ДокументЗаказПоставщику.Ссылка = РасчетыСПоставщикамиПоДокументамОстатки.ЗаказПоставщику
	|ГДЕ
	|	ДокументЗаказПоставщику.Проведен
	|	И ДокументЗаказПоставщику.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПоставщикам.КПоступлению)
	...
	|	И ЕСТЬNULL(РасчетыСПоставщикамиПоДокументамОстатки.КПоступлениюОстаток, 0) = 0
	|	И ЕСТЬNULL(РасчетыСПоставщикамиПоДокументамОстатки.КОплатеОстаток, 0) = 0 %ТекстУсловияОтборДокумент%";
...
Функция ТекстЗапросаЗаказыКлиентов(Запрос,СтруктураФормированияОтчета)
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.РасчетыСКлиентамиПоДокументам.Остатки(, ДатаПлатежа < &ДатаОкончания %ТекстУсловияОтбор%) КАК РасчетыСКлиентамиПоДокументамОстатки
	|		ПО ЗаказКлиентаЭтапыГрафикаОплаты.Ссылка = РасчетыСКлиентамиПоДокументамОстатки.ЗаказКлиента
	|			И ЗаказКлиентаЭтапыГрафикаОплаты.ДатаПлатежа = РасчетыСКлиентамиПоДокументамОстатки.ДатаПлатежа
	|ГДЕ
	|	ДокументЗаказКлиента.Проведен
	|	И (ДокументЗаказКлиента.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовКлиентов.Согласован)
	|		ИЛИ &НеИспользоватьСтатусыЗаказовКлиента)
	...
	|	ДокументЗаказКлиента.Ссылка КАК Предмет,
	|	&ОбеспечениеЗаказа КАК ТипСобытия,
	...
	|			ТОГДА &Текущее
	|		ИНАЧЕ ВЫБОР
	|			КОГДА ДокументЗаказКлиента.ДатаОтгрузки = НАЧАЛОПЕРИОДА(&ТекущаяДата, ДЕНЬ)
	|				ТОГДА &Текущее
	|			КОГДА ДокументЗаказКлиента.ДатаОтгрузки > НАЧАЛОПЕРИОДА(&ТекущаяДата, ДЕНЬ)
	|				ТОГДА &Предстоящее
	|			ИНАЧЕ &Просроченное
	...
	|						ТОГДА ЗаказКлиента.ДатаОтгрузки < &ДатаОкончания
	|					ИНАЧЕ ИСТИНА
	|				КОНЕЦ %ТекстУсловияОтбор%) КАК РасчетыСКлиентамиПоДокументамОстатки
	|	ПО ДокументЗаказКлиента.Ссылка = РасчетыСКлиентамиПоДокументамОстатки.ЗаказКлиента
	|ГДЕ
	|	ДокументЗаказКлиента.Проведен
	|	И ДокументЗаказКлиента.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовКлиентов.Согласован)
	|	И ДокументЗаказКлиента.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказамНакладным)
	|	И ДокументЗаказКлиента.СуммаАвансаДоОбеспечения > 0
	|	И ДокументЗаказКлиента.СуммаДокумента - ЕСТЬNULL(РасчетыСКлиентамиПоДокументамОстатки.КОплатеОстаток, 0) >= ДокументЗаказКлиента.СуммаАвансаДоОбеспечения
	|	И ВЫБОР
	|		КОГДА ДокументЗаказКлиента.ДатаОтгрузки <> ДАТАВРЕМЯ(1, 1, 1)
	...
	|	И ДокументЗаказКлиента.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовКлиентов.КОбеспечению)
	|	И ДокументЗаказКлиента.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказамНакладным)
	|	И (ДокументЗаказКлиента.СуммаАвансаДоОбеспечения + ДокументЗаказКлиента.СуммаПредоплатыДоОтгрузки) > 0
	|	И ДокументЗаказКлиента.СуммаДокумента - ЕСТЬNULL(РасчетыСКлиентамиПоДокументамОстатки.КОплатеОстаток, 0) >= (ДокументЗаказКлиента.СуммаАвансаДоОбеспечения + ДокументЗаказКлиента.СуммаПредоплатыДоОтгрузки)
	|	И ВЫБОР
	|		КОГДА ДокументЗаказКлиента.ДатаОтгрузки <> ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА ДокументЗаказКлиента.ДатаОтгрузки < &ДатаОкончания
	|		ИНАЧЕ ИСТИНА
	...
	|		,
	|		ЗаказКлиента ССЫЛКА Документ.ЗаказКлиента
	|			И ЗаказКлиента.Товары.ДатаОтгрузки < &ДатаОкончания
	|			И (ЗаказКлиента.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовКлиентов.КОтгрузке) ИЛИ &НеИспользоватьСтатусыЗаказовКлиента) %ТекстУсловияОтбор%) КАК ЗаказыКлиентовОстатки
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказКлиента КАК ДокументЗаказКлиента
	|		ПО ЗаказыКлиентовОстатки.ЗаказКлиента = ДокументЗаказКлиента.Ссылка
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказКлиента.Товары КАК ЗаказКлиентаТовары
	|		ПО ЗаказыКлиентовОстатки.КодСтроки = ЗаказКлиентаТовары.КодСтроки
	|			И ЗаказыКлиентовОстатки.ЗаказКлиента = ЗаказКлиентаТовары.Ссылка
	|ГДЕ
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказКлиента КАК ДокументЗаказКлиента
	|		ПО ЗаказКлиентаЭтапыГрафикаОплаты.Ссылка = ДокументЗаказКлиента.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.РасчетыСКлиентамиПоДокументам.Остатки(, ДатаПлатежа < &ДатаОкончания И ЗаказКлиента Ссылка Документ.ЗаказКлиента %ТекстУсловияОтбор%) КАК РасчетыСКлиентамиПоДокументамОстатки
	|		ПО ЗаказКлиентаЭтапыГрафикаОплаты.Ссылка = РасчетыСКлиентамиПоДокументамОстатки.ЗаказКлиента
	|			И ЗаказКлиентаЭтапыГрафикаОплаты.ДатаПлатежа = РасчетыСКлиентамиПоДокументамОстатки.ДатаПлатежа
	|ГДЕ
	|	ДокументЗаказКлиента.Проведен
	...
	|			,
	|			ЗаказКлиента ССЫЛКА Документ.ЗаказКлиента
	|				И ЗаказКлиента.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовКлиентов.КОтгрузке) %ТекстУсловияОтбор%) КАК РасчетыСКлиентамиПоДокументамОстатки
	|	ПО ДокументЗаказКлиента.Ссылка = РасчетыСКлиентамиПоДокументамОстатки.ЗаказКлиента
	|ГДЕ
	|	ДокументЗаказКлиента.Проведен
	|	И ДокументЗаказКлиента.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовКлиентов.КОтгрузке)
...
Функция ТекстЗапросаВходящиеПисьма(Запрос, СтруктураФормированияОтчета)
	...
	|	ВЫБОР
	|		КОГДА ЭлектронноеПисьмоВходящее.РассмотретьПосле = ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА ЭлектронноеПисьмоВходящее.Дата
	|		ИНАЧЕ ЭлектронноеПисьмоВходящее.РассмотретьПосле
	|	КОНЕЦ КАК ДатаСобытия,
	...
	|				КОНЕЦ
	|		ИНАЧЕ ВЫБОР
	|				КОГДА ЭлектронноеПисьмоВходящее.РассмотретьПосле > &ТекущаяДата
	|					ТОГДА &Предстоящее
	|				КОГДА ДОБАВИТЬКДАТЕ(ЭлектронноеПисьмоВходящее.РассмотретьПосле, ЧАС, 1) < &ТекущаяДата
	|					ТОГДА &Просроченное
	|				ИНАЧЕ &Текущее
	|			КОНЕЦ
	|	КОНЕЦ КАК Статус,
	...
	|			ИНАЧЕ ЭлектронноеПисьмоВходящее.РассмотретьПосле < &ДатаОкончания
	|	КОНЕЦ
	|	%ТекстУсловияОтбор%";
...
Функция ТекстЗапросаИсходящиеПисьма(Запрос, СтруктураФормированияОтчета)
	...
	|	ВЫБОР
	|		КОГДА ЭлектронноеПисьмоИсходящее.РассмотретьПосле = ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА ЭлектронноеПисьмоИсходящее.ДатаОтправления
	|		ИНАЧЕ ЭлектронноеПисьмоИсходящее.РассмотретьПосле
	|	КОНЕЦ КАК ДатаСобытия,
	...
	|				КОНЕЦ
	|		ИНАЧЕ ВЫБОР
	|				КОГДА ЭлектронноеПисьмоИсходящее.РассмотретьПосле > &ТекущаяДата
	|					ТОГДА &Предстоящее
	|				КОГДА ДОБАВИТЬКДАТЕ(ЭлектронноеПисьмоИсходящее.РассмотретьПосле, ЧАС, 1) < &ТекущаяДата
	|					ТОГДА &Просроченное
	|				ИНАЧЕ &Текущее
	|			КОНЕЦ
	|	КОНЕЦ КАК Статус,
	...
	|			ИНАЧЕ ЭлектронноеПисьмоИсходящее.РассмотретьПосле < &ДатаОкончания
	|	КОНЕЦ
	|	%ТекстУсловияОтбор%";
...
Функция ТекстЗапросаВстречи(Запрос, СтруктураФормированияОтчета)
	...
	|	ВЫБОР
	|		КОГДА Встреча.РассмотретьПосле = ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА Встреча.ДатаОкончания
	|		ИНАЧЕ Встреча.РассмотретьПосле
	|	КОНЕЦ КАК ДатаСобытия,
	...
	|				КОНЕЦ
	|		ИНАЧЕ ВЫБОР
	|				КОГДА Встреча.РассмотретьПосле > &ТекущаяДата
	|					ТОГДА &Предстоящее
	|				КОГДА ДОБАВИТЬКДАТЕ(Встреча.РассмотретьПосле, ЧАС, 1) < &ТекущаяДата
	|					ТОГДА &Просроченное
	|				ИНАЧЕ &Текущее
	|			КОНЕЦ
	|	КОНЕЦ КАК Статус,
	...
	|			КОГДА Встреча.РассмотретьПосле = ДАТАВРЕМЯ(1, 1, 1)
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ Встреча.РассмотретьПосле < &ДатаОкончания
	|	КОНЕЦ
	|	%ТекстУсловияОтбор%
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	Запрос.УстановитьПараметр("РассмотретьВcтречу",НСтр("ru = 'Отработать результаты встречи'"));
...
Функция ТекстЗапросаТелефонныеЗвонки(Запрос, СтруктураФормированияОтчета)
	...
	|	ВЫБОР
	|		КОГДА ТелефонныйЗвонок.РассмотретьПосле = ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА ТелефонныйЗвонок.Дата
	|		ИНАЧЕ ТелефонныйЗвонок.РассмотретьПосле
	|	КОНЕЦ КАК ДатаСобытия,
	...
	|				КОНЕЦ
	|		ИНАЧЕ ВЫБОР
	|				КОГДА ТелефонныйЗвонок.РассмотретьПосле > &ТекущаяДата
	|					ТОГДА &Предстоящее
	|				КОГДА ДОБАВИТЬКДАТЕ(ТелефонныйЗвонок.РассмотретьПосле, ЧАС, 1) < &ТекущаяДата
	|					ТОГДА &Просроченное
	|				ИНАЧЕ &Текущее
	|			КОНЕЦ
	|	КОНЕЦ КАК Статус,
	...
	|			ИНАЧЕ ТелефонныйЗвонок.РассмотретьПосле < &ДатаОкончания
	|	КОНЕЦ 
	|	%ТекстУсловияОтбор%";
...
Функция ТекстЗапросаЗапланированныеВзаимодействия(Запрос, СтруктураФормированияОтчета)
	...
	|	ВЫБОР
	|		КОГДА ЗапланированноеВзаимодействие.РассмотретьПосле = ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА ЗапланированноеВзаимодействие.Дата
	|		ИНАЧЕ ЗапланированноеВзаимодействие.РассмотретьПосле
	|	КОНЕЦ КАК ДатаСобытия,
	...
	|				КОНЕЦ
	|		ИНАЧЕ ВЫБОР
	|				КОГДА ЗапланированноеВзаимодействие.РассмотретьПосле > &ТекущаяДата
	|					ТОГДА &Предстоящее
	|				КОГДА ДОБАВИТЬКДАТЕ(ЗапланированноеВзаимодействие.РассмотретьПосле, ЧАС, 1) < &ТекущаяДата
	|					ТОГДА &Просроченное
	|				ИНАЧЕ &Текущее
	|			КОНЕЦ
	|	КОНЕЦ КАК Статус,
	...
	|			ИНАЧЕ ЗапланированноеВзаимодействие.РассмотретьПосле < &ДатаОкончания
	|	КОНЕЦ 
	|	%ТекстУсловияОтбор%";
	...
	Пока ВыборкаСтатус.Следующий() Цикл
		...
		Иначе
			Если ВыборкаСтатус.Статус = "Текущее" Тогда
				...
				ОбластьГруппировка.Параметры.Значение = НСтр("ru = 'Сегодня ( '") + Формат(ТекущаяДата(),"ДЛФ=DD") + " ) - " + ВыборкаСтатус.ДатаОбъектаСобытия;
...
Процедура ПреобразоватьПредставлениеПредмета(Описание,Выборка)
	...
	Если ТипЗнч(Выборка.Предмет) = Тип("СправочникСсылка.СоглашенияСПоставщиками") ИЛИ
		...
		Если Дата(Выборка.ДатаОбъектаСобытия) <> Дата(1,1,1) Тогда
			Описание = Описание + " от " + Формат(Выборка.ДатаОбъектаСобытия,"ДЛФ=D");
		КонецЕсли;
		
	ИначеЕсли ТипЗнч(Выборка.Предмет) = Тип("СправочникСсылка.СделкиСКлиентами") Тогда
		
		Описание = НСтр("ru = 'Сделка '") + "'" + Описание + "'" + " от " + Формат(Выборка.ДатаОбъектаСобытия,"ДЛФ=D");
	...
	ИначеЕсли ТипЗнч(Выборка.Предмет) = Тип("ДокументСсылка.ЭлектронноеПисьмоВходящее") Тогда
		...
		Если ТипЗнч(Выборка.СсылкаНаОбъектСобытия) = Тип("ДокументСсылка.ЭлектронноеПисьмоВходящее") Тогда
			Описание = НСтр("ru = 'Входящее письмо '") + " '" + ?(ПустаяСтрока(Выборка.Описание),НСтр("ru = 'Тема не указана '"),Выборка.Описание) + "' от " + Выборка.ДатаОбъектаСобытия;
		Иначе
			Описание = НСтр("ru = 'Входящее письмо '") + " '" + ?(ПустаяСтрока(Выборка.ДопРеквизитОписание1),НСтр("ru = 'Тема не указана '"),Выборка.ДопРеквизитОписание1) + "' от " + Выборка.ДопРеквизитОписание2;
		КонецЕсли;
	...
	ИначеЕсли ТипЗнч(Выборка.Предмет) = Тип("ДокументСсылка.ЭлектронноеПисьмоИсходящее") Тогда
		...
		Если ТипЗнч(Выборка.СсылкаНаОбъектСобытия) = Тип("ДокументСсылка.ЭлектронноеПисьмоИсходящее") Тогда
			Описание = НСтр("ru = 'Исходящее письмо '") + " '" + ?(ПустаяСтрока(Выборка.Описание),НСтр("ru = 'Тема не указана '"),Выборка.Описание) + "' от " + Выборка.ДатаОбъектаСобытия;
		Иначе
			Описание = НСтр("ru = 'Исходящее письмо '") + " '" + ?(ПустаяСтрока(Выборка.ДопРеквизитОписание1),НСтр("ru = 'Тема не указана '"),Выборка.ДопРеквизитОписание1) + "' от " + Выборка.ДопРеквизитОписание2;
		КонецЕсли;
	...
	ИначеЕсли ТипЗнч(Выборка.Предмет) = Тип("ДокументСсылка.Встреча") Тогда
		...
		Если ТипЗнч(Выборка.СсылкаНаОбъектСобытия) = Тип("ДокументСсылка.Встреча") Тогда
			Описание = НСтр("ru = 'Встреча '") + " '" + ?(ПустаяСтрока(Выборка.Описание),НСтр("ru = 'Тема не указана '"),Выборка.Описание) + "' от " + Выборка.ДатаОбъектаСобытия;
		Иначе
			Описание = НСтр("ru = 'Встреча '") + " '" + ?(ПустаяСтрока(Выборка.ДопРеквизитОписание1),НСтр("ru = 'Тема не указана '"),Выборка.ДопРеквизитОписание1) + "' от " + Выборка.ДопРеквизитОписание2;
		КонецЕсли;
	...
	ИначеЕсли ТипЗнч(Выборка.Предмет) = Тип("ДокументСсылка.ЗапланированноеВзаимодействие") Тогда
		...
		Если ТипЗнч(Выборка.СсылкаНаОбъектСобытия) = Тип("ДокументСсылка.ЗапланированноеВзаимодействие") Тогда
			Описание = НСтр("ru = 'Запланированное взаимодействие '") + " '" + ?(ПустаяСтрока(Выборка.Описание),НСтр("ru = 'Тема не указана '"),Выборка.Описание) + "' от " + Выборка.ДатаОбъектаСобытия;
		Иначе
			Описание = НСтр("ru = 'Запланированное взаимодействие '") + " '" + ?(ПустаяСтрока(Выборка.ДопРеквизитОписание1),НСтр("ru = 'Тема не указана '"),Выборка.ДопРеквизитОписание1) + "' от " + Выборка.ДопРеквизитОписание2;
		КонецЕсли;
	...
	ИначеЕсли ТипЗнч(Выборка.Предмет) = Тип("ДокументСсылка.ТелефонныйЗвонок") Тогда
		...
		Если ТипЗнч(Выборка.СсылкаНаОбъектСобытия) = Тип("ДокументСсылка.ТелефонныйЗвонок") Тогда
			...
			Описание = ?(Выборка.ДопРеквизитОписание1,НСтр("ru = 'Входящий '"),НСтр("ru = 'Исходящий '")) + 
			           НСтр("ru = ' тел. звонок '") + " '" + ?(ПустаяСтрока(Выборка.Описание),НСтр("ru = 'Тема не указана '"),Выборка.Описание) + 
			           "' от " + Выборка.ДатаОбъектаСобытия;
		...
		Иначе
			...
			Описание = НСтр("ru = 'Телефонный звонок '") + " '" + ?(ПустаяСтрока(Выборка.ДопРеквизитОписание1),НСтр("ru = 'Тема не указана '"),Выборка.ДопРеквизитОписание1) + 
			            "' от " + Выборка.ДопРеквизитОписание2;
