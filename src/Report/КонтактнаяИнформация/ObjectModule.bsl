		...
		Если ЕстьКИПартнера Тогда
			...
			Пока ВыборкаКИПартнера.Следующий() Цикл
				...
				Пока ВыборкаПредставлениеКИ.Следующий() Цикл
					ОбластьСтрокаРеквизитПартнера.Параметры.Значение = ВыборкаПредставлениеКИ.ПредставлениеКИПартнера;
					ТаблицаОтчета.Присоединить(ОбластьСтрокаРеквизитПартнера);
					ТекущаяКолонкаКИПартнера = ТекущаяКолонкаКИПартнера + 1;
				КонецЦикла;
		...
		Если ЕстьДанныеПоРолям Тогда
			...
			Пока ВыборкаРоли.Следующий() Цикл
					...
					Пока ВыборкаКЛ.Следующий() Цикл
						...
						Пока ВыборкаВидыКИКЛ.Следующий() Цикл
							...
							Если ВыборкаДетали.Следующий() Тогда
								...
								Если НЕ ПустаяСтрока(ВыборкаДетали.ПредставлениеКИКЛ) Тогда
									Если КоличествоВыбранныхКИКЛ = 1 Тогда
										ТекстКЛ = ВыборкаДетали.ПредставлениеКИКЛ;
									Иначе
										ТекстКЛ = ТекстКЛ +  ?(ПустаяСтрока(ТекстКЛ),"",Символы.ПС)+ Символы.Таб +  ВыборкаДетали.ВидКИКЛ + ": " + ВыборкаДетали.ПредставлениеКИКЛ;
									КонецЕсли;
								КонецЕсли;
							КонецЕсли;
						...
						Иначе
							ТекстКЛ = НаименованиеКЛ + ?(ПустаяСтрока(ТекстКЛ),"",Символы.ПС + ТекстКЛ);
							ТекстРоль = ТекстРоль + ?(ПустаяСтрока(ТекстРоль),"", Символы.ПС) + ТекстКЛ;
						КонецЕсли;
		...
		Если КоличествоКолонокКИ > 0 Тогда
			...
			Пока ВыборкаКЛ.Следующий() Цикл 
				...
				Пока ВыборкаВидыКИКЛ.Следующий() Цикл
					...
					ВыборкаДетали = ВыборкаВидыКИКЛ.Выбрать();
					Пока ВыборкаДетали.Следующий() Цикл
						ПредставлениеКИ = ВыборкаДетали.ПредставлениеКИКЛ;
					КонецЦикла;
...
Процедура ДобавитьВЗапросДанныеКлассификации(Запрос,СтруктураФормированияОтчета)
	...
	Если ЕстьВыводКлассификации Тогда
		...
		Запрос.УстановитьПараметр("НеКлассифицирован","Не классифицирован");
...
Процедура ДобавитьВЗапросОтбор(Запрос)
	...
	НайденнаяПозицияИЗ = Найти(ТекстЗапросаОтбор,"ИЗ");
