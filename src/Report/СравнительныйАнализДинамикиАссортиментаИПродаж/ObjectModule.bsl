...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	ЭтоРасшифровка = ВыбранныйВариант <> "ПоКатегориям";
...
Функция РезультатВыборкиДанных(ПредставлениеОтбора, МассивПолей)
	...
	|	ВЫБОР КОГДА СУММА(ПодзапросРезультат.КоличествоОборотПериодаСравнения) = 0 ТОГДА 0
	|		ИНАЧЕ (СУММА(ПодзапросРезультат.КоличествоОборот) - СУММА(ПодзапросРезультат.КоличествоОборотПериодаСравнения)) * 100 / СУММА(ПодзапросРезультат.КоличествоОборотПериодаСравнения)
	...
	|	ВЫБОР КОГДА СУММА(ПодзапросРезультат.СуммаВыручкиРеглОборотПериодаСравнения) = 0 ТОГДА 0
	|		ИНАЧЕ (СУММА(ПодзапросРезультат.СуммаВыручкиРеглОборот) - СУММА(ПодзапросРезультат.СуммаВыручкиРеглОборотПериодаСравнения)) * 100 / СУММА(ПодзапросРезультат.СуммаВыручкиРеглОборотПериодаСравнения)
	...
	|	ВЫБОР КОГДА СУММА(ПодзапросРезультат.РТНОборотПериодаСравнения) = 0 ТОГДА 0
	|		ИНАЧЕ (СУММА(ПодзапросРезультат.РТНОборот) - СУММА(ПодзапросРезультат.РТНОборотПериодаСравнения)) * 100 / СУММА(ПодзапросРезультат.РТНОборотПериодаСравнения)
	...
	|	ВЫБОР КОГДА СУММА(ПодзапросРезультат.Квота) < СУММА(ПодзапросРезультат.Наполненность) ТОГДА
	|		ВЫБОР КОГДА СУММА(ПодзапросРезультат.Квота) = 0 ТОГДА 0
	|			ИНАЧЕ (СУММА(ПодзапросРезультат.Представленность) - СУММА(ПодзапросРезультат.Квота)) * 100 / СУММА(ПодзапросРезультат.Квота)
	|		КОНЕЦ 
	|	ИНАЧЕ
	|		ВЫБОР КОГДА СУММА(ПодзапросРезультат.Наполненность) = 0 ТОГДА 0
	|			ИНАЧЕ (СУММА(ПодзапросРезультат.Представленность) - СУММА(ПодзапросРезультат.Наполненность)) * 100 / СУММА(ПодзапросРезультат.Наполненность)
	|		КОНЕЦ 
	|	КОНЕЦ КАК ОтклонениеПредставленности,
	|	ВЫБОР КОГДА СУММА(ПодзапросРезультат.Квота) = 0 ТОГДА 0
	|		ИНАЧЕ (СУММА(ПодзапросРезультат.Наполненность) - СУММА(ПодзапросРезультат.Квота)) * 100 / СУММА(ПодзапросРезультат.Квота)
	...
	|	ПодзапросРезультат.ИзменениеКоличестваОтносительное КАК ИзменениеКоличестваОтносительное,
	...
	|	ПодзапросРезультат.ИзменениеСуммыВыручкиОтносительное КАК ИзменениеСуммыВыручкиОтносительное,
	...
	|	ПодзапросРезультат.ИзменениеВаловойПрибылиОтносительное КАК ИзменениеВаловойПрибылиОтносительное,
	...
	|	ПодзапросРезультат.ОтклонениеПредставленности КАК ОтклонениеПредставленности,
	|	ПодзапросРезультат.ОтклонениеНаполненности КАК ОтклонениеНаполненности,
	...
	СоответствиеПолейУсловия.Вставить("ИзменениеКоличестваОтносительное", "ПодзапросРезультат.ИзменениеКоличестваОтносительное");
	СоответствиеПолейУсловия.Вставить("ИзменениеСуммыВыручкиОтносительное", "ПодзапросРезультат.ИзменениеСуммыВыручкиОтносительное");
	СоответствиеПолейУсловия.Вставить("ИзменениеВаловойПрибылиОтносительное", "ПодзапросРезультат.ИзменениеВаловойПрибылиОтносительное");
...
Функция ТекстЗапросаТаблицыФорматов()
	...
	|		ПО (Склады.ТипСклада = ЗНАЧЕНИЕ(Перечисление.ТипыСкладов.РозничныйМагазин))
	|		%УсловиеМагазины%
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
...
Функция ТекстЗапросаКвоты()
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Склады КАК Склады
	|		ПО (Склады.ТипСклада = ЗНАЧЕНИЕ(Перечисление.ТипыСкладов.РозничныйМагазин))
	|			И (Склады.ФорматМагазина = втТекущиеКвотыБезМагазинов.ФорматМагазина)
	|			%УсловиеМагазины%
	|;
	|";
	Если ЭтоРасшифровка Тогда
...
Функция ТекстЗапросаОборотыТоваровНаСкладах()
	...
	Если ЭтоРасшифровка Тогда
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|		ОстаткиИОбороты.Склад.ФорматМагазина КАК ФорматМагазина,
	...
	|ИЗ
	|	(ВЫБРАТЬ
	|		ТД.Склад.ФорматМагазина КАК ФорматМагазина,
...
Функция ТекстЗапросаАссортимент()
	...
	|						И Ассорти.Период <= ТД.День
	|					УПОРЯДОЧИТЬ ПО
	|						Ассорти.Период УБЫВ,
	|						Ассорти.ДатаРегистратора УБЫВ,
	|						Ассорти.Регистратор УБЫВ))) КАК ТаблицаДней
...
Функция ТекстЗапросаВыручка()
	...
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ВыручкаОбороты.АналитикаУчетаНоменклатуры.Склад.ФорматМагазина КАК ФорматМагазина,
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	ВыручкаОбороты.АналитикаУчетаНоменклатуры.Склад.ФорматМагазина,
	...
	Если ЭтоРасшифровка Тогда
		...
		|			И (ТекущиеКвоты.Марка = ВыручкаОбороты.АналитикаУчетаНоменклатуры.Номенклатура.Марка)
		|			И (ТекущиеКвоты.ФорматМагазина = ВыручкаОбороты.АналитикаУчетаНоменклатуры.Склад.ФорматМагазина)
		|СГРУППИРОВАТЬ ПО
		|	ВыручкаОбороты.АналитикаУчетаНоменклатуры.Склад.ФорматМагазина,
		|	ВыручкаОбороты.АналитикаУчетаНоменклатуры.Склад,
		|	ВыручкаОбороты.АналитикаУчетаНоменклатуры.Номенклатура.ВидНоменклатуры,
...
Функция ТекстЗапросаИтоговыеТаблицы()
	...
	|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Склады КАК Склады
	|			ПО (Склады.ТипСклада = ЗНАЧЕНИЕ(Перечисление.ТипыСкладов.РозничныйМагазин))
	|				И (Склады.ФорматМагазина = Наполненность.ФорматМагазина)
	|				%УсловиеМагазины%
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	...
	|			ПО (Склады.ТипСклада = ЗНАЧЕНИЕ(Перечисление.ТипыСкладов.РозничныйМагазин))
	|				%УсловиеМагазины%
	|				И (Склады.ФорматМагазина = Квоты.ФорматМагазина)) КАК тРезультат
...
Процедура ДобавитьУсловияИПараметры(Запрос, ТекстЗапроса, ЭлементыОтбора)
	...
	СчетчикУсловий = 0;
	СоответствиеПолейУсловия = Новый Соответствие;
	СоответствиеПолейУсловия.Вставить("Магазин", "Склады.Ссылка");
	СоответствиеПолейУсловия.Вставить("Магазин.", "Склады.");
	ТекстНачальногоУсловия = "И";
	УсловиеГруппы = "";
	ТекстУсловия = "";
	СформироватьТекстУсловия(Запрос,
	...
	СчетчикУсловий = 0;
	СоответствиеПолейУсловия = Новый Соответствие;
	СоответствиеПолейУсловия.Вставить("ФорматМагазина", "Склад.ФорматМагазина");
	...
	СчетчикУсловий = 0;
	СоответствиеПолейУсловия = Новый Соответствие;
	СоответствиеПолейУсловия.Вставить("ФорматМагазина", "АналитикаУчетаНоменклатуры.Склад.ФорматМагазина");
