...
Функция ИмяОбработкиВыгрузки() Экспорт
	...
	Возврат "ОбменУправлениеТорговлейБухгалтерияПредприятияКОРПВыгрузка";
...
Функция ИмяОбработкиЗагрузки() Экспорт
	...
	Возврат "ОбменУправлениеТорговлейБухгалтерияПредприятияКОРПЗагрузка";
...
Функция КраткаяИнформацияПоОбмену() Экспорт
	...
	ПоясняющийТекст = НСтр("ru = 'Позволяет синхронизировать данные между конфигурацией Управление торговлей ред. 11 и Бухгалтерия Предприятия КОРП ред. 2. 
...
Функция ОписаниеОграниченийПередачиДанных(НастройкаОтборовНаУзле) Экспорт
	...
	ТекстОписания = ТекстОписания + Символы.ПС + "Настройка выгрузки подразделений: ";
	Если НастройкаОтборовНаУзле.ПравилаВыгрузкиПодразделений = "ТолькоОбособленные" Тогда
		ТекстОписания = ТекстОписания + "выгружаются только обособленные подразделения";
	Иначе
		ТекстОписания = ТекстОписания + "выгружаются все подразделения";
	КонецЕсли;
...
Функция ОписаниеЗначенийПоУмолчанию(ЗначенияПоУмолчаниюНаУзле) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСкладыВТабличнойЧастиДокументовЗакупки") 
		...
		Иначе
			ТекстОписания = ТекстОписания + "не указан";
		КонецЕсли;
...
Функция ЗаголовокКомандыДляСозданияНовогоОбменаДанными() Экспорт
	...
	Возврат "Создать обмен с конфигурацией ""Бухгалтерия предприятия КОРП, ред. 2.0""";
