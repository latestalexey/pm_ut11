...
////////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ЭЛЕМЕНТОВ ТАБЛИЦЫ ФОРМЫ ДЕРЕВО ПОЛЬЗОВАТЕЛЕЙ
...
////////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ЭЛЕМЕНТОВ ТАБЛИЦЫ ФОРМЫ ДЕРЕВО ВАРИАНТОВ АНАЛИЗА
	...
	Для Каждого ВариантАнализа Из ВариантыАнализаСохраненные Цикл 
		...
		Если ЗаписываемыеНастройки.Количество() > 0 Тогда
			Для Каждого ЗаписываемаяНастройка Из ЗаписываемыеНастройки Цикл 
				...
				НоваяСтрока.ВариантОтображения = Перечисления.ВариантыОтображенияВариантовАнализа.Диаграмма;
...
Процедура ЗаполнитьДеревоПользователей()
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	Пользователи.ТекущееПодразделение.ПометкаУдаления КАК ПодразделениеПометкаУдаления,
	               |	1 КАК ТипСтроки,
	               |	ВЫБОР
	               ...
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	Пользователи.ТекущееПодразделение.Наименование,
...
Процедура ЗаполнитьНастройкиДоступности()
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ВариантыАнализаЦелевыхПоказателейНастройкиДоступности.Пользователь.ТекущееПодразделение КАК Подразделение
	               |ИЗ
...
Функция ПолучитьТекстПакетаПользователей()
	...
	Возврат "ВЫБРАТЬ
	        ...
	        |	Справочник.Пользователи КАК Пользователи
	        |ГДЕ
	        |	Пользователи.ТекущееПодразделение В ИЕРАРХИИ (&Подразделения)
			|;";
...
Функция ПолучитьТекстПакетаПользователейПустогоПодразделения()
	...
	Возврат "ВЫБРАТЬ
	        ...
	        |	Справочник.Пользователи КАК Пользователи
	        |ГДЕ
	        |	Пользователи.ТекущееПодразделение = &Подразделение
			|;";
...
Функция ПолучитьВариантыАнализа(ЗНАЧ ПараметрыОтбора = Неопределено, РезультатВДерево = Истина, ВсеВариантыАнализа = Ложь)
	...
	Для Каждого НастройкаДоступностиЗначение Из НастройкиДоступностиЗначение Цикл 
		НастройкаДоступностиЗначение.Подразделение = НастройкаДоступностиЗначение.Пользователь.ТекущееПодразделение;
	...
	Если ИспользоватьНесохраненныеНастройки Тогда
		...
		|
		|УПОРЯДОЧИТЬ ПО ВариантыАнализаЦелевыхПоказателей.Владелец.РеквизитДопУпорядочивания, ВариантыАнализаЦелевыхПоказателей.Ссылка.РеквизитДопУпорядочивания
		|
		|%ПараметрыИтогов%";
	...
	Иначе
		...
		|	ВАЦПНастройкиДоступности.Пользователь.ТекущееПодразделение КАК Подразделение
	...
	Если НЕ ВсеВариантыАнализа Тогда
		Если ЕстьПустыеПодразделения Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "%ПараметрОтбора%", "ГДЕ ВАЦПНастройкиДоступности.Пользователь.ТекущееПодразделение = &Подразделение");
			Если ИспользоватьНесохраненныеНастройки Тогда
			...
			Иначе
				Запрос.Текст = СтрЗаменить(Запрос.Текст, "%ПараметрОтбораНесохраненных%", "ГДЕ НесохраненныеНастройкиДоступности.Пользователь.ТекущееПодразделение  = &Подразделение");
			КонецЕсли;
		...
		ИначеЕсли ЕстьПодразделения Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "%ПараметрОтбора%", "ГДЕ ВАЦПНастройкиДоступности.Пользователь.ТекущееПодразделение В ИЕРАРХИИ(&Подразделения)");
