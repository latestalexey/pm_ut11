...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если ОбменВыполняется Тогда
		...
		Результат = Вопрос("После закрытия формы обмен с оборудованием выполняться не будет. Закрыть форму и остановить обмен? ", РежимДиалогаВопрос.ДаНет);
...
Процедура ВыполнитьОбмен()
	...
	Для Каждого СтрокаТЧ Из Оборудование Цикл
		...
		Если СтрокаТЧ.ТипОборудования = ПредопределенноеЗначение("Перечисление.ТипыПодключаемогоОборудования.ВесыСПечатьюЭтикеток") Тогда
			Выполнено = ПодключаемоеОборудованиеOfflineКлиент.ВыгрузитьТоварыВВесы(МассивУстройств, ТекстСообщения, Ложь);
		Иначе
			Выполнено = ПодключаемоеОборудованиеOfflineКлиент.ВыгрузитьТоварыВККМOffline(МассивУстройств, ТекстСообщения, Ложь);
		КонецЕсли;
		
		СтрокаТЧ.СостояниеВыгрузки      = ТекстСообщения;
		СтрокаТЧ.ИндексКартинкиВыгрузки = ?(Выполнено > 0, 1, 0);
		...
		Если СтрокаТЧ.ТипОборудования = ПредопределенноеЗначение("Перечисление.ТипыПодключаемогоОборудования.ККМOffline") Тогда
			...
			ТекстСообщения = "";
			Выполнено = ПодключаемоеОборудованиеOfflineКлиент.ЗагрузитьОтчетОРозничныхПродажах(МассивУстройств, ТекстСообщения, Ложь);
			
			СтрокаТЧ.СостояниеЗагрузки      = ТекстСообщения;
			СтрокаТЧ.ИндексКартинкиЗагрузки = ?(Выполнено > 0, 1, 0);
