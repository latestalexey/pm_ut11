...
Процедура ОбновитьКомандыРезультатовСинхронизации()
	...
	Если КоличествоСинхронизаций <> 0 Тогда
		...
		Если ИспользуетсяВерсионирование Тогда
			...
			СтандартныеПодсистемыПереопределяемый.ПриОбновленииЗаголовковКомандМонитораОбменаДанными(ДатаПоследнейКоллизии, Истина);
			...
			СтандартныеПодсистемыПереопределяемый.ПриОбновленииЗаголовковКомандМонитораОбменаДанными(ДатаПоследнегоЗапретаЗагрузки, Ложь);
...
Процедура УстановитьЭлементыОднойСинхронизации()
	...
	Если НастроеннаяСинхронизация.ДатаПоследнейУспешнойЗагрузки <> ПустаяДата Тогда
		
		Элементы.ДатаУспешнойЗагрузки.Заголовок = Формат(НастроеннаяСинхронизация.ДатаПоследнейУспешнойЗагрузки, "ДЛФ=DDT");
		
	Иначе
		
		Элементы.ДатаУспешнойЗагрузки.Заголовок = НСтр("ru = 'Никогда'");
		
	КонецЕсли;
	
	Если НастроеннаяСинхронизация.ДатаПоследнейУспешнойВыгрузки <> ПустаяДата Тогда
		
		Элементы.ДатаУспешнойВыгрузки.Заголовок = Формат(НастроеннаяСинхронизация.ДатаПоследнейУспешнойВыгрузки, "ДЛФ=DDT");
		
	Иначе
		
		Элементы.ДатаУспешнойВыгрузки.Заголовок = НСтр("ru = 'Никогда'");
	...
	Если НастроеннаяСинхронизация.ДатаПоследнейУспешнойВыгрузки <> НастроеннаяСинхронизация.ДатаПоследнейВыгрузки
		...
		ПодсказкаВыгрузки = СтрЗаменить(ПодсказкаВыгрузки, "%ДатаОтправки%", Элементы.ДатаУспешнойВыгрузки.Заголовок);
		ПодсказкаВыгрузки = СтрЗаменить(ПодсказкаВыгрузки, "%ДатаПопытки%", Формат(НастроеннаяСинхронизация.ДатаПоследнейВыгрузки, "ДЛФ=DDT"));
	...
	Если НастроеннаяСинхронизация.ДатаПоследнейУспешнойЗагрузки <> НастроеннаяСинхронизация.ДатаПоследнейЗагрузки
		...
		|Последняя попытка: %ДатаПопытки%'");
		ПодсказкаЗагрузки = СтрЗаменить(ПодсказкаЗагрузки, "%ДатаПолучения%", Элементы.ДатаУспешнойЗагрузки.Заголовок);
		ПодсказкаЗагрузки = СтрЗаменить(ПодсказкаЗагрузки, "%ДатаПопытки%", Формат(НастроеннаяСинхронизация.ДатаПоследнейЗагрузки, "ДЛФ=DDT"));
	...
	Для Каждого Элемент ИЗ СписокПлановОбмена Цикл
		...
		Если МенеджерПланаОбмена.ИспользоватьПомощникСозданияОбменаДанными() 
			...
			Иначе
				...
				Если МенеджерПланаОбмена.ПланОбменаИспользуетсяВМоделиСервиса() Тогда
			...
			НастраиваемыеСинхронизации.Добавить(ИмяПланаОбмена, ЗаголовокКоманды + "...");
