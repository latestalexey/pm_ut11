...
Процедура ВывестиВариантыСГруппой(Варианты, ПеременныеКонтекста)
	...
	ДеревоРаспределения.Колонки.Добавить("ВсегоВложеныхПустыхСтрок", Новый ОписаниеТипов("Число"));
		...
		Если СтрокаРодителя <> Неопределено Тогда
			Пока СтрокаРодителя <> Неопределено Цикл
				Если ЕстьВарианты Тогда
					...
					СтрокаРодителя.ВсегоВложеныхПустыхСтрок = СтрокаРодителя.ВсегоВложеныхПустыхСтрок + 1;
				КонецЕсли;
	...
	КоличествоСтрок = КорневаяСтрока.ВсегоВложенныхВариантов + КорневаяСтрока.ВсегоВложенныхПодсистем + Макс(КорневаяСтрока.ВсегоВложеныхПустыхСтрок - 2, 0);
	...
	Для Каждого СтрокаДерева Из СтрокаРодителя.Строки Цикл
		...
		Если СтрокаДерева.КоличествоВариантов > 0 Тогда
			...
			Для Каждого Вариант Из СтрокаДерева.Варианты Цикл
				...
				// Если это не так, то исходную таблицу неоходимо скопировать
