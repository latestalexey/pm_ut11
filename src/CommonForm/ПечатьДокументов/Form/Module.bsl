...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НазваниеФайла = "";
	Попытка
		Для Каждого текОбъкт Из Параметры.ПараметрКоманды Цикл
			НазваниеФайла = НазваниеФайла+Строка(текОбъкт)+"_";
		КонецЦикла;
		Если НазваниеФайла <> "" Тогда
			НазваниеФайла=Лев(НазваниеФайла,СтрДлина(НазваниеФайла)-1);
		КонецЕсли;  
	Исключение
		Попытка
			Для Каждого текОбъкт Из Параметры.ПараметрыИсточника.ОбъектыНазначения Цикл
				НазваниеФайла = НазваниеФайла+Строка(текОбъкт)+"_";
			КонецЦикла;
			Если НазваниеФайла <> "" Тогда
				НазваниеФайла=Лев(НазваниеФайла,СтрДлина(НазваниеФайла)-1);
...
Функция ПоместитьТабличныеДокументыВоВременноеХранилище(НастройкиСохранения)
	...
	Для Каждого НастройкаПечатнойФормы Из НастройкиПечатныхФорм Цикл
		...
		Для Каждого ТипФайла Из ВыбранныеФорматыСохранения Цикл
			...
			ИмяФайла = ПолучитьИмяВременногоФайлаДляПечатнойФормы(НазваниеФайла,НастройкиФормата.Расширение,ИспользованныеИменаФайлов);
...
Процедура СохранитьПечатныеФормыВПапку(СписокФайловВоВременномХранилище, Папка = "")
	...
	Для Каждого ФайлДляЗаписи Из СписокФайловВоВременномХранилище Цикл
		...
		// Доработка +
		Если Параметры.ИмяМенеджераПечати = "Документ.КоммерческоеПредложениеКлиенту" Тогда 
		АК_ОбщегоНазначения.УстановитьПараметрыПечатиExcel(ПолноеИмяФайла,ОриентацияСтраницы.Ландшафт,0,0,0,0,Ложь);
		КонецЕсли;
		// Доработка -
	КонецЦикла;
