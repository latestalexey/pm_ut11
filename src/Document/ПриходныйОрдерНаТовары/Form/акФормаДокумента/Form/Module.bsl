...
Процедура ТоварыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если новаяСтрока Тогда
		Если НЕ ПроверитьЗаполнениеДанных(Элемент.ТекущиеДанные.Номенклатура,Элемент.ТекущиеДанные.Характеристика) Тогда
			...
			Элемент.ТекущиеДанные.акКоличествоГрузовыхМестПлан = 0;
			Элемент.ТекущиеДанные.акКоличествоГрузовыхМестФакт = 0;
            Элемент.ТекущиеДанные.КоличествоУпаковок =0 ;
			 Элемент.ТекущиеДанные.АККоличествоПлан =0 ;             			
			//}
...
//{ ООО "АСТЭК" Разработчик: Бурыкин Александр  06.12.2013
//&НаКлиенте
//Процедура ШтрихКодПоискПриИзменении(Элемент)	
//	Отбор = новый структура("ШтрихКодГрузовогоМеста",Элемент.ТекстРедактирования);
//	Для каждого СтрокаГрузовыеМеста Из Объект.акГрузовыеМеста.НайтиСтроки(Отбор) Цикл		
//		СтрокаГрузовыеМеста.КоличествоПринято = СтрокаГрузовыеМеста.КоличествоПринято + 1;		
//	КонецЦикла;	
//КонецПроцедуры


&НаКлиенте
Процедура ШтрихКодПоискОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, СтандартнаяОбработка)
	Отбор = новый структура("ШтрихКодГрузовогоМеста",Элемент.ТекстРедактирования);
	Для каждого СтрокаГрузовыеМеста Из Объект.акГрузовыеМеста.НайтиСтроки(Отбор) Цикл		
		СтрокаГрузовыеМеста.КоличествоПринято = СтрокаГрузовыеМеста.КоличествоПринято + 1;		
	КонецЦикла;	
