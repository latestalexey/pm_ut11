...
Процедура РассчитатьДатыОтгрузки() Экспорт
	...
	// Доработка Станислав + 18.02.2015
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ЗаказКлиента.Статус
	...
	СтарыйСтатус = РезультатЗапроса.Статус;
	
	Если Статус = Перечисления.СтатусыЗаказовКлиентов.КОбеспечению и СтарыйСтатус = Перечисления.СтатусыЗаказовКлиентов.Согласован Тогда
		 ЖелаемаяДатаОтгрузки = НачалоДня(ТекущаяДата()  + 60*(86400));
	КонецЕсли;	
	// Доработка Станислав - 18.02.2015
КонецПроцедуры
			...
			Для каждого колонка из ТБ1.Колонки Цикл
				...
				Иначе
					...
					Иначе
						...
						Если (Имя <> "АК_ИнформацияДляОтделаЗакупки") или (Имя <>"АК_ПроцентНаценкиДоставкаАгент") или (Имя <>"АК_ПроцентНаценкиМонтажАгент")тогда
...
Процедура ЗаполнитьДокументНаОснованииКоммерческогоПредложенияКлиенту(Знач Основание)
	...
	|		АК_ПунктДоставки,
	|		АК_ПунктМонтажа,
	|		АК_ДопКомплектация КАК АК_ДопКомплектация1,
	|		АК_ПунктДоставки КАК АК_ПунктДоставки1,
	|		АК_ПунктМонтажа КАК АК_ПунктМонтажа1,
	|		АК_ПроцентНаценкиДоставка КАК АК_ПроцентНаценкиДоставка1,
	|		АК_ПроцентНаценкиДоставкаКлиент,
	|		АК_ПроцентНаценкиДоставкаАгент,
	|		АК_ПроцентНаценкиМонтаж КАК АК_ПроцентНаценкиМонтаж1,
	|		АК_ПроцентНаценкиМонтажКлиент,
	|		АК_ПроцентНаценкиМонтажАгент,
	|		АК_ДопУслугиСумма КАК АК_ДопУслугиСумма1,
	|		АК_ДопУслугиСуммаНДС КАК АК_ДопУслугиСуммаНДС1,
	|		АК_ПризнакНестандарт КАК АК_ПризнакНестандарт1,
	|		АК_НестандартОписание КАК АК_НестандартОписание1,
	|		АК_ПризнакЭскиз КАК АК_ПризнакЭскиз1,
	|		АК_ЭскизФайл КАК АК_ЭскизФайл1,
	|		АК_ИнформацияДляОтделаЗакупки КАК АК_ИнформацияДляОтделаЗакупки1
	|	) КАК Товары,
