...
Функция ПолучитьДанныеТабЧасти(Ссылка)
	ТекстЗапроса = "ВЫБРАТЬ
	               |	Товары.Номенклатура,
	               |	ВЫБОР
	               |		КОГДА Товары.АК_ПризнакНестандарт
	               |			ТОГДА Товары.АК_НестандартОписание
	...
	Пока выборка.Следующий() Цикл
		...
		|Код,Количество,Цена,Сумма,СтавкаНДС,Описание,СуммаНДС,УчитыватьНДС,НомерСтроки");
...
Процедура СформироватьПечатнуюФормуСпецификации(МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого СсылкаНаОбъект Из МассивОбъектов Цикл
		...
		Для Каждого ТекСтрока ИЗ ТЗ Цикл
			...
			ДокументВорд.Tables(1).Cell(НомерСтроки, 3).Range.Text = Строка(Выборка.Номенклатура);
			ДокументВорд.Tables(1).Cell(НомерСтроки, 4).Range.Text = Строка(Выборка.Характеристика);
