...
Функция ПолучитьТекстУсловияОплаты(ГрафикОплаты,Валюта,СуммаВсего,СуммаНДС)
	  ТекстУсловияОплаты = "Заказчик на основании счетов, выставленных Поставщиком, производит оплату:" + Символы.ПС;
			Для Каждого  СтрокаСправочника Из ГрафикОплаты.Этапы Цикл
				Если СтрокаСправочника.ВариантОплаты = Перечисления.ВариантыОплатыКлиентом.АвансДоОбеспечения Тогда
					ТекстУсловияОплаты = ТекстУсловияОплаты + "-" + СокрЛП(СтрокаСправочника.ПроцентПлатежа) + "% внесение аванса до обеспечения от общей суммы Договора, что составляет " + Окр(СуммаВсего * СтрокаСправочника.ПроцентПлатежа / 100,2,1) + " (";
					...
					Если СуммаНДС Тогда
						ТекстУсловияОплаты = ТекстУсловияОплаты + " (в т.ч. НДС: " +  РаботаСКурсамиВалют.СформироватьСуммуПрописью(Окр(СуммаНДС * СтрокаСправочника.ПроцентПлатежа / 100,2,1), Валюта) + " руб. ) ";
					Иначе
						ТекстУсловияОплаты = ТекстУсловияОплаты + "(НДС не облагается) ";
					КонецЕсли;
					ТекстУсловияОплаты = ТекстУсловияОплаты + "в течение " + СтрокаСправочника.Сдвиг + "-и банковских дней с момента подписания Спецификации." + Символы.ПС;
					
				ИначеЕсли СтрокаСправочника.ВариантОплаты = Перечисления.ВариантыОплатыКлиентом.КредитПослеОтгрузки Тогда
					ТекстУсловияОплаты = ТекстУсловияОплаты + "-" + СокрЛП(СтрокаСправочника.ПроцентПлатежа) + "% кредита покупателю от общей суммы Договора, что составляет" + Окр(СуммаВсего * СтрокаСправочника.ПроцентПлатежа / 100,2,1) + " (";
					ТекстУсловияОплаты = ТекстУсловияОплаты + РаботаСКурсамиВалют.СформироватьСуммуПрописью(Окр(СуммаВсего * СтрокаСправочника.ПроцентПлатежа / 100,2,1), Валюта)+ " )";
					...
					Иначе
						ТекстУсловияОплаты = ТекстУсловияОплаты + "(НДС не облагается). ";
					КонецЕсли;
					ТекстУсловияОплаты = ТекстУсловияОплаты + "Заказчик производит оплату в течение " + СтрокаСправочника.Сдвиг + "-и банковских дней  с момента поставки ему товара , если иное не указанно в соответствующей Спецификации." + Символы.ПС;
					
				ИначеЕсли СтрокаСправочника.ВариантОплаты = Перечисления.ВариантыОплатыКлиентом.ПредоплатаДоОтгрузки тогда
					ТекстУсловияОплаты = ТекстУсловияОплаты + "-" + СокрЛП(СтрокаСправочника.ПроцентПлатежа) + "% предоплаты от общей суммы Договора, что составляет " + Окр(СуммаВсего * СтрокаСправочника.ПроцентПлатежа / 100,2,1) + " (";
					ТекстУсловияОплаты = ТекстУсловияОплаты + РаботаСКурсамиВалют.СформироватьСуммуПрописью(Окр(СуммаВсего * СтрокаСправочника.ПроцентПлатежа / 100,2,1), Валюта) + " )";
					Если СуммаНДС Тогда
						ТекстУсловияОплаты = ТекстУсловияОплаты + " (в т.ч. НДС: " +  РаботаСКурсамиВалют.СформироватьСуммуПрописью(Окр(СуммаНДС * СтрокаСправочника.ПроцентПлатежа / 100,2,1), Валюта) + " руб. ). ";
					Иначе
						ТекстУсловияОплаты = ТекстУсловияОплаты + "(НДС не облагается). ";
					КонецЕсли;
						ТекстУсловияОплаты = ТекстУсловияОплаты + "Заказчик производит оплату в течение " + СтрокаСправочника.Сдвиг + "-и банковских дней  с момента готовности товара к отгрузке по письму-уведомлению, если иное не указанно в соответствующей Спецификации." + Символы.ПС;
				КонецЕсли; 
			КонецЦикла; 
...
Процедура СформироватьПечатнуюФормуСпецификации(МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого СсылкаНаОбъект Из МассивОбъектов Цикл
		...
		//Доработка + (Станислав)
			...
			ТекстСрокПоставки  	= "Срок поставки: " + СокрЛП(СтруктураДанных.СрокПоставки) + "дней";
			ЗаменитьПараметр(ДокументВорд,"УсловияОплаты",ТекстУсловияОплаты);
