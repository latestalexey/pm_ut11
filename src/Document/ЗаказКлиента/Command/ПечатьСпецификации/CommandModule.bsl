	  ...
	  ТекстУсловияОплаты = "Заказчик на основании счетов, выставленных Поставщиком, производит оплату:" + Символ(13)+Символ(10);
			Для Каждого  СтрокаСправочника Из ГрафикОплаты.Этапы Цикл
				Если СтрокаСправочника.ВариантОплаты = Перечисления.ВариантыОплатыКлиентом.АвансДоОбеспечения Тогда
					...
					КонецЕсли;
					ТекстУсловияОплаты = ТекстУсловияОплаты + "в течение " + СтрокаСправочника.Сдвиг + "-и банковских дней с момента подписания Спецификации." + Символ(13)+Символ(10);
				...
				ИначеЕсли СтрокаСправочника.ВариантОплаты = Перечисления.ВариантыОплатыКлиентом.ПредоплатаДоОтгрузки тогда
					...
					Иначе
						ТекстУсловияОплаты = ТекстУсловияОплаты + "(НДС не облагается). ";
					КонецЕсли;
						ТекстУсловияОплаты = ТекстУсловияОплаты + "Заказчик производит оплату в течение " + СтрокаСправочника.Сдвиг + "-и банковских дней  с момента готовности товара к отгрузке по письму-уведомлению, если иное не указанно в соответствующей Спецификации." + Символ(13)+Символ(10);
				КонецЕсли; 
			КонецЦикла; 
