...
Функция ПолучитьСтруктуруДанных(СсылкаНаОбъект)
	...
	Если Выборка.Следующий() Тогда
		...
		ОстаткиРеквизитов = ОписаниеОрганизации(СведенияОрганизация, "ИНН")+ Символы.ПС + Символы.ВК +
		ОписаниеОрганизации(СведенияОрганизация, "КПП")+ Символы.ПС + Символы.ВК +
		ОписаниеОрганизации(СведенияОрганизация, "НомерСчета,КоррСчет,Банк,БИК") + Символы.ПС + Символы.ВК +
		...
		//
		//Доработка +
		Если  Справочники.Контрагенты.ПустаяСсылка() = Выборка.Контрагент Тогда
				
			КонтрагентПоПартнеру = Справочники.Контрагенты.НайтиПоРеквизиту("Партнер",Партнер);
			КонтрагентНаименованиеПолное = КонтрагентПоПартнеру.НаименованиеПолное; 
			КонтрагентРуководитель = КонтрагентПоПартнеру.АК_ТекущийРуководитель;
			КонтрагентРуководительНаименование = КонтрагентПоПартнеру.АК_ТекущийРуководитель.Наименование;
			КонтрагентРуководительДолжность = "" + КонтрагентПоПартнеру.АК_ТекущийРуководитель.ДолжностьПоВизитке;
			
			Если НЕ ПустаяСтрока(КонтрагентРуководительДолжность)  Тогда 
			КонтрагентРуководительДолжность = КонтрагентРуководительДолжность + " " + КонтрагентНаименованиеПолное;
			КонецЕсли;
		    СтруктураРезультата.Вставить("КонтрагентНаименованиеКратко",КонтрагентНаименованиеПолное);
		Иначе
		//Доработка -
...
Процедура ОбработкаКоманды(ПараметрКоманды, ПараметрыВыполненияКоманды)
	...
	Если УправлениеПечатьюКлиент.ПроверитьДокументыПроведены(ПараметрКоманды, ПараметрыВыполненияКоманды.Источник) Тогда
		...
		Для Каждого СсылкаНаОбъект Из ПараметрКоманды Цикл
			...
			ИмяФайлаБезРасширения = "Договор_мебель_"+СтрЗаменить(СтрЗаменить(Строка(СсылкаНаОбъект)," ","_"),":","-");
