...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЦенаВключетНДСДокумента = Параметры.ВозвратТоваровОтКлиента.ЦенаВключаетНДС;
...
Процедура ВыбратьВсеТоварыНаСервере(ЗначениеВыбора = Истина)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаТоваров.НайтиСтроки(Новый Структура("СтрокаВыбрана", Ложь)) Цикл
...
Процедура ЗаполнитьТаблицуТоваров(ЗаявкаНаВозвратТоваровОтКлиента, ВозвратТоваровОтКлиента, ВалютаДокумента)
	...
	ВыборкаТовары = Документы.ВозвратТоваровОтКлиента.ПолучитьРезультатЗапросаПоОстаткамЗаявкиНаВозврат(
		ЗаявкаНаВозвратТоваровОтКлиента,
		ВозвратТоваровОтКлиента
	).Выбрать();
	...
	Пока ВыборкаТовары.Следующий() Цикл
		...
		СтрокаТаб = ТаблицаТоваров.Добавить();
			...
			"КодСтроки,Номенклатура, Характеристика, НоменклатураОприходование, ХарактеристикаОприходование, Порча, Упаковка, Количество, Цена, СтавкаНДС,
			|СтавкаНДС, ДатаПоступления"
		);
		...
		Если ПересчитатьСтроки Тогда
			Ценообразование.ПересчитатьСуммыВСтрокеПоСуммеСНДС(СтрокаТаб, ЦенаВключетНДСДокумента, Истина, Истина, Истина);
		КонецЕсли;
