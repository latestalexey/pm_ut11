...
Процедура ПриУстановкеНовогоНомера(СтандартнаяОбработка, Префикс)
	...
	// Доработка + (Станислав 15.01.2014)
	Если Год(Дата) = Год(ТекущаяДата()) Тогда
	Выборка = Документы.СчетФактураВыданный.Выбрать(НачалоГода(ТекущаяДата()));	
	Иначе	
	Выборка = Документы.СчетФактураВыданный.Выбрать(ТекущаяДата()-60*60*24*90);
	КонецЕсли;
	// Доработка -
	ТекНомерЧисло = 0;
	Пока Выборка.Следующий() цикл
		...
		Позиция1 = Найти(ТекНомер,"/");
		Позиция2 = Найти(ТекНомер,"\");
		Если Позиция1 <> 0 Тогда
		ТекНомер = Сред(ТекНомер,1,Позиция1-1);
		КонецЕсли;
		Если Позиция2 <> 0 Тогда
		ТекНомер = Сред(ТекНомер,1,Позиция2-1);
		КонецЕсли;
				Попытка
