...
Функция ПолучитьРезультатЗапросаПоОстаткамЗаказов(ДанныеОтбора,
	...
	|		И (ТаблицаРасхождения.ЗаказПоставщику В (&МассивЗаказов) ИЛИ НЕ &ОтобратьПоЗаказу)
	|		И ВЫБОР
	|			КОГДА
	|				ВЫРАЗИТЬ(&СкладПоступления КАК Справочник.Склады).ЭтоГруппа
	|			ТОГДА
	|				ТаблицаРасхождения.Склад В ИЕРАРХИИ (&СкладПоступления) ИЛИ ТаблицаРасхождения.Склад = ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
	|			ИНАЧЕ
	|				ТаблицаРасхождения.Склад В (ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка), &СкладПоступления)
	|		КОНЕЦ
	|	) КАК ТаблицаЗаказы
	|	
	|	СГРУППИРОВАТЬ ПО
