...
&НаКлиенте
Процедура ЗаполнитьЦеныПоУмолчанию(Команда)
	ЗаполнитьЦеныПоУмолчаниюНаСервере();
КонецПроцедуры


&НаСервере
Процедура ЗаполнитьЦеныПоУмолчаниюНаСервере()
	текОб = РеквизитФормыВЗначение("Объект");
	Для Каждого  текСтрока из текОб.Товары Цикл
