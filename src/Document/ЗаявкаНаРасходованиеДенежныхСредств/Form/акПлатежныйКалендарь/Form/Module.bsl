...
Процедура СписокЗаявокПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	//Доработка +
	ИндексТекущегоЭлементаКоллекции = СписокЗаявок.Индекс(Элементы.СписокЗаявок.ТекущиеДанные);

	УстановитьОтборы();
	
	КоличествоЭлементовКоллекции = СписокЗаявок.Количество();
	Если ИндексТекущегоЭлементаКоллекции < КоличествоЭлементовКоллекции - 1 Тогда
		Элементы.СписокЗаявок.ТекущаяСтрока = СписокЗаявок[ИндексТекущегоЭлементаКоллекции].ПолучитьИдентификатор();
	КонецЕсли;	
	//Доработка -
КонецПроцедуры
...
Функция ОбновитьСодержимоеСчетаСервер(ЗаявкаСсылка)
	...
	КонецЕсли;
	НоваяСтрока	=	Новый Структура("Номенклатура,Количество,цена,Сумма");
	СтрокаОпаисания = СокрЛП(ЗаявкаСсылка.Заметки);
	СтрокаОпаисания = ?(СтрокаОпаисания="",
	                    ЗаявкаСсылка.НазначениеПлатежа,
						?(СокрЛП(ЗаявкаСсылка.НазначениеПлатежа)="",
						  СтрокаОпаисания, 
						  СтрокаОпаисания+", "+СокрЛП(ЗаявкаСсылка.НазначениеПлатежа)
						  )
						);
	НоваяСтрока.Номенклатура =СтрокаОпаисания;
	НоваяСтрока.Сумма = ЗаявкаСсылка.СуммаДокумента;
	НоваяСтрока.цена = ЗаявкаСсылка.СуммаДокумента;
...
Процедура АвтоТест_УтвердитьЗаявки(Команда) Экспорт
	...
	//ДанныеТабПоля = Элементы.СписокЗаявок.ТекущиеДанные;
	ИндексТекущегоЭлементаКоллекции = СписокЗаявок.Индекс(Элементы.СписокЗаявок.ТекущиеДанные);
	...
	Если ИндексТекущегоЭлементаКоллекции < КоличествоЭлементовКоллекции - 1 Тогда
		Элементы.СписокЗаявок.ТекущаяСтрока = СписокЗаявок[ИндексТекущегоЭлементаКоллекции].ПолучитьИдентификатор();
	КонецЕсли;	
	//СписокЗаявокДинамическийсписок.НайтиПоИдентификатору(НомерТекущейСтроки);
	//МассивСтрок = ДанныеТабПоля.НайтиСтроки(Новый Структура("ИмяКолонки", Значение));
	//Если МассивСтрок.Количество() > 0 Тогда
	//	Элементы.ТабПоле.ТекущаяСтрока = МассивСтрок[0].ПолучитьИдентификатор();
	//КонецЕсли;
...
&НаКлиенте
Процедура СписокЗаявокПримечаниеПриИзменении(Элемент)
	ИндексТекущегоЭлементаКоллекции = СписокЗаявок.Индекс(Элементы.СписокЗаявок.ТекущиеДанные);
	
	ТекущиеДанные = ТекущийЭлемент.ТекущиеДанные;
	Текст = ТекущийЭлемент.ТекущиеДанные.Примечание;
	УстановитьКомментарийВЗаявке(ТекущиеДанные.ЗаявкаСсылка,Текст);
...
//}
&НаСервере 
Процедура УстановитьКомментарийВЗаявке(Данные,ЗначениеПоля)
	ЗаявкаОбъект = Данные.получитьОбъект(); 
	ЗаявкаОбъект.Заметки = ЗначениеПоля;
...
Процедура	ЗаполнитьСписокЗаявок()
	...
	Для каждого СтрокаРезультата ИЗ Результат цикл
		...
		//Попытка
			НомерКатегории = СтрокаРезультата.НомерКатегории;
			//Если Сред(НомерКатегории,2,1) = "." Тогда
			//НомерКатегории = Сред(НомерКатегории,1,1);	
			//КонецЕсли;
		//Исключение
		//	НомерКатегории = 0;
		//КонецПопытки;
		Новаястрока.НомерКатегории = НомерКатегории;
		...
		Новаястрока.Примечание = СтрокаРезультата.Примечание;
	...
	КонецЦикла;
	ТабДок.Сортировать("ДатаПолучения Возр");	
...
Процедура УтвердитьИлиОтклонитьЗаявкуНаРасходованиеДС(СсылкаНаЗаявку,Действие)
	...
	Если ЗаявкаОбъект.Статус <> Перечисления.СтатусыЗаявокНаРасходованиеДенежныхСредств.Согласована Тогда
		Сообщить("Заявка не согласована! Не подлежить утверждению!");
		Возврат ;
	КонецЕсли;	
...
&НаКлиенте
Процедура УстановитьВидимостьДопИнформации(Команда)
	Если Элементы.ДополнительнаяИнформация.Видимость = Ложь Тогда
		Элементы.ДополнительнаяИнформация.Видимость = Истина;
		ЭтаФорма.КоманднаяПанель.ПодчиненныеЭлементы.ФормаУстановитьВидимостьДопИнформации.Заголовок = "Скрыть дно" ;
	Иначе
		Элементы.ДополнительнаяИнформация.Видимость = Ложь;
		ЭтаФорма.КоманднаяПанель.ПодчиненныеЭлементы.ФормаУстановитьВидимостьДопИнформации.Заголовок = "Отоборазить дно" ;
	КонецЕсли;
КонецПроцедуры
