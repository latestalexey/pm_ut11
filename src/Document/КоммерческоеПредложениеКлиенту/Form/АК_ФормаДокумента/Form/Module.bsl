...
Процедура ПерезаполнитьЗаказКлиентаНаСервере()
	...
	Если Выборка.Количество() > 0 Тогда			 
		Если (Найти(ПараметрыСеанса.ТекущийПользователь.Наименование,  "Савина") ) ИЛИ 
			 (Найти(ПараметрыСеанса.ТекущийПользователь.Наименование,  "astek")) ИЛИ 
			 (Найти(ПараметрыСеанса.ТекущийПользователь.Наименование,  "Ефременко")) Тогда
...
Процедура ТоварыПредварительноХарактеристикаПриИзменении(Элемент)
	...
	ТекущаяСтрока = Элементы.ТоварыПредварительно.ТекущиеДанные;
	//если цена заполненная, то производится корректировка цветов
	//2014-07-28.
