...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КоммерческоеПредложениеXLS_PaxMed") Тогда	
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "КоммерческоеПредложениеXLS_PaxMed", "Коммерческое предложение", ПечатьКоммПредложения(МассивОбъектов, ОбъектыПечати,"ПМ"));
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КоммерческоеПредложениеКартикиXLS_PaxMed") Тогда	
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "КоммерческоеПредложениеКартикиXLS_PaxMed", "Коммерческое предложение", ПечатьКоммПредложенияКартинки(МассивОбъектов, ОбъектыПечати,"ПМ"));
		//--------------------------------------------	
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КоммерческоеПредложениеКартикиXLS_ДокторМебель") Тогда	
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "КоммерческоеПредложениеКартикиXLS_ДокторМебель", "Коммерческое предложение", ПечатьКоммПредложенияКартинки(МассивОбъектов, ОбъектыПечати,"ДМ"));
		//--------------------------------------------
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КоммерческоеПредложениеXLS_ДокторМебель") Тогда	
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "КоммерческоеПредложениеXLS_ДокторМебель", "Коммерческое предложение", ПечатьКоммПредложения(МассивОбъектов, ОбъектыПечати,"ДМ"));
...
Функция ОбновитьЗаказКлиентаПоКоммерческомуПредложению(ЗаказКлиента, ДокументОснование) Экспорт
	...
	//1с-it.com Сохраняем соглашение 
	ДокументОбъект.Соглашение =  ЗаказКлиента.Соглашение;	
...
//
//
Функция ПечатьКоммПредложения(МассивОбъектов, ОбъектыПечати,Префикс = "")
	...
	Для Каждого СсылкаНаОбъект из МассивОбъектов Цикл
		...
		Запрос.Текст ="ВЫБРАТЬ
		              ...
		              |	ВЫБОР
		              |		КОГДА ЕСТЬNULL(ГабаритныеРазмеры.Значение, """") <> """"
		              |			ТОГДА ГабаритныеРазмеры.Значение
		...
		Если (тзТовары.Итог("СуммаСкидки") = 0) И (тзТовары.Итог("СуммаРучнойСкидки") = 0) Тогда
			Макет = ПолучитьМакет("АК_КоммерческоеПредложениеБезСкидки");	
		Иначе
			Макет = ПолучитьМакет("АК_КоммерческоеПредложение");		
		КонецЕсли;	
		
		Если Префикс = "ПМ" Тогда
			ШапкаЛоготип = Макет.ПолучитьОбласть("ЛогоПМ");
		ИначеЕсли Префикс = "ДМ" Тогда
		...
		//ЗАГОЛОВОК
		ЗаголовокПостоянная = Макет.ПолучитьОбласть("Заголовок");
	...
	//		//
			Если СтрокаТЗ.Выводить Тогда
				...
				//Удалим габариты из названия если они есть
				НовНаименование = СтрЗаменить(Строка(СтрокаТЗ.Номенклатура),СтрокаТЗ.Габариты,"");
				ТоварыСтрокаНоменклатура.Параметры.Номенклатура = НовНаименование;
				//
				...
				//
				НомерСтроки = НомерСтроки + 1;
				ВесИтого = ВесИтого + СтрокаТЗ.Количество *  СтрокаТЗ.Номенклатура.ПолучитьОбъект().Вес;
				ОбъемИтого = ОбъемИтого + СтрокаТЗ.Количество *  СтрокаТЗ.Номенклатура.ПолучитьОбъект().Объем;
	...
	//	
		...
		//Доработка + 
		ТоварыПодвалПостоянная.Параметры.ВесИтого = ВесИтого;
		ТоварыПодвалПостоянная.Параметры.ОбъемИтого = ОбъемИтого;
...
//
//
Функция ПечатьКоммПредложенияКартинки(МассивОбъектов, ОбъектыПечати,Префикс = "")
	...
	Для Каждого СсылкаНаОбъект из МассивОбъектов Цикл
		...
		Запрос.Текст ="ВЫБРАТЬ
		              ...
		              |	Таблица.Ссылка = &Ссылка
		              |	И Таблица.Активность
		              |	И НЕ Таблица.Номенклатура В (ЗНАЧЕНИЕ(Справочник.Номенклатура.АК_Доставка), ЗНАЧЕНИЕ(Справочник.Номенклатура.АК_Монтаж), ЗНАЧЕНИЕ(Справочник.Номенклатура.АК_ДоставкаАгент), ЗНАЧЕНИЕ(Справочник.Номенклатура.АК_ДоставкаКлиент), ЗНАЧЕНИЕ(Справочник.Номенклатура.АК_МонтажАгент), ЗНАЧЕНИЕ(Справочник.Номенклатура.АК_МонтажКлиент))
		              |
		              |ОБЪЕДИНИТЬ ВСЕ
		              |
		              |ВЫБРАТЬ
		...
		Если (тзТовары.Итог("СуммаСкидки") = 0) И (тзТовары.Итог("СуммаРучнойСкидки") = 0) Тогда
		// Макет и области
		Макет = ПолучитьМакет("АК_КоммерческоеПредложениеКартинкиБезСкидки");
		Иначе
	...
	//		//
			Если СтрокаТЗ.Выводить Тогда
				...
				Если Не СтрокаТЗ.Номенклатура.ФайлКартинки.Пустая() Тогда
					...
					ТекКартинка.Верх = 1;
					//ТекКартинка.ГраницаСверху  = Истина;
				...
				Цена = СтрокаТЗ.СуммаБезСкидки/СтрокаТЗ.Количество;
				Сумма = СтрокаТЗ.СуммаБезСкидки;
				ЦенаБезСкидки = СтрокаТЗ.СуммаБезСкидки/СтрокаТЗ.Количество;
				...
				//
				//Доработка +
				ТоварыСтрокаОсновнаяЧасть.Параметры.Цена = ЦенаБезСкидки;
				Если (тзТовары.Итог("СуммаСкидки") = 0) И (тзТовары.Итог("СуммаРучнойСкидки") = 0) Тогда
				Иначе
				ТоварыСтрокаОсновнаяЧасть.Параметры.СуммаСкидки = СуммаСкидки;
				ТоварыСтрокаОсновнаяЧасть.Параметры.СуммаСоСкидкой = СуммаСоСкидкой;				
