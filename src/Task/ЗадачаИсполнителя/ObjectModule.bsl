...
Процедура ОтправитьПисьмо()
	...
	Если Не ЗначениеЗаполнено(Исполнитель) тогда
		...
		Пока Выборка.Следующий() Цикл
			мИсполнитель = Выборка.Исполнитель;
			ТаблицаКИ = мИсполнитель.КонтактнаяИнформация;
			...
			НовыйПолучатель.Контакт = мИсполнитель;
			НовыйПолучатель.Представление = мИсполнитель;
			
		КонецЦикла;
	Иначе
		мИсполнитель = Исполнитель;
		ТаблицаКИ = мИсполнитель.КонтактнаяИнформация;
		НайденныеСтроки = ТаблицаКИ.НайтиСтроки(Новый Структура("Вид", 
		Справочники.ВидыКонтактнойИнформации.EmailПользователя));
