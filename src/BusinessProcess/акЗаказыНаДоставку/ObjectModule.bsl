...
Процедура ОбработкаЗаказаКлиентаПриСозданииЗадач(ТочкаМаршрутаБизнесПроцесса, ФормируемыеЗадачи, Отказ)
	Для каждого ТекСтр из ФормируемыеЗадачи цикл
		...
		ТекСтр.Наименование = "Проверить для доставки "+ТекСтр.БизнесПроцесс.ЗаказКлиента;
		ТекСтр.Описание = "Проверить заполнение реквизитов, уточнить контактные данные, уточнить данные о доставке, согласовать доставку с клиентом";
	КонецЦикла;
КонецПроцедуры
...
Процедура ОбработкаОтказаПриСозданииЗадач(ТочкаМаршрутаБизнесПроцесса, ФормируемыеЗадачи, Отказ)
	Для каждого ТекСтр из ФормируемыеЗадачи цикл
		...
		ТекСтр.АвторСтрокой =ПараметрыСеанса.ТекущийПользователь;
		ТекСтр.СрокИсполнения = КонецДня(ТекущаяДата()); 
		ТекСтр.Наименование = "Обработать отказ в доставке для " + ТекСтр.БизнесПроцесс.ЗаказКлиента;
...
Процедура ОбеспечитьЗаказПриСозданииЗадач(ТочкаМаршрутаБизнесПроцесса, ФормируемыеЗадачи, Отказ)
	Для каждого ТекСтр из ФормируемыеЗадачи цикл
		...
		ТекСтр.Наименование = "Сформировать доставку для "+ТекСтр.БизнесПроцесс.ЗаказКлиента;
		ТекСтр.Описание = "Сформировать заказ на доставку";
	КонецЦикла;
КонецПроцедуры
...
Процедура ПолучитьДокументыПриСозданииЗадач(ТочкаМаршрутаБизнесПроцесса, ФормируемыеЗадачи, Отказ)
	Для каждого ТекСтр из ФормируемыеЗадачи цикл
		...
		ТекСтр.ДатаНачала = ТекущаяДата();
		ТекСтр.АвторСтрокой =ПараметрыСеанса.ТекущийПользователь;
		ТекСтр.Наименование = "Закрыть заказы на доставку "+ТекСтр.БизнесПроцесс.ЗаказКлиента.Партнер;
		ТекСтр.Описание = "Закрыть заказы на доставку, сформировать приходные документы";
	КонецЦикла;
КонецПроцедуры
...
Процедура ОбеспечитьЗаказПриВыполнении(ТочкаМаршрутаБизнесПроцесса, Задача, Отказ)
	Если ЗаказыПоставщика.Количество() = 0 тогда
		Отказ = Истина;
		Сообщить("Заказ покупателя не размещен в заказах на доставку",СтатусСообщения.Внимание);	
	КонецЕсли;
	...
	Для каждого ТекДокумент из ЗаказыПоставщика цикл
		...
		Если НЕ ЗначениеЗаполнено(ТекДокумент.Документ.АК_ДатаВремяИсполненияУслуги) тогда
			Отказ = Истина;
			Сообщить("Не установлена дата доставки у документа " + ТекДокумент.Документ,СтатусСообщения.Внимание);	
		КонецЕсли;	
	КонецЦикла;
КонецПроцедуры
