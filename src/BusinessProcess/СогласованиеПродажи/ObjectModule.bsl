	...
	Если НеобходимоСогласовать Тогда
		...
		Если ТипПредмета = Тип("ДокументСсылка.ЗаказКлиента") Тогда
			...
			Если ПредметОбъект.Статус = Перечисления.СтатусыЗаказовКлиентов.НеСогласован Тогда
				ПредметОбъект.Статус = Перечисления.СтатусыЗаказовКлиентов.Согласован;
...
Процедура ЗаполнитьБизнесПроцессНаОснованииЗаказаКлиента(ДокументОснование)
	...
	Запрос = Новый Запрос("
		...
		|	ВЫБОР
		|		КОГДА
		|			ЗаказКлиента.Приоритет = ЗНАЧЕНИЕ(Перечисление.Приоритеты.Низкий)
		|		ТОГДА
		|			ЗНАЧЕНИЕ(Перечисление.ВариантыВажностиЗадачи.Низкая)
		|		КОГДА
		|			ЗаказКлиента.Приоритет = ЗНАЧЕНИЕ(Перечисление.Приоритеты.Средний)
		|		ТОГДА
		|			ЗНАЧЕНИЕ(Перечисление.ВариантыВажностиЗадачи.Обычная)
		|		КОГДА
		|			ЗаказКлиента.Приоритет = ЗНАЧЕНИЕ(Перечисление.Приоритеты.Высокий)
		|		ТОГДА
		|			ЗНАЧЕНИЕ(Перечисление.ВариантыВажностиЗадачи.Высокая)
		|	КОНЕЦ                         КАК Важность,
