...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.Проект.Гиперссылка = Объект.акПроект <> Неопределено И НЕ Объект.акПроект.Пустая();//АСТЕК
	
	ПредметСтрокой = ОбщегоНазначения.ПредметСтрокой(Объект.Предмет);	
	ПроектСтрокой = ОбщегоНазначения.ПредметСтрокой(Объект.акПроект);//АСТЕК
...
//Астек
&НаКлиенте
Процедура ПроектНажатие(Элемент, СтандартнаяОбработка)
	
	ОткрытьЗначение(Объект.акПроект);
	СтандартнаяОбработка = Ложь;
