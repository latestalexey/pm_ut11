...
Функция ВставитьПараметрыСкрипта(Знач Текст, Знач СтруктураПараметров)
	...
	Результат = СтрЗаменить(Результат, "[ИмяИсполняемогоФайлаКонфигуратора]"	, Подг(ИмяИсполняемогоФайлаКонфигуратора));
	Результат = СтрЗаменить(Результат, "[ИмяИсполняемогоФайлаКлиента]"			, Подг(ИмяИсполняемогоФайлаКлиента));
...
Функция ОбработатьСтраницуВыборРежимаОбновления(ВыдаватьСообщения = Истина, ФлагЗавершенияРаботы = Ложь)
	...
	ИначеЕсли Объект.РежимОбновления = 1 Тогда  // при завершении работы // Емец
