...
Процедура Настройка(Команда)
	...
	ОткрытьФормуМодально("Обработка.ОценкаПроизводительности.Форма.АвтоматическийЭкспортЗамеровПроизводительности");
...
Функция РассчетатьЦелевоеВремя(КлючеваяОперация, ЗначениеAPDEX, ИндексПериода)
	...
	ШагЧисло = 0;
	КоличествоШагов = 0;
	Если Не ЭтотОбъект.ПериодичностьДиаграммы(ШагЧисло, КоличествоШагов) Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	ПараметрыВычисления = ЭтотОбъект.СтруктураПараметровДляРасчётаАпдекса();
	...
	Пока Истина Цикл
		...
		ТаблицаКлючевыхОперацийРассчитанная = ЭтотОбъект.ВычислитьAPDEX(ПараметрыВычисления);
...
//
&НаСервере
Процедура ОбментятьПриоритеты(КлючеваяОперация1, Приоритет1, КлючеваяОперация2, Приоритет2)
...
Функция НастройкаВыполнена()
	...
	Для Каждого СтрокаТЧ Из Объект.Производительность Цикл
		...
		Если СтрокаТЧ.ЦелевоеВремя = 0 
			И СтрокаТЧ.КлючеваяОперация <> Справочники.КлючевыеОперации.ОбщаяПроизводительностьСистемы Тогда
