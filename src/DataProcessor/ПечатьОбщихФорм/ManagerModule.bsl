...
Процедура ЗаполнитьРеквизитыШапкиТОРГ12(ДанныеПечати, Макет, ТабличныйДокумент)
	...
	Если ТипЗнч(ДанныеПечати.Ссылка) = Тип("ДокументСсылка.ВозвратТоваровПоставщику") Тогда
		...
		//КонецЕсли;
		//
		//Дата = ДанныеПечати.ЗаказКлиента.Дата;
		//ПечатьОснования = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		//НСтр("ru = '%1 № %2 от %3'"),
		//"Счет на оплату", НомерНаПечать, Формат(Дата, "ДЛФ=DD"));
		
		ОбластьМакета.Параметры.Основание	= ДанныеПечати.Основание;	
	Иначе
		
	Если (ДанныеПечати.ГосконтрактДата = '00010101000000') И (ПустаяСтрока(ДанныеПечати.ГосконтрактНаименование)) Тогда
		...
		ОбластьМакета.Параметры.Основание	= ?(ДанныеПечати.ЗаказКлиента = Неопределено,"",ПечатьОснования);
	Иначе
		ПечатьОснования = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '%1 от %2'"),
		ДанныеПечати.ГосконтрактНаименование, Формат(ДанныеПечати.ГосконтрактДата, "ДЛФ=DD"));
		
		ОбластьМакета.Параметры.Основание = ПечатьОснования;
...
Функция итПолучитьСодержание(Строка) Экспорт
	Попытка
