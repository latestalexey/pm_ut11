...
Процедура КомандаГотово(Команда)
	...
	СтандартныеПодсистемыКлиентПереопределяемый.ПриЗавершенииРаботыПомощникаСозданияОбмена(ЭтаФорма, ЗапуститьМенеджерЗаданий());
...
&НаСервере
Процедура ОбновитьНадписиЗаголовковИнформационныхБаз(НастройкиЗачитаны = Ложь)
	...
	Если Объект.ЭтоНастройкаРаспределеннойИнформационнойБазы Тогда
		...
		Если ВариантРаботыМастера = "ПродолжитьНастройкуОбменаДанными" Тогда
			...
			ИмяВторойБазы = Объект.НаименованиеЭтойБазы + НСтр("ru = ' (Подчиненный узел)'");
			ИмяПервойБазы = Объект.НаименованиеВторойБазы + НСтр("ru = ' (Главный узел)'");
			
		Иначе
			
			// Правильно
			ИмяПервойБазы = Объект.НаименованиеЭтойБазы + НСтр("ru = ' (Главный узел)'");
	...
	Иначе
		...
		Если ВариантРаботыМастера = "ПродолжитьНастройкуОбменаДанными" Тогда
			...
			Если НастройкиЗачитаны Тогда
				...
				ИмяПервойБазы = Объект.НаименованиеВторойБазы;
...
Процедура ЗагрузитьПараметрыМастера(Отказ, АдресВременногоХранилища)
	...
	ОбновитьНадписиЗаголовковИнформационныхБаз(Истина);
