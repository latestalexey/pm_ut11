...
Процедура ТаблицаНастройкиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Для Сч = 0 По КоличествоСимволов - 1 Цикл
		Если Найти("0123456789", Сред(Поле.Имя, КоличествоСимволов - Сч, 1)) = 0 Тогда
	...
	Если СтрокаТаблицы.Свойство("СпособПередачи" + ИндексКолонки) Тогда
		...
		Если СтрокаТаблицы["СпособПередачи" + ИндексКолонки] <> СпособПередачиНеТребуется Тогда
			...
			ВозвращаемыеПараметры = ОткрытьФормуМодально("Обработка.НастройкаПередачиТоваровМеждуОрганизациями.Форма.ФормаРедактирования", СтруктураПараметры);
...
Процедура ЗаполнитьДеревоНастройки()
	...
	Для Каждого Реквизит из МассивРеквизитов Цикл
		...
		Если Элемент = Неопределено Тогда
			...
			Если Найти(ИмяЭлемента, "Валюта") > 0 Тогда
	...
	Для Сч = 0 По ИндексОрганизации Цикл
		...
		НовыйЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Текст", "Не передается");
