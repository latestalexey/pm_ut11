...
&НаКлиенте
Перем ФормаНастройкиЗначенийПоУмолчаниюБазыКорреспондента;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОписаниеЗначенийПоУмолчанию = ОписаниеЗначенийПоУмолчанию(ИмяПланаОбмена, ЗначенияПоУмолчаниюНаУзле);
	ОписаниеЗначенийПоУмолчаниюКорреспондента = ОписаниеЗначенийПоУмолчаниюКорреспондента(ИмяПланаОбмена, ЗначенияПоУмолчаниюНаУзлеБазыКорреспондента);
	
	ПараметрыУчетаПоясняющаяНадпись = МенеджерПланаОбмена.ПояснениеДляНастройкиПараметровУчета();
	ПараметрыУчетаКорреспондентаПоясняющаяНадпись = МенеджерПланаОбмена.ПояснениеДляНастройкиПараметровУчетаБазыКорреспондента();
	
	ТаблицыКорреспондента = ОбменДаннымиСервер.ТаблицыКорреспондентаДляЗначенийПоУмолчанию(ИмяПланаОбмена);
...
&НаСервереБезКонтекста
Функция ОписаниеЗначенийПоУмолчанию(Знач ИмяПланаОбмена, Знач Настройки)
	
	Возврат ПланыОбмена[ИмяПланаОбмена].ОписаниеЗначенийПоУмолчанию(Настройки);
	
КонецФункции

&НаСервереБезКонтекста
Функция ОписаниеЗначенийПоУмолчаниюКорреспондента(Знач ИмяПланаОбмена, Знач Настройки)
	
	Возврат ПланыОбмена[ИмяПланаОбмена].ОписаниеЗначенийПоУмолчаниюБазыКорреспондента(Настройки);
...
Процедура ОжиданиеАнализаДанных_ОбработкаДлительнойОперации(Отказ)
	...
	Попытка
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			ПараметрыМетода,
			НСтр("ru = 'Настройка синхронизации данных между приложениями в интернете (шаг 1)'")
		);
...
Процедура ВыполнитьАвтоматическоеСопоставлениеДанных(Отказ)
	...
	Попытка
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			ПараметрыМетода,
			НСтр("ru = 'Настройка синхронизации данных между приложениями в интернете (автоматическое сопоставление данных)'")
		);
...
Процедура ПолучитьСтатистикуСопоставления(Отказ)
	...
	Попытка
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			ПараметрыМетода,
			НСтр("ru = 'Настройка синхронизации данных между приложениями в интернете (получение статистики сопоставления)'")
		);
...
Процедура ОжиданиеСинхронизацииДанных_ОбработкаДлительнойОперации(Отказ)
	...
	Попытка
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			ПараметрыМетода,
			НСтр("ru = 'Настройка синхронизации данных между приложениями в интернете (синхронизация справочников)'")
		);
...
Процедура ПолучитьДанныеКорреспондента(Отказ)
	...
	Попытка
		...
		Сообщение = СообщенияВМоделиСервиса.НовоеСообщение(
			ОбменДаннымиВМоделиСервисаПовтИсп.СообщениеПолучитьДанныеКорреспондента()
		);
...
Процедура ПолучитьПараметрыУчетаКорреспондента(Отказ)
	...
	Попытка
		...
		// Отправляем сообщение корреспонденту
		Сообщение = СообщенияВМоделиСервиса.НовоеСообщение(
			ОбменДаннымиВМоделиСервисаПовтИсп.СообщениеПолучитьПараметрыУчетаКорреспондента()
		);
		Сообщение.Body.CorrespondentZone = ОбластьДанныхКорреспондента;
...
Процедура ОжиданиеСохраненияНастроек_ОбработкаДлительнойОперации(Отказ)
	...
	Попытка
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			ПараметрыМетода,
			НСтр("ru = 'Настройка синхронизации данных между приложениями в интернете (шаг 2)'")
		);
...
Процедура ЗафиксироватьСозданиеНастройкиОбменаВМенеджереСервиса(Отказ)
	...
	Попытка
		...
		Сообщение = СообщенияВМоделиСервиса.НовоеСообщение(
			ОбменДаннымиВМоделиСервисаПовтИсп.СообщениеВключитьСинхронизацию()
		);
		Сообщение.Body.CorrespondentZone = ОбластьДанныхКорреспондента;
		...
		Сообщение = СообщенияВМоделиСервиса.НовоеСообщение(
			ОбменДаннымиВМоделиСервисаПовтИсп.СообщениеПротолкнутьСинхронизациюДвухПриложений()
		);
		Сообщение.Body.CorrespondentZone = ОбластьДанныхКорреспондента;
