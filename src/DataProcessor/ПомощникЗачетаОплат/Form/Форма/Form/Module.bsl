...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПравоЗачетаОплаты = Пользователи.РолиДоступны("ЗачетОплаты");
	Если Не ПравоЗачетаОплаты Тогда
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если МассивСтрок.Количество() > 0 Тогда
		...
		ОтветНаВопрос = Вопрос(НСтр("ru = 'Данные зачета оплаты были изменены. Сохранить изменения?'"), РежимДиалогаВопрос.ДаНетОтмена);
...
///////////////////////////////////////////////////////////////////////////////
// Заполнение таблицы расчетов с партнерами
...
Функция ПолучитьСуммуКОплате()
	...
	Запрос.УстановитьПараметр("ОбъектРасчетов", ?(РасчетыПоДоговору, Договор, Документ));
...
Процедура ПолучитьРеквизитыРасчетовСПартнером()
	...
	Если СтруктураРеквизитов <> Неопределено Тогда
		...
		КонецЕсли;
		Если СтруктураРеквизитов.Свойство("РасчетыПоДоговору") Тогда
			РасчетыПоДоговору = СтруктураРеквизитов.РасчетыПоДоговору;
		КонецЕсли;
...
Процедура ЗаполнитьТаблицуПоРасчетамСПартнерами()
	...
	Если (Не ЕстьРасчетыСКлиентами И Не ЕстьРасчетыСПоставщиками)
	 ИЛИ (ПоЗаказу И Не РасчетыПоДоговору) Тогда
		...
		Пока ВыборкаПоПартнерам.Следующий() Цикл
			...
			Пока ВыборкаПоВалютам.Следующий() Цикл
				...
				Пока Выборка.Следующий() Цикл
					...
					Если РасчетыПоДоговору И Выборка.Заказ = Договор Тогда
						НоваяСтрока.СуммаЗачетаОплаты = НоваяСтрока.СуммаАванса;
						НоваяСтрока.ДоступноКЗачету = 0;
					ИначеЕсли Не РасчетыПоДоговору И Выборка.Заказ = Документ Тогда
						НоваяСтрока.СуммаЗачетаОплаты = НоваяСтрока.СуммаАванса;
						НоваяСтрока.ДоступноКЗачету = 0;
					КонецЕсли;
		...
		Если (СтрокаДокумента.Заказ = Документ И ОрганизацияВДокументе = Организация)
		 ...
		 ИЛИ Не ЗначениеЗаполнено(СтрокаДокумента.Заказ) Тогда
			...
			Иначе
				...
				Если РасчетыПоДоговору И СтрокаДокумента.Заказ <> Договор Тогда
					СтрокаДокумента.Заказ = Договор;
					ДанныеИзменены = Истина;
				ИначеЕсли Не РасчетыПоДоговору И СтрокаДокумента.Заказ <> Документ Тогда
...
Процедура СохранитьРезультатЗачетаОплаты(МассивОбработанныхДокументов, Отказ = Ложь)
	...
	Для Каждого СтрокаТаблицы Из Оплаты Цикл
		...
		Если СтрокаТаблицы.ДанныеИзменены И СтрокаТаблицы.ДоступноРедактирование Тогда
			СуммаВВалютеДокумента = ?(СтрокаТаблицы.СуммаАванса <> 0, СтрокаТаблицы.СуммаЗачетаОплаты * СтрокаТаблицы.СуммаДокумента / СтрокаТаблицы.СуммаАванса, 0);
