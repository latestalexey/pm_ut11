...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
    ...
    Элементы.ИсторияОтборов.Видимость = ПравоДоступа("СохранениеДанныхПользователя", Метаданные);
...
Процедура ПодтвердитьВыбор(Команда)
    ...
    ОповеститьОВыборе(Новый Структура(
        ...
        Параметры.ДействиеВыбора, ПоместитьВоВременноеХранилище(Объект, УникальныйИдентификатор)
    ));
...
Процедура ИсторияОтборов(Команда)
    ...
    ВыбраныйВариант = ВыбратьИзМеню(СписокВариантов, Элементы.ИсторияОтборов);
...
&НаСервере
Процедура ОбновитьКоличествоСервер()
...
Функция ЗагрузитьЗначенияКоличеств()
    ...
    Для Каждого Строка Из Объект.ДополнительнаяРегистрация Цикл
        ...
        КолвоВыгружать = 0;
        КолвоВсего     = 0;
        СоставСтроки = ЭтотОбъект.СоставУкрупненнойГруппыМетаданных(Строка.ПолноеИмяМетаданных);
...
Процедура УстановитьНастройкиСервер(ПредставлениеНастройки)
    ...
    ЭтотОбъект = ЭтотОбъект();
    ЭтотОбъект.ВосстановитьТекущееИзНастроек(
