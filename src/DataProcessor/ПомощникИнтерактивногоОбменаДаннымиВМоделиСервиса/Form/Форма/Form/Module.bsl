...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
    ...
    ИначеЕсли Не Параметры.Свойство("УзелИнформационнойБазы", Объект.УзелИнформационнойБазы) Тогда
		ТекстОшибки = НСтр("ru='Непосредственное открытие помощника не предусмотрено.'");
    ...
    Если ТекстОшибки<>Неопределено Тогда
        ОбменДаннымиСервер.СообщитьОбОшибке(ТекстОшибки, Отказ);
        Возврат;
    КонецЕсли;
...
Процедура ДополнениеВыгрузкиЗарегистрироватьДанные()
    ...
    ДополнениеВыгрузкиОбъект.ЗарегистрироватьДополнительныеИзменения(
        ПоместитьВоВременноеХранилище(ДополнениеВыгрузкиОбъект, УникальныйИдентификатор)
    );
...
Процедура ДополнениеВыгрузкиОткрытьДетальныйПодбор()
    ОткрытьФорму("Обработка.ИнтерактивноеИзменениеВыгрузки.Форма",
        Новый Структура(
            "ДействиеВыбора, АдресОбъекта", 2, ДополнениеВыгрузкиАдресОбъекта()
        ),
        Элементы.ДополнениеВыгрузкиОбщийПериодДокументов
    );        
КонецПроцедуры
...
Функция Подключаемый_Окончание_ПриОткрытии(Отказ, ПропуститьСтраницу, ЭтоПереходДалее)
    ...
    // Регистрируем все, что накликано
