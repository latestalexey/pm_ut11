...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КлассОшибкиСозданиеСлужебногоПользователя = АвтономнаяРаботаСлужебный.КлассОшибкиСозданиеСлужебногоПользователя();
	...
	Если ПолучитьПроверкуСложностиПаролейПользователей() Тогда
		Элементы.ПояснениеКПаролю.Заголовок = 
			НСтр("ru = 'Пароль должен содержать 7 и более символов, используйте буквы и цифры.
			|Необходимо наличие строчных и заглавных букв (например: Qazx123)'"
		);
	Иначе
		Элементы.ПояснениеКПаролю.Заголовок =
			НСтр("ru = 'Нежелательно, чтобы пароль совпадал с именем пользователя, полностью состоял из цифр, содержал понятные слова, содержал чередующиеся группы символов.'"
		);
	КонецЕсли;
...
Процедура ОбработчикОжиданияДлительнойОперации()
	...
	Исключение
		...
		КлассОшибки = КлассОшибкиСозданиеСлужебногоПользователя;
		
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		КраткоеПредставлениеОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		
		Если Найти(ПодробноеПредставлениеОшибки, КлассОшибки) > 0 Тогда
			
			ПодробноеПредставлениеОшибки = СтрЗаменить(ПодробноеПредставлениеОшибки, КлассОшибки, "");
			КраткоеПредставлениеОшибки = СтрЗаменить(КраткоеПредставлениеОшибки, КлассОшибки, "");
			
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(КраткоеПредставлениеОшибки,, "Объект.ПарольДляСинхронизацииДанных");
		...
		ЗаписатьОшибкуВЖурналРегистрации(
			ПодробноеПредставлениеОшибки, СобытиеЖурналаРегистрацииСозданиеАвтономногоРабочегоМеста
		);
	КонецПопытки;
...
Процедура СоздатьНачальныйОбразАвтономногоРабочегоМестаНаСервере(Отказ)
	...
	Попытка
		...
		Если ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
			...
			Попытка
				ОбработкаОбъект.СоздатьНачальныйОбразАвтономногоРабочегоМеста(НастройкаОтборовНаУзле,
	...
	Исключение
		...
		Если Найти(ПодробноеПредставлениеОшибки, КлассОшибки) > 0 Тогда
			...
			ПодробноеПредставлениеОшибки = АвтономнаяРаботаСлужебный.УдалитьКлассОшибки(ПодробноеПредставлениеОшибки, КлассОшибки);
			КраткоеПредставлениеОшибки = АвтономнаяРаботаСлужебный.УдалитьКлассОшибки(КраткоеПредставлениеОшибки, КлассОшибки);
			
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(КраткоеПредставлениеОшибки,, "Объект.ПарольДляСинхронизацииДанных",, Отказ);
...
Функция ОписаниеОграниченийПередачиДанных(Знач ИмяПланаОбмена, НастройкаОтборовНаУзле)
	...
	Возврат ПланыОбмена[ИмяПланаОбмена].ОписаниеОграниченийПередачиДанных(НастройкаОтборовНаУзле);
...
Функция Подключаемый_НастройкаВыгрузки_ПриПереходеДалее(Отказ)
	...
	Если ПустаяСтрока(Объект.ПарольДляСинхронизацииДанных) Тогда
		
		НСтрока = НСтр("ru = 'Не задан пароль для синхронизации данных.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтрока,,"Объект.ПарольДляСинхронизацииДанных",, Отказ);
		
	ИначеЕсли Объект.ПарольДляСинхронизацииДанных <> ПодтверждениеПароля Тогда
