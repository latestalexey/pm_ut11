	...
	Для Каждого ОчищаемаяНастройка Из ОчищаемыеНастройки Цикл
		...
		Для Каждого Источник Из Источники Цикл
			...
			Если ОчищаемаяНастройка = "ПрочиеПользовательскиеНастройки" Тогда
				...
				СтандартныеПодсистемыПереопределяемый.ПолучитьПрочиеПользовательскиеНастройки(Источник, НастройкиБыстрогоДоступа);
				...
				КонецЕсли;
				СтандартныеПодсистемыПереопределяемый.УдалитьПрочиеПользовательскиеНастройки(Ключи, Источник);
				Продолжить;
			КонецЕсли;
			...
			Если ОчищаемаяНастройка = "НастройкиОтчетов" Тогда
				...
				Для Каждого ВариантОтчета Из ТаблицаПользовательскихВариантовОтчетов Цикл
					...
					СтандартныеПодсистемыПереопределяемый.УдалитьПользовательскийВариантОтчета(ВариантОтчета, Источник, СтандартнаяОбработка);
...
Функция КопированиеНастроекПользователей(ПользовательИсточникСсылка, ПользователиПриемник, КопируемыеНастройки,
	...
	// Получение настроек быстрого доступа к дополнительным отчетам и обработкам
	НастройкиБыстрогоДоступа = Неопределено;
	СтандартныеПодсистемыПереопределяемый.ПолучитьПрочиеПользовательскиеНастройки(ПользовательИсточникСсылка, НастройкиБыстрогоДоступа);
	Ключи = Новый СписокЗначений;
	Если НастройкиБыстрогоДоступа <> Неопределено Тогда
	...
	Для Каждого КопируемаяНастройка Из КопируемыеНастройки Цикл
		...
		Если КопируемаяНастройка = "ПрочиеПользовательскиеНастройки" Тогда
			СтандартныеПодсистемыПереопределяемый.СохранитьПрочиеПользовательскиеНастройки(Ключи, ПользователиПриемник);
			Продолжить;
		КонецЕсли;
...
Процедура УдалитьВариантыОтчетов(МассивВариантовОтчетов, ТаблицаПользовательскихВариантовОтчетов, ПользовательИнформационнойБазы) Экспорт
	...
	Для Каждого Настройка Из МассивВариантовОтчетов Цикл
		...
		СтандартнаяОбработка = Истина;
		СтандартныеПодсистемыПереопределяемый.УдалитьПользовательскийВариантОтчета(НайденныйВариантОтчета[0], ПользовательИнформационнойБазы, СтандартнаяОбработка);
		Если СтандартнаяОбработка Тогда
...
Функция ПользовательскиеВариантыОтчетов(ПользовательИнформационнойБазы)
	...
	Для Каждого ОтчетМетаданные Из Метаданные.Отчеты Цикл
		...
		СтандартнаяОбработка = Истина;
		СтандартныеПодсистемыПереопределяемый.ПолучитьПользовательскиеВариантыОтчетов(ОтчетМетаданные, ПользовательИнформационнойБазы, ТаблицаВариантовОтчетов, СтандартнаяОбработка);
		Если СтандартнаяОбработка Тогда
...
Процедура ЗаполнитьСписокПрочихНастроек(Форма)
	...
	СтандартныеПодсистемыПереопределяемый.ПолучитьПрочиеПользовательскиеНастройки(Форма.ПользовательСсылка, Результат);
	Ключи = Новый СписокЗначений;
