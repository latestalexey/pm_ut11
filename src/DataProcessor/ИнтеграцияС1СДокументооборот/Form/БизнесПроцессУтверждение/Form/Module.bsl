...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли ЗначениеЗаполнено(Параметры.ID) И ЗначениеЗаполнено(Параметры.Type) Тогда
   	...
   	Иначе 
		ОбъектXDTO= ИнтеграцияС1СДокументооборот.ПолучитьНовыйБизнесПроцесс(Прокси, "DMBusinessProcessConfirmation");
		ЗаполнитьФормуИзОбъектаXDTO(ОбъектXDTO);
	КонецЕсли;
	...
	Если Параметры.Свойство("Шаблон") Тогда
		Если Параметры.Свойство("Предмет") Тогда
			ШаблонПроцесса = ИнтеграцияС1СДокументооборот.ПолучитьНовыйБизнесПроцессПоШаблону(Прокси, "DMBusinessProcessConfirmation", Параметры.Шаблон, Параметры.Предмет);
		Иначе
			ШаблонПроцесса = ИнтеграцияС1СДокументооборот.ПолучитьНовыйБизнесПроцессПоШаблону(Прокси, "DMBusinessProcessConfirmation", Параметры.Шаблон);
		КонецЕсли;
...
Процедура ЗаполнитьФормуИзОбъектаXDTO(ОбъектXDTO)
	...
	Если ОбъектXDTO.DigitalSignEnabled Тогда
		ПодписыватьЭЦП = ОбъектXDTO.UseDigitalSign;
	Иначе
...
Функция ПодготовитьБизнесПроцесс(Прокси)
	...
	ОбъектXDTO = ИнтеграцияС1СДокументооборот.ПодготовитьШапкуБизнесПроцесса(Прокси, "DMBusinessProcessConfirmation", ЭтаФорма);
	
	ОбъектXDTO.useDigitalSign = ПодписыватьЭЦП;
