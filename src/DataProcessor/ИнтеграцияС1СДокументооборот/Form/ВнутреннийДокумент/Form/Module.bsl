...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ID) И ЗначениеЗаполнено(Тип) Тогда 
		...
		Заголовок = ОбъектXDTO.name + НСтр("ru = ' (Внутренний документ)'");
...
Процедура ПапкаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ИнтеграцияС1СДокументооборотКлиент.ВыбратьЗначениеИзСписка("DMInternalDocumentFolder", "Папка", ЭтаФорма);
...
Процедура ПапкаОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		ИнтеграцияС1СДокументооборотВызовСервера.ПолучитьДанныеДляАвтоПодбора("DMInternalDocumentFolder", ДанныеВыбора, Текст, СтандартнаяОбработка);
		
		Если ДанныеВыбора.Количество() = 1 Тогда 
			ИнтеграцияС1СДокументооборотКлиент.ОбработкаВыбораДанныхДляАвтоПодбора("Папка", ДанныеВыбора[0].Значение, СтандартнаяОбработка, ЭтаФорма);
...
Процедура ПапкаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ИнтеграцияС1СДокументооборотКлиент.ОбработкаВыбораДанныхДляАвтоПодбора("Папка", ВыбранноеЗначение, СтандартнаяОбработка, ЭтаФорма);
...
Процедура КорреспондентОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		...
		Если ДанныеВыбора.Количество() = 1 Тогда 
			ИнтеграцияС1СДокументооборотКлиент.ОбработкаВыбораДанныхДляАвтоПодбора("Корреспондент", ДанныеВыбора[0].Значение, СтандартнаяОбработка, ЭтаФорма);
			СтандартнаяОбработка = Истина;
		КонецЕсли;
	КонецЕсли;
...
Процедура КорреспондентОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ИнтеграцияС1СДокументооборотКлиент.ОбработкаВыбораДанныхДляАвтоПодбора("Корреспондент", ВыбранноеЗначение, СтандартнаяОбработка, ЭтаФорма);
...
Процедура КонтактноеЛицоНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(КорреспондентID) И ЗначениеЗаполнено(КорреспондентТип) Тогда 
		...
		ИнтеграцияС1СДокументооборотКлиент.ВыбратьЗначениеИзСписка("DMContactPerson", "КонтактноеЛицо", ЭтаФорма, Отбор);
...
Процедура КонтактноеЛицоОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		...
		Если ДанныеВыбора.Количество() = 1 Тогда 
			ИнтеграцияС1СДокументооборотКлиент.ОбработкаВыбораДанныхДляАвтоПодбора("КонтактноеЛицо", ДанныеВыбора[0].Значение, СтандартнаяОбработка, ЭтаФорма);
			СтандартнаяОбработка = Истина;
		КонецЕсли;
	КонецЕсли;
...
Процедура КонтактноеЛицоОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ИнтеграцияС1СДокументооборотКлиент.ОбработкаВыбораДанныхДляАвтоПодбора("КонтактноеЛицо", ВыбранноеЗначение, СтандартнаяОбработка, ЭтаФорма);
...
Процедура ПорядокПродленияОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		ИнтеграцияС1СДокументооборотВызовСервера.ПолучитьДанныеДляАвтоПодбора("DMProlongationProcedure", ДанныеВыбора, Текст, СтандартнаяОбработка);
		
		Если ДанныеВыбора.Количество() = 1 Тогда 
			ИнтеграцияС1СДокументооборотКлиент.ОбработкаВыбораДанныхДляАвтоПодбора("ПорядокПродления", ДанныеВыбора[0].Значение, СтандартнаяОбработка, ЭтаФорма);
			СтандартнаяОбработка = Истина;
		КонецЕсли;
	КонецЕсли;
...
Процедура ВидДокументаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ИнтеграцияС1СДокументооборотКлиент.ВыбратьЗначениеИзСписка("DMInternalDocumentType", "ВидДокумента", ЭтаФорма) Тогда 
...
Процедура ВидДокументаОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		ИнтеграцияС1СДокументооборотВызовСервера.ПолучитьДанныеДляАвтоПодбора("DMInternalDocumentType", ДанныеВыбора, Текст, СтандартнаяОбработка);
...
Процедура ПрочитатьОбъектВФорму(ОбъектXDTO)
	...
	Элементы.Корреспондент.Видимость  	  = (ОбъектXDTO.documentType <> Неопределено И ОбъектXDTO.documentType.correspondentEnabled = Истина); 
	Элементы.КонтактноеЛицо.Видимость 	  = (ОбъектXDTO.documentType <> Неопределено И ОбъектXDTO.documentType.correspondentEnabled = Истина); 
	...
	Элементы.Бессрочный.Видимость 	  	  = (ОбъектXDTO.documentType <> Неопределено И ОбъектXDTO.documentType.durationEnabled = Истина); 
	Элементы.ПорядокПродления.Видимость   = (ОбъектXDTO.documentType <> Неопределено И ОбъектXDTO.documentType.durationEnabled = Истина); 
	Элементы.ДатаНачалаДействия.Видимость = (ОбъектXDTO.documentType <> Неопределено И ОбъектXDTO.documentType.durationEnabled = Истина); 
	Элементы.ДатаОкончанияДействия.Видимость = (ОбъектXDTO.documentType <> Неопределено И ОбъектXDTO.documentType.durationEnabled = Истина); 
...
&НаСервере
Процедура ДобавитьПодписиКОбъекту(Прокси, ОбъектXDTO)
	
	МассивПодписей = ПолучитьМассивСуществующихПодписейФайла(ID);
	
	//добавим электронные подписи
...
Процедура СоздатьИзФайлаНаДиске()
	...
	Если РасширениеПодключено Тогда
		...
		ИдентификаторСозданногоФайла = ИнтеграцияС1СДокументооборотКлиент.СоздатьИзФайлаНаДискеКлиент(ПолноеИмяФайла, УникальныйИдентификатор, ID, Тип, Представление);
