...
Процедура ЗаполнитьРеквизитыШапкиСчетаНаОплату(ДанныеПечати, Макет, ТабличныйДокумент, ТаблицаЭтапыОплаты, Постфикс = "")
	...
	ТекстЗаголовка = СформироватьЗаголовокДокумента(ДанныеПечати, НСтр("ru='Счет на оплату'"),Постфикс);
...
// ИГОРЬ 08 04 2014
// Формируем заголовок своими средствами

Функция СформироватьЗаголовокДокумента(Шапка, знач НазваниеДокумента = "", Постфикс = "") Экспорт
	...
	// ИГОРЬ 03 04 2014
	// Добавим постфикс
	НомерНаПечать = НомерНаПечать + Постфикс;
	
	Дата = ДанныеДокумента.Дата;
	Если Шапка.Ссылка.АК_ОдинаковаяДата тогда
		Дата = ПолучитьДатуРеализации(Шапка.Ссылка);
		Дата = ?(Дата = Неопределено,ДанныеДокумента.Дата,Дата);
	КонецЕсли;
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '%1 № %2 от %3'"),
		НазваниеДокумента, НомерНаПечать, Формат(Дата, "ДЛФ=DD"));
	
КонецФункции

Функция ПолучитьДатуРеализации(Заказ)
	Запрос = Новый Запрос;
	...
	Иначе
		Сообщить("Не найден документ реализации");
...
Процедура ЗаполнитьТабличныйДокументСчетаНаОплату(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, КомплектыПечати)
	...
	Пока ДанныеПечати.Следующий() Цикл
		Для каждого ТекСтавкаНДС из Перечисления.СтавкиНДС цикл
			...
			// ИГОРЬ 03 04 2014
			Постфикс = ?(сч = 0, "" ,"/" + Строка(сч));
			Сч = сч + 1;
			ЗаполнитьРеквизитыШапкиСчетаНаОплату(ДанныеПечати, Макет, ТабличныйДокумент, ТаблицаЭтапыОплаты, Постфикс);
			...
			Иначе
				...
				Для Каждого СтрокаТовары Из ТаблицаТовары Цикл
					...
					ОписаниеНестандарта = "";
					НаименованиеПолное = СтрокаТовары.НаименованиеПолное;
					Характеристика = СтрокаТовары.Характеристика;
					
					Если СтрокаТовары.Нестандарт Тогда
						ГабаритыНестандарт = АК_ОбщегоНазначения.ПолучитьГабаритыИзСтроки(СтрокаТовары.НестандартОписание);
						ОписаниеНестандарта = ?(СокрЛП(СтрокаТовары.НестандартОписание)<>"",
						"/нестандарт "+СтрЗаменить(СокрЛП(СтрокаТовары.НестандартОписание),ГабаритыНестандарт,""),
						"");

						//если есть в описании габариты, замених их
						Если ГабаритыНестандарт <> "" Тогда
							//1.ищем в наименовании
							ГабаритыСтандарт = АК_ОбщегоНазначения.ПолучитьГабаритыИзСтроки(СтрокаТовары.НаименованиеПолное);
							Если ГабаритыСтандарт <> "" Тогда
								НаименованиеПолное = СтрЗаменить(СтрокаТовары.НаименованиеПолное,ГабаритыСтандарт,ГабаритыНестандарт);		
							КонецЕсли;
						    //2.ищем в характеристике
							ГабаритыСтандарт = АК_ОбщегоНазначения.ПолучитьГабаритыИзСтроки(СтрокаТовары.Характеристика);
							Если ГабаритыСтандарт <> "" Тогда
								Характеристика = СтрЗаменить(СтрокаТовары.Характеристика,ГабаритыСтандарт,ГабаритыНестандарт);		
							КонецЕсли;
