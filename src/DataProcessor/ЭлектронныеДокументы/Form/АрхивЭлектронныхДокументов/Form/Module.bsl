...
&НаСервере
Функция ПолучитьПрисоедиенныеФайлыПакетовЭДНаСервере(Знач ПакетыЭД)
	...
	|	Справочник.ЭДПрисоединенныеФайлы КАК ЭДПрисоединенныеФайлы
	|ГДЕ
	|	ЭДПрисоединенныеФайлы.ВладелецФайла В (&ВладелецыФайлов)";
	Запрос.УстановитьПараметр("ВладелецыФайлов", ПакетыЭД);
	
	РезультатЗапроса = Запрос.Выполнить().Выгрузить();
	ПрисоедиенныеФайлы = РезультатЗапроса.ВыгрузитьКолонку("Ссылка");
	
	Возврат ПрисоедиенныеФайлы;
...
Функция МассивСоглашений(МассивЭД)
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		МассивСоглашений = ТабРез.ВыгрузитьКолонку("СоглашениеЭД");
	КонецЕсли;
	
	Возврат МассивСоглашений;
...
Процедура Отправить(Команда)
	...
	МассивСоглашений = МассивСоглашений(МассивЭД);
	СтруктураПаролейИМаркеров = ЭлектронныеДокументыСлужебныйКлиент.СтруктураПаролейИМаркеров(МассивСоглашений);
	КолОтправленныхПакетов = ЭлектронныеДокументыСлужебныйКлиент.ОтправитьМассивПакетовЭД(
																		МассивЭД,
...
Процедура СохранитьПакетыЭДНаДиск(Команда)
	...
	ПрисоединенныеФайлы = ПолучитьПрисоедиенныеФайлыПакетовЭДНаСервере(Элементы.ВсеПакеты.ВыделенныеСтроки);
