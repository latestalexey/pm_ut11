...
Процедура ВыгрузитьРеквизитыОрганизации(АдресХранилища, УникальныйИдентификатор)
	...
	Попытка
		...
		Иначе
			...
			ЭлектронныеДокументыВнутренний.ОшибкаФормированияЭлектронногоДокумента("", СтруктураПараметров, ТекстОшибки);
		КонецЕсли;
	...
	Исключение
		...
		ЭлектронныеДокументыСлужебныйВызовСервера.ОбработатьИсключениеПоЭДНаСервере(НСтр("ru = 'Формирование ЭД'"), ТекстОшибки, ТекстСообщения);
	КонецПопытки;
...
&НаСервере
Процедура СохранитьДанныеВФайлНаСервере(Знач АдресВХранилище, ПолноеИмяФайла)
...
Процедура ВыгрузитьРеквизиты(Команда)
	...
	Если СпособВыгрузки = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезКаталог") Тогда
		...
		ПолучитьФайл(АдресХранилища, ИмяФайлаПоУмолчанию, Истина);
	...
	ИначеЕсли СпособВыгрузки = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезЭлектроннуюПочту") Тогда
		...
		ПолноеИмяФайла = ЭлектронныеДокументыСлужебныйВызовСервера.ТекущийКаталогВременныхФайлов() + ИмяФайлаДанных;
		СохранитьДанныеВФайлНаСервере(АдресХранилища, ПолноеИмяФайла);
