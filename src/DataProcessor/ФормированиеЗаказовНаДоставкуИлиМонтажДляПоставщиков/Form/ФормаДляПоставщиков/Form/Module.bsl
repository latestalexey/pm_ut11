...
Процедура СоздатьДокумент(Команда)
	...
	Для Каждого СтрокаТЧ Из Объект.ДополнительныеУслуги Цикл
		Если СтрокаТЧ.Выбрана  и  СтрокаТЧ.НомерМашины = ВыбЗнач.Значение Тогда
			...
			Форма.Объект.АК_ЗаказчикУслуги = ПОлучитьКлиента(СтрокаТЧ.ЗаказПоставщика);
...
функция ПОлучитьКлиента(ЗаказПоставщика)
	Возврат ЗаказПоставщика.Партнер;	
КонецФункции
...
Процедура ВыполнитьДействие(Действие,ТекЗаказ)
	Если Действие = "Добавить" тогда
		...
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ
		               |	ЗаказПоставщикуДополнительныеУслуги.Ссылка КАК ЗаказПоставщика,
		               |	ЗаказПоставщикуДополнительныеУслуги.Номенклатура,
		               |	ЗаказПоставщикуДополнительныеУслуги.Характеристика,
		               |	ЗаказПоставщикуДополнительныеУслуги.Количество,
		               ...
		               |ГДЕ
		               |	ЗаказПоставщикуДополнительныеУслуги.Ссылка = &Заказ";
		...
		Пока Выборка.Следующий() Цикл
			...
			новСтр.Выбрана = Истина;
			новСтр.ЗаказПоставщика = Выборка.ЗаказПоставщика;
			новСтр.ПунктПогрузки = Выборка.ПунктПогрузки;
			новСтр.ПунктДоставки = Выборка.ЗаказПоставщика.АК_ПунктНазначения;
		КонецЦикла;
	Иначе
		СтруктураОтбора = Новый Структура;
		СтруктураОтбора.Вставить("ЗаказПоставщика",ТекЗаказ);
...
Процедура ДополнительныеУслугиПриАктивизацииСтроки(Элемент)
	попытка
		ДопУслугиСсылкаНаЗаказ= Элемент.ТекущиеДанные.ЗаказПоставщика;
	Исключение
	КонецПопытки;
КонецПроцедуры              
