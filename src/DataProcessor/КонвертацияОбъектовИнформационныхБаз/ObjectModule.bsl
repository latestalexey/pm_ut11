...
Функция ИспользуетсяВерсионирование()
	...
	Если ТипЗнч(ПолеИспользуетсяВерсионирование) <> Тип("Булево") Тогда
		...
		СтандартныеПодсистемыПереопределяемый.ИспользуетсяВерсионирование(ПолеИспользуетсяВерсионирование);
...
Процедура ВыполнитьАнализСообщенияОбмена() Экспорт
	...
	Попытка
		...
		ТаблицаДанныхЗаголовкаПакета().Свернуть(
			"ТипОбъектаСтрокой, ТипИсточникаСтрокой, ТипПриемникаСтрокой, СинхронизироватьПоИдентификатору, ЭтоУдалениеОбъекта, ИспользоватьПредварительныйПросмотр",
	...
	Если Отказ Тогда
		...
		Если ИспользуетсяВерсионирование() И (ОбщегоНазначения.ЭтоДокумент(МетаданныеОбъекта) Или ОбщегоНазначения.ЭтоСправочник(МетаданныеОбъекта)) Тогда
			...
			Иначе
				...
				СтандартныеПодсистемыПереопределяемый.ПриСозданииВерсииОбъекта(ОбъектВбазе, , Перечисления["ТипыВерсийОбъекта"].ИзмененоПользователем,,,Истина);
				СтрокаСообщенияОбОшибке = Строка(СообщениеОбОшибке);
			КонецЕсли;
			
			СтандартныеПодсистемыПереопределяемый.ПриСозданииВерсииОбъекта(Объект, УзелОбменаЗагрузкаДанных,Перечисления["ТипыВерсийОбъекта"].НепринятыеДанныеПоДатеЗапретаОбъектСуществует,
	...
	Если ЕстьКоллизия Тогда
		...
		Если ИспользуетсяВерсионирование() И (ОбщегоНазначения.ЭтоДокумент(МетаданныеОбъекта) Или ОбщегоНазначения.ЭтоСправочник(МетаданныеОбъекта)) Тогда
			...
			Если ПриоритетОбъектаОбмена <> Перечисления.ПриоритетыОбъектовОбмена.ПриоритетПоступившегоПриОбменеОбъектаВыше Тогда
				...
				СтандартныеПодсистемыПереопределяемый.ПриСозданииВерсииОбъекта(Объект, УзелОбменаЗагрузкаДанных,
					Перечисления["ТипыВерсийОбъекта"].НепринятыеДанныеПоКоллизии, Комментарий, ,Истина);
			...
			ИначеЕсли ИмяТипаОбъекта <> "Перечисление" Тогда
				...
				Если РежимЗагрузкиДанныхВИнформационнуюБазу() Тогда
					...
					Если ЗаписатьОбъект Тогда
						...
						// Если объкт был успешно записан и создана ссылка,
			...
			Попытка
				...
				Иначе
					...
					ВыполнитьОбработчик_ПКС_ПриВыгрузкеСвойства(ФайлОбмена, Источник, Приемник, ВходящиеДанные, ИсходящиеДанные,
																...
																ИмяПКОВидСубконто, ВыгрузитьТолькоСсылку
					);
	...
	Пока ФайлОбмена.Прочитать() Цикл
		...
		Если    ИмяУзла = "Объект"
			...
			Иначе
				...
				Если Не ПКО.ПродолжитьПоискПоПолямПоискаЕслиПоИдентификаторуНеНашли Тогда
