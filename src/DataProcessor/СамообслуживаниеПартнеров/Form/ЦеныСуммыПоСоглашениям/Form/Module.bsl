...
Процедура СформироватьНаСервере()
	...
	|	ЕСТЬNULL(УпаковкиНоменклатуры.Коэффициент, 0) КАК КоэффициентУпаковки
	...
	|	МАКСИМУМ(ЕСТЬNULL(ЦеныНоменклатурыСрезПоследних.Упаковка, ЗНАЧЕНИЕ(Справочник.УпаковкиНоменклатуры.ПустаяСсылка))) КАК Упаковка
...
Функция ИмяУсловия(Имя)
	...
	ИначеЕсли Имя = "НалогообложениеНДС" Тогда
		Возврат НСтр("ru = 'Налогообложение НДС'");
