	...
	Для Каждого СтрокаСекции Из ДокументыКИмпорту Цикл
		...
		Если СтрокаСекции.Загружать Тогда
			...
			Если ПустаяСтрока(РезультатПроверки) Тогда
				//Доработка + (Станислав)
				Если  СтрокаСекции.ВидОперации= Перечисления.ХозяйственныеОперации.ПустаяСсылка() Тогда
					Если ЗначениеЗаполнено(СтрокаСекции.Списано)  Тогда
					  СтрокаСекции.ВидОперации = Перечисления.ХозяйственныеОперации.ОплатаПоставщику;
				  ИначеЕсли ЗначениеЗаполнено(СтрокаСекции.Поступило) Тогда
					  СтрокаСекции.ВидОперации = Перечисления.ХозяйственныеОперации.ПоступлениеОплатыОтКлиента;
				  КонецЕсли;
				  КонецЕсли;
				//Доработка -	(Станислав)
...
Процедура ЗаполнитьРеквизитыДокументаПоступлениеБезналичныхДенежныхСредств(ОбъектДокумента, ТипДокумента, СтрокаСекции, ЭтоНовыйДокумент, Организация)
	...
	// Доработка + (Станислав)
	Если СтрокаСекции.СтатьяДДС = Справочники.СтатьиДвиженияДенежныхСредств.ПустаяСсылка() Тогда
		СтрокаСекции.СтатьяДДС =  Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоНаименованию("Поступление оплаты от клиента",Истина,,);
	КонецЕсли;
	// Доработка -
			...
			Если Документы.ТипВсеСсылки().СодержитТип(ТипЗнч(СтрокаТаблицы.Заказ)) Тогда
				...
				Если Документы.ТипВсеСсылки().СодержитТип(ТипЗнч(СтрокаТаблицы.Заказ)) Тогда
					...
					Если СтрЧислоВхождений(НазначениеПлатежа, НомерДокументаРасчетов)>0 Тогда
						...
						УстановитьСвойство(СтрокаРасшифровки, "СтатьяДоходов", СтрокаСекции.СтатьяДоходовИлиРасходов, , ЭтоНовыйДокумент);
...
Процедура ЗаполнитьРеквизитыДокументаСписаниеБезналичныхДенежныхСредств(ОбъектДокумента, ТипДокумента, СтрокаСекции, ЭтоНовыйДокумент, Организация)
	...
	// Доработка + (Станислав)
	Если СтрокаСекции.СтатьяДДС = Справочники.СтатьиДвиженияДенежныхСредств.ПустаяСсылка() Тогда
		СтрокаСекции.СтатьяДДС =  Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоНаименованию("Оплата поставщику",Истина,,);
	КонецЕсли;
	// Доработка -
